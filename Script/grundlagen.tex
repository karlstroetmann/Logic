...
in dem obigen Text auftretenden Teilsätze in Terme.  Die folgende Tabelle zeigt diesen Prozeß: \\[0.3cm]
\hspace*{1.3cm} 
\begin{tabular}{ll}
...
\item Darüber hinaus läßt sich die Bedeutung von Formeln mathematisch definieren und
...
\item \emph{Junktoren} 
      ...
      die umgangssprachlichen Formulierung zwei verschiedene Interpretationen zuläßt.
      ...
      die beiden Operatoren ``$\wedge$'' und ``$\vee$'' die selbe Bindungsstärke.
...
\item \emph{Quantoren}
      ...
      Formal läßt sich das als \\[0.2cm]
...
\item Fall: $\neg\; R \in R$. Also enthält die Menge $R$ sich nicht selbst.
      ...
      enthalten, müßte $R$ eine Element von $R$ sein, es müsste also
...
Klammern ``\texttt{\{}'' und ``\texttt{\}}'' eingefaßt und durch Kommas getrennt.
...
bezeichnen.  Mit den bisher behandelten Verfahren läßt diese Menge sich nicht definieren.
...
\noindent
Der Begriff der Vereinigung von Mengen läßt sich verallgemeinern.  Betrachten
...
\noindent
Der Begriff der Schnitt-Menge von zwei Mengen läßt sich verallgemeinern.  Betrachten
...
\item[I.S.:] $n \mapsto n + 1$
     ...
     \item $b \in C$.
           ...
           Dann ist $C$ keine Teilmenge von $A$, aber $C$ läßt sich dann in der Form 
           \\[0.2cm]
           \hspace*{1.3cm}
...
Der Begriff des geordneten Paares läßt sich leicht zum Begriff des $n$-Tupels verallgemeinern:
...
Beispiel:  Es läßt sich zeigen, dass \\[0.2cm]
\hspace*{1.3cm} 
...
die in den folgenden Gleichungen zusammengefaßt sind.  Bevor wir diese Gesetze angeben, vereinbaren wir noch,
...
Zum Abschluß des Kapitels führen wir \emph{transitive Relationen} und
...
In diesem Fall läßt sich neben der Funktion
...
gilt, so muß dass $y$ dessen Existenz gefordert wird, den Wert $x$ haben und dann gilt
...
$\pair(x,y) \in R$ gelegentlich auch $x\, R\, y$.  Beispielsweise läßt sich die Relation $\leq$ 
...
\item $R_2 = \{ \pair(1,2), \pair(2,1), \pair(3,3) \}$.
      ...
      $\pair(2,1) \in R_2$ müßte $2=1$ folgen.
      Schließlich ist $R_2$ auch nicht transitiv, denn aus $\pair(1,2) \in R_2$ und 
      $\pair(2,1) \in R_2$ müßte $\pair(1,1) \in R_2$ folgen.
...
\item $R_3 := \{ \pair(n,m) \in \mathbb{N}^2 \mid n \leq m \}$.
      ...
      denn wenn $n \leq m$  und $m \leq n$ gilt, so muß schon $m = n$ gelten.
...
\item $R_4 := \{ \pair(m,n) \in \mathbb{N}^2 \mid \exists k\in \mathbb{N}: m\cdot k = n \}$
      ...
      sowohl $m$ ein Teiler von $n$ ist und auch $n$ ein Teiler vom $m$, so muß $m = n$
...
\noindent
Wir definieren den \emph{transitiven Abschluß} von $R$ als die Menge \\[0.2cm]
...
$\textsl{Eltern} := \{ \pair(x,y) \in \textsl{Mensch}^2 \mid \mbox{$x$ ist Vater von $y$ oder 
                                                                   $x$ ist Mutter von $y$} \}$
\\[0.2cm]
Dann besteht der transitive Abschluß der Relation \textsl{Eltern} aus allen Paaren
...
Der transitive Abschluß von $D$ enthält
...
diese Funktion definierten Äquivalenz-Relation sind dann alle die Menschen äquivalent, welche die selbe
...
\example
Setzen wir das letzte Beispiel fort, in dem alle die Menschen äquivalent waren, die die selbe
Staatsbürgerschaft haben, so finden wir, dass die Äquivalenz-Klassen, die von dieser
Äquivalenz-Relation erzeugt werden, gerade aus den Menschen besteht, die die selbe
...
aus jeder Partition einer Menge läßt sich eine Äquivalenz-Relation erzeugen.
