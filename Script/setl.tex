...
erfahrungsgemÃ¤Ã dem AnfÃ¤nger aufgrund ihrer Abstraktheit gewisse Schwierigkeiten.  Um
...
Diese Sprache basiert auf der Ende der sechziger Jahre von Jacob T.~Schwartz eingefÃ¼hrten
...
\section{EinfÃ¼hrende Beispiele}
...
Die Sprache \textsc{SetlX} ist eine interaktive Sprache, Sie kÃ¶nnen diese
Sprache also unmittelbar Ã¼ber einen Interpreter aufrufen.  Falls Sie \textsc{SetlX} auf
Ihrem Rechner installiert haben, kÃ¶nnen Sie den Befehl
\\[0.2cm]
\hspace*{1.3cm}
\texttt{setlx}
\\[0.2cm]
in einer Kommando-Zeile eingeben.  AnschlieÃend meldet sich der Interpreter dann wie wie
...
\texttt{1 + 2;}
\\[0.2cm]
ein und drÃ¼cken anschlieÃend auf die Eingabe-Taste, so erhalten Sie
...
Funktion, mit deren Hilfe Sie Werte ausdrucken kÃ¶nnen.  Diese Funktion heiÃt \texttt{print}.  Wenn
Sie im Interpreter
\\[0.2cm]
\hspace*{1.3cm}
\texttt{print(\symbol{34}Hallo\symbol{34});}
\\[0.2cm]
eingeben und anschlieÃend auf die Eingabe-Taste drÃ¼cken, so erhalten Sie
die folgende Ausgabe:
...
\begin{Verbatim}[ frame         = lines, 
    ...
    => 
\end{Verbatim}
Hier hat der Interpreter zunÃ¤chst den Befehl \texttt{print(\symbol{34}Hallo\symbol{34})} ausgefÃ¼hrt und dabei
...
berechnet, ist der RÃ¼ckgabe-Wert undefiniert.  Ein undefinierter Wert wird in
...
beispielsweise das Ergebnis der Rechnung $36 \cdot 37 / 2$ ausgeben wollen, so kÃ¶nnen Sie dies Ã¼ber den Befehl
\\[0.2cm]
\hspace*{1.3cm}
\texttt{print(\symbol{34}36 * 37 / 2 = \symbol{34}, 36 * 37 / 2);}
\\[0.2cm]
erreichen.  Wenn Sie nur an der Auswertung dieses Ausdrucks interessiert sind, so kÃ¶nnen Sie diesen 
Ausdruck auch unmittelbar hinter dem Prompt eingeben und mit einem Semikolon ``\texttt{;}'' abschlieÃen.
Wenn Sie nun die Eingabe-Taste betÃ¤tigen, wird der Ausdruck ausgewertet und das Ergebnis angezeigt.

Der \textsc{SetlX}-Interpreter lÃ¤sst sich nicht nur interaktiv betreiben, sondern er kann auch vollstÃ¤ndige
Programme ausfÃ¼hren.  Speichern wir das in Abbildung \ref{fig:sum.stlx} gezeigte 
\href{https://github.com/karlstroetmann/Logik/blob/master/SetlX/sum.stlx}{Programm} in einer Datei mit
dem Namen ``\texttt{sum.stlx}'' ab, so kÃ¶nnen wir in der Kommando-Zeile den Befehl
\\[0.2cm]
\hspace*{1.3cm}
\texttt{setlx sum.stlx}
\\[0.2cm] 
eingeben.  Dann wird zunÃ¤chst der Text ``\texttt{Type a natural number} $\cdots$'' gefolgt von einem
Doppelpunkt als Prompt ausgegeben.  Geben wir nun eine Zahl $n$ ein und betÃ¤tigen die Eingabe-Taste, so wird
als Ergebnis die Summe der ersten $n$ natÃ¼rlichen Zahlen, also
\\[0.2cm]
\hspace*{1.3cm}
$\sum\limits_{i=1}^n i$ 
\\[0.2cm]
...
Zeile fÃ¼r Zeile.  Die Zeilen-Nummern in dieser und den folgenden Abbildungen von \textsc{SetlX}-Programmen
sind nicht Bestandteil der Programme sondern wurden hinzugefÃ¼gt um in den Diskussionen
dieser Programme besser auf einzelne Zeilen Bezug nehmen zu kÃ¶nnen. 
\begin{enumerate}
\item Die erste Zeile enthÃ¤lt einen Kommentar.  In \textsc{SetlX} werden Kommentare
      ...
      einzeiligen Kommentaren unterstÃ¼tzt \textsc{SetlX} auch mehrzeilige Kommentare, die
      wie in der Sprache \texttt{C} durch die Strings ``\texttt{/*}'' und ``\texttt{*/}''
      begrenzt werden.
\item Die zweite Zeile enthÃ¤lt eine Zuweisung.  Die Funktion $\textsl{read}()$ gibt
      zunÃ¤chst den Text aus, der den Benutzer zur Eingabe einer Zahl auffordert und liest dann
      ...
      \texttt{C}:
      \begin{enumerate}
      \item \texttt{SetlX} verwendet den Zuweisungs-Operator ``\texttt{:=}'', wÃ¤hrend
            die Programmiersprache \texttt{C} den String ``\texttt{=}'' als
            Zuweisungs-Operator benutzt.
      \item Die Namen von Variablen oder Funktion mÃ¼ssen in der Sprache \textsc{SetlX} mit einem
            kleinen Buchstaben beginnen.  Daneben kÃ¶nnen die Namen von  Variablen und Funktionen
            noch beliebig viele Buchstaben, Ziffern und den Unterstrich ``\texttt{\_}'' enthalten.
            In Gegensatz dazu dÃ¼rfen diese Namen in \texttt{C} auch mit einem groÃen
      ...
      \emph{getypt}. Daher ist es weder notwendig noch mÃ¶glich, die Variable \texttt{n} zu
      deklarieren.  WÃ¼rde der Benutzer an Stelle einer Zahl einen String eingeben, so wÃ¼rde das
      Programm spÃ¤ter mit einer Fehlermeldung abbrechen.
\item Die dritte Zeile zeigt zunÃ¤chst, wie sich Mengen als AufzÃ¤hlungen definieren lassen.  Sind
      ...
      \\[0.2cm]
      Das ist natÃ¼rlich genau die Summe
      \\[0.2cm]
      \hspace*{1.3cm}
...
\end{enumerate}
Als nÃ¤chstes betrachten wir das in Abbildung \ref{fig:sum-recursive.stlx} auf Seite
...
\item Zeile 1 bis Zeile 7 enthalten die Definition der Prozedur \texttt{sum}.  Die 
      Definition einer Prozedur wird in \textsc{SetlX} durch das SchlÃ¼sselwort
      ``\texttt{procedure}'' eingeleitet.  Hinter diesem SchlÃ¼sselwort 
      folgt zunÃ¤chst eine Ã¶ffnende Klammer ``\texttt{(}'', dann
      eine Liste von Parametern, welche durch ``\texttt{,}'' voneinander getrennt sind, 
      und danach  eine schlieÃenden Klammer ``\texttt{)}''.
      ...
      \begin{enumerate}
      \item ZunÃ¤chst wird der Ausdruck \textsl{test} evaluiert.  Dabei
            muss sich entweder der Wert ``\texttt{true}'' oder ``\texttt{false}'' ergeben.
      \item Falls sich ``\texttt{true}'' ergibt, werden anschlieÃend die Kommandos
            in  \textsl{body}$_1$ ausgefÃ¼hrt.  Dabei ist \textsl{body}$_1$ 
            eine Liste von Kommandos.
      \item Andernfalls werden die Kommandos in der Liste \textsl{body}$_2$ ausgefÃ¼hrt.
      \end{enumerate}
      \textbf{Beachten Sie} hier die folgenden beiden \underline{Unterschiede} zur Sprache \texttt{C}:
      \begin{enumerate}
      \item Bei dem \texttt{if}-Befehl mÃ¼ssen Sie auch dann geschweifte Klammern
...
\item Dann wird in Zeile 10 fÃ¼r den eben eingelesenen Wert von \texttt{n} die oben definierte Prozedur
      \texttt{sum} aufgerufen.  

      ZusÃ¤tzlich enthÃ¤lt Zeile 10 eine Zuweisung: Der Wert, den der Prozedur-Aufruf
      \texttt{sum(n)} zurÃ¼ck liefert, wird in die Variable \texttt{total}, die auf der
      linken Seite des \emph{Zuweisungs-Operators} ``\texttt{:=}'' steht, geschrieben.
\item AnschlieÃend wird das berechnete Ergebnis durch einen \texttt{print}-Befehl ausgegeben.
\end{enumerate}
Die Prozedur \texttt{sum} in dem obigen Beispiel ist \emph{rekursiv}, d.h.~sie ruft sich
selber auf.  Die Logik, die hinter der Implementierung steht, lÃ¤sst sich am einfachsten
durch die beiden folgenden bedingten Gleichungen erfassen:
\begin{enumerate}
\item $\textsl{sum}(0) = 0$,
\item $n > 0 \rightarrow \textsl{sum}(n) = sum(n-1) + n$.
\end{enumerate}
Die Korrektheit dieser Gleichungen wird unmittelbar klar, wenn wir fÃ¼r $\textsl{sum}(n)$
die Definition
\\[0.2cm]
\hspace*{1.3cm}
...
sonst wÃ¼rde die Prozedur in einer Endlos-Schleife stecken bleiben. 

\section{Darstellung von Mengen}
Der wichtigste Unterschied zwischen der Sprache \textsc{SetlX} und der Sprache \texttt{C}
besteht darin, dass \textsc{SetlX} die Verwendung von Mengen und Listen unmittelbar unterstÃ¼tzt.
Um zu zeigen, wie wir in \textsc{SetlX} mit Mengen umgehen kÃ¶nnen, zeigen wir ein
...
\begin{Verbatim}[ codes         = {\catcode`$=3\catcode`_=8\catcode`^=7},
    ...
    c := 2 ** a;
    print("2 ** ", a, " = ", c);
    // \emph{ÃberprÃ¼fung einer Teilmengen-Beziehung}  a $\subseteq$ b
    print("(", a, " <= ", b, ") = ", (a <= b)); 
...
\noindent
In Zeile 1 und 2 sehen wir, dass wir Mengen ganz einfach durch explizite AufzÃ¤hlung ihrer
Argumente angeben kÃ¶nnen.  In den Zeilen 4, 7 und 10 berechnen wir dann nacheinander 
Vereinigung, Schnitt und Differenz dieser Mengen.  Hier ist zu beachten, dass dafÃ¼r
in \textsc{SetlX} die Operatoren ``\texttt{+}'', ``\texttt{*}'', ``\texttt{-}''
verwendet werden.  
In Zeile 13 berechnen wir die Potenz-Menge mit Hilfe des Operators ``\texttt{**}''.
Weiter Ã¼berprÃ¼fen wir in Zeile 17 mit dem Operator ``\texttt{<=}'', 
ob \texttt{a} eine Teilmenge von \texttt{b} ist.  SchlieÃlich testen wir in Zeile 18 mit Hilfe des
Operators ``\texttt{in}'', ob die Zahl 1 ein Element der oben definierten Menge \texttt{a} ist.

FÃ¼hren wir dieses Programm aus, so
...
\begin{verbatim}
    ...
    1 in {1, 2, 3} = true
\end{verbatim}
Um interessantere Programme zeigen zu kÃ¶nnen, stellen wir jetzt weitere
MÃ¶glichkeiten vor, mit denen wir in \textsc{SetlX} Mengen definieren kÃ¶nnen.

\subsubsection{Definition von Mengen durch arithmetische AufzÃ¤hlung}
In dem letzten Beispiel hatten wir Mengen durch explizite AufzÃ¤hlung definiert.  Das ist
bei groÃen Mengen viel zu mÃ¼hsam.  Eine Alternative ist daher die Definition einer Menge
durch eine \emph{arithmetische AufzÃ¤hlung}.  Wir betrachten zunÃ¤chst ein Beispiel: 
\begin{verbatim}
        a := { 1 .. 100 };
\end{verbatim}
Die Menge, die hier der Variablen \texttt{a} zugewiesen wird, ist die Menge aller
natÃ¼rlichen Zahlen von 1 bis 100.  Die allgemeine Form einer solchen Definition ist \\[0.2cm]
\hspace*{1.3cm} \texttt{a := \{ \textsl{start} .. \textsl{stop} \};} \\[0.2cm]
Mit dieser Definition wÃ¼rde \texttt{a} die Menge aller ganzen Zahlen von
...
Es gibt noch eine Variante der arithmetischen AufzÃ¤hlung, die wir ebenfalls durch ein
Beispiel einfÃ¼hren. 
\begin{verbatim}
        a := { 1, 3 .. 100 };
\end{verbatim}
Die Menge, die hier der Variablen \texttt{a} zugewiesen wird, ist die Menge aller
ungeraden natÃ¼rlichen Zahlen von 1 bis 100.  Die Zahl 100 liegt natÃ¼rlich nicht
...
Definieren wir $\textsl{step} = \textsl{second} - \textsl{start}$ und ist \textsl{step} positiv, so lÃ¤sst sich
...
Eine weitere MÃ¶glichkeit, Mengen zu definieren, ist durch die Verwendung von
\emph{Iteratoren} gegeben.  Wir geben zunÃ¤chst ein einfaches Beispiel: 
\\[0.2cm]
\hspace*{1.3cm} 
\texttt{p := \{ n * m : n in \{2..10\}, m in \{2..10\} \};} 
\\[0.2cm]
Nach dieser Zuweisung enthÃ¤lt \texttt{p} die Menge aller \emph{nicht-trivialen} Produkte, deren
Faktoren $\leq$ 10 sind.  (Ein Produkt der Form $a \cdot b$ heiÃt dabei \emph{trivial} genau
dann, wenn einer der Faktoren $a$ oder $b$ den Wert $1$ hat.)
In der Schreibweise der Mathematik  gilt fÃ¼r die oben definierte Menge \texttt{p}: 
\\[0.2cm]
\hspace*{1.3cm} 
...
$. 
\\[0.2cm]
Wie ausdruckstark Iteratoren sind, lÃ¤sst sich an dem Programm 
...
vorgegebenen GrÃ¶Ãe $n$ und ist so kurz wie eindrucksvoll.  Die zugrunde liegende
...
schreiben lÃ¤sst.  Um also die Menge der Primzahlen kleiner gleich $n$ zu
berechnen, ziehen wir einfach von der Menge  $\{ 2, \cdots, n \}$ die Menge aller Produkte
ab, denn dann bleiben genau die Primzahlen Ã¼brig.   Diese Rechnung wird in Zeile 2 des Programms
durchgefÃ¼hrt.  
...
$S_1$ bis $S_n$ AusdrÃ¼cke, die bei ihrer Auswertung Mengen ergeben.  Die
AusdrÃ¼cke ``\texttt{$x_i$ in $S_i$}'' werden dabei als \emph{Iteratoren} bezeichnet,
weil die Variablen $x_i$ Ã¼ber alle Werte der entsprechenden Menge $S_i$ laufen
...
\\[0.2cm]
gegeben.  Die Definition einer Menge Ã¼ber Iteratoren entspricht  der
Definition einer Menge als Bild-Menge.

Es ist in \textsc{SetlX} auch mÃ¶glich, das \emph{Auswahl-Prinzip} zu verwenden.  Dazu
kÃ¶nnen wir Iteratoren mit einer Bedingung verknÃ¼pfen.   Die allgemeine Syntax
dafÃ¼r ist: \\[0.2cm]
\hspace*{1.3cm}  
$\{ \textsl{expr} : x_1 \;\mathtt{in}\; S_1,\; \cdots,\; x_n \;\mathtt{in}\; S_n \mid \textsl{cond}\, \}$. 
\\[0.2cm]
Hierbei haben die AusdrÃ¼cke  $\textsl{expr}$ und $S_i$ dieselbe
Bedeutung wie oben und 
 $\textsl{cond}$ ist ein Ausdruck, der von den Variablen $x_1$, $\cdots$, $x_n$ abhÃ¤ngt und dessen Auswertung
...
\end{alltt}
 enthÃ¤lt \texttt{primes} die Menge aller Primzahlen, die
...
andere Zahl $x$ teilbar ist, kÃ¶nnen wir in \textsc{SetlX} den Operator
\texttt{\%} verwenden: Der Ausdruck $p \;\texttt{\%}\; x$ berechnet den Rest,
der Ã¼brig bleibt, wenn Sie die Zahl $p$ durch $x$ teilen.  
...
ZunÃ¤chst haben wir in den Zeilen 1 bis 3 die Funktion $\texttt{teiler}(p)$ definiert, die
...
kÃ¶nnen.  Ist $R$ eine solche Menge und $x \in \textsl{dom}(R)$, so bezeichnet in
\textsc{SetlX} der Ausdruck $R(x)$  das eindeutig bestimmte Element $y$, fÃ¼r das
...
ZusÃ¤tzlich zeigt das Programm noch, dass in \textsc{SetlX} bei einer binÃ¤ren
Relation $\textsl{dom}(R)$ als $\texttt{domain}(R)$ und $\textsl{rng}(R)$ als $\mathtt{range}(R)$ geschrieben wird.
AuÃerdem sehen wir in Zeile 2, dass wir den Wert einer funktionalen Relation durch eine
Zuweisung Ã¤ndern kÃ¶nnen.
...
\end{Verbatim} 
\vspace*{-0.3cm}
\caption{Rechnen mit binÃ¤ren Relationen.}  \label{fig:function.stlx}
\end{figure} %$

\noindent
Das Programm berechnet in Zeile 1 die binÃ¤re Relation $q$ so, dass $q$ die
Funktion $x \mapsto x^2$ auf der Menge $\{n\in \mathbb{N} \mid 1 \leq n \wedge n \leq 10 \}$
reprÃ¤sentiert.  

An dieser Stelle eine Warnung: In \textsc{SetlX} mÃ¼ssen alle Variablen mit einem kleinen
Buchstaben beginnen!  Normalerweise hÃ¤tte ich die Relation \texttt{q} mit einem groÃen
\texttt{Q} bezeichnen wollen, aber das geht nicht, denn alle Namen, die mit einem groÃen
Buchstaben beginnen, sind zur Darstellung von \emph{Termen} reserviert.  (Was genau \emph{Terme} in
\textsc{SetlX} sind, werden wir spÃ¤ter erklÃ¤ren.)

In Zeile 2 wird die Relation $q$ an der Stelle $x=5$ so abgeÃ¤ndert, dass nun $q(5)$ den Wert
$7$ hat.   AnschlieÃend werden noch $\textsl{dom}(Q)$ und $\textsl{rng}(Q)$ berechnet.
...
\end{Verbatim} 
\vspace*{-0.3cm}
\caption{Rechnen mit nicht-funktionalen binÃ¤ren Relationen.}  \label{fig:buggy-function.stlx}
\end{figure} %$

Falls die Menge  $r = \{ y \mid \langle x, y \rangle \in r \}$  entweder leer ist
oder mehr als eine Element enthÃ¤lt, so ist der Ausdruck $r(x)$ undefiniert.
Ein solcher Ausdruck wird in \textsc{SetlX} durch den String ``\texttt{om}'' dargestellt.
Der Versuch, einen undefinierten Wert in eine Menge $M$ einzufÃ¼gen, Ã¤ndert diese
Menge nicht, es gibt aber auch keine Fehlermeldung.  Deswegen wird in Zeile 4 fÃ¼r 
die Menge \texttt{\{ r(1), r(2) \}} einfach die leere Menge ausgegeben.

Will man das Auftreten von undefinierten Werten beim Auswerten einer binÃ¤ren
Relation $r$ vermeiden, so gibt es in \textsc{SetlX} die MÃ¶glichkeit, $r\{x\}$ statt
$r(x)$ zu schreiben, die runden Klammern werden also durch geschweifte Klammern ersetzt.  
Der Ausdruck $r\{x\}$  ist fÃ¼r eine binÃ¤re Relation $r$ wie folgt definiert: 
\\[0.2cm]
\hspace*{1.3cm}
...
Auch beliebige $n$-Tupel lassen sich in \textsc{SetlX} darstellen.  Diese kÃ¶nnen
ganz analog zu Mengen definiert werden.  Das geht denkbar einfach: Es mÃ¼ssen nur
alle geschweiften Klammern der Form ``\texttt{\{}'' und ``\texttt{\}}'' durch
die entsprechenden eckigen Klammern ``\texttt{[}'' und ``\texttt{]}'' ersetzt
werden.  Dann kÃ¶nnen Tupel durch explizite AufzÃ¤hlung, arithmetische AufzÃ¤hlung, Iteration und das
...
kÃ¶nnen.  Dieses Programm berechnet die Primzahlen nach dem selben 
...
\pageref{fig:sort.stlx} zeigt ein einfaches Verfahren
 um eine Liste von natÃ¼rlichen Zahlen zu sortieren.  
Der Ausdruck \\[0.2cm]
\hspace*{1.3cm}
\texttt{max(s)}
\\[0.2cm]
berechnet das grÃ¶Ãte Element der Liste $s$.  
Damit lÃ¤uft die Variable $n$ in dem Iterator
\\[0.2cm]
\hspace*{1.3cm}
\texttt{n in [1 .. max(s)]}
\\[0.2cm]
dann von 1 bis zur grÃ¶Ãten in $s$ auftretenden Zahl.  
Aufgrund der Bedingung ``\texttt{n in s}''
wird die Zahl $n$ genau dann in die resultierende Liste eingefÃ¼gt, wenn $n$ ein
Element der Liste $s$ ist.  Da der Iterator 
\\[0.2cm]
\hspace*{1.3cm}
``\texttt{n in [1 .. max(s)]}'' 
\\[0.2cm]
die Zahlen der Reihe nach aufzÃ¤hlt, ist das
Ergebnis eine sortierte Liste, die genau die Zahlen enthÃ¤lt, die Elemente von $s$
sind.

Offensichtlich ist der in der Prozedur $\texttt{sort}()$ implementierte Algorithmus nicht sehr effizient.
Wir werden spÃ¤ter noch effizientere Algorithmen diskutieren.
...
Weiterhin kÃ¶nnen die Operatoren ``\texttt{+/}''
und ``\texttt{*/}'' auf Mengen angewendet werden.  Der Operator  ``\texttt{+/}'' berechnet
die Summe aller Elemente einer Menge, wÃ¤hrend der Operator ``\texttt{*/}'' das Produkt der
Elemente berechnet.  Ist die zu Grunde liegende Menge oder Liste leer, so geben diese
Operatoren ``\texttt{+/}'' als Ergebnis ``\texttt{om}'' zurÃ¼ck.
Die Operatoren ``\texttt{+/}'' und ``\texttt{*/}'' kÃ¶nnen auch als binÃ¤re Operatoren
verwendet werden.  Ein Ausdruck der Form
\\[0.2cm]
\hspace*{1.3cm}
\texttt{x +/ s}
\\[0.2cm]
gibt als Ergebnis den Wert $x$ zurÃ¼ck, wenn die Menge $s$ leer ist.  Falls \texttt{s}
...
wird \texttt{x} zurÃ¼ck gegeben, sonst ist das Ergebnis das Produkt der Elemente aus \texttt{s}.

Als nÃ¤chstes besprechen wir die Funktion
``\texttt{from}'', mit dem wir ein (nicht nÃ¤her spezifiziertes) Element aus einer
Menge auswÃ¤hlen kÃ¶nnen.  Die Syntax ist: 
\\[0.2cm]
\hspace*{1.3cm} 
$x$ \texttt{:= from(}$s$\texttt{);}
\\[0.2cm]
Hierbei ist $s$ eine Menge und $x$  eine Variable.  Wird diese Anweisung
ausgefÃ¼hrt, so wird ein nicht nÃ¤her spezifiziertes Element aus der Menge $s$
entfernt.  Dieses Element wird darÃ¼ber hinaus der Variablen $x$ zugewiesen.
Falls $s$ leer ist, so erhÃ¤lt $x$ den undefinierten Wert ``\texttt{om}'' und
$s$ bleibt unverÃ¤ndert.  Das Programm 
...
``\texttt{arb}'',  die ein beliebiges Element aus einer Menge auswÃ¤hlt, die Menge selbst
aber unverÃ¤ndert lÃ¤sst.  Nach den Befehlen
\begin{verbatim}
    s := { 1, 2 };
    x := arb(s);
    print("x = ", x);
...
Weiterhin steht fÃ¼r Listen der Operator ``\texttt{+}''
zur VerfÃ¼gung, mit dem zwei Listen aneinander gehÃ¤ngt werden kÃ¶nnen.
AuÃerdem  gibt es noch den unÃ¤ren Operator ``\texttt{\#}'',
der fÃ¼r Mengen und Listen die Anzahl der Elemente berechnet.
SchlieÃlich kann man Elemente von Listen mit der Schreibweise \\[0.2cm]
\hspace*{1.3cm} \texttt{$x$ := $t$[$n$];} \\[0.2cm]
indizieren.  In diesem Fall muss $t$ eine Liste sein, die mindestens die LÃ¤nge
$n$ hat.  Die obige Anweisung weist der Variablen $x$ dann den Wert des $n$-ten
Elementes der Liste $t$ zu.  Die obige Zuweisung lÃ¤sst sich auch umdrehen: Mit \\[0.2cm]
\hspace*{1.3cm} \texttt{$t$[$n$] := $x$;} \\[0.2cm]
wird die Liste $t$ so abgeÃ¤ndert, dass das $n$-te Element danach den Wert $x$ hat.
...
\\[0.2cm]
ausfÃ¼hren, hat \texttt{x} also anschlieÃend den Wert 1.  Wollen wir auf das letzte Element einer
Liste \texttt{l} zugreifen, so kÃ¶nnen wir dafÃ¼r den Ausdruck
\\[0.2cm]
\hspace*{1.3cm}
\texttt{l[\#l]}
\\[0.2cm]
benutzen, denn \texttt{\#l} gibt die Anzahl der Elemente der Liste \texttt{l} an.  Alternativ kÃ¶nnen
...
\begin{Verbatim}[ frame         = lines, 
    ...
    b := [ 2, 3, 4, 5, 6 ];
    c := { 5, 6, 7 };
    // AneinanderhÃ¤ngen von Tupeln mit +
    print(a, " + ", b, " = ", a + b);
    // Berechnung der Anzahl der Elemente einer Menge
    print("# ", c, " = ", # c);
    // Berechnung der LÃ¤nge eines Tupels
    print("# ", a, " = ", # a);
    // Auswahl des dritten Elements von b
    print(b, "[3] = ", b[3] );
    // Ãberschreiben des 10. Elements von b
    b[10] := 42;
    print("b = ", b);
...
ZusÃ¤tzlich sehen wir in Zeile 19, dass \textsc{SetlX} simultane Zuweisungen unterstÃ¼tzt.
...
Die Sprache \textsc{SetlX} stellt alle Kontroll-Strukturen zur VerfÃ¼gung, die
 in modernen Sprachen Ã¼blich sind.  Wir haben  ``\texttt{if}''-Abfragen bereits mehrfach gesehen.
...
Hierbei steht $\textsl{test}_i$ fÃ¼r einen Test, der ``\texttt{true}'' oder
``\texttt{false}'' liefert.  Liefert der Test ``\texttt{true}'', so wird der zugehÃ¶rigen
Anweisungen in $\textsl{body}_i$ ausgefÃ¼hrt, andernfalls wird der nÃ¤chste
Test  $\textsl{test}_{i+1}$ versucht.  Schlagen alle Tests fehl, so wird
$\textsl{body}_{n+1}$ ausgefÃ¼hrt.
 
Die Tests selber kÃ¶nnen dabei die binÃ¤ren Operatoren 
\\[0.2cm]
\hspace*{1.3cm}
``\texttt{==}'',
``\texttt{!=}'',
...
``\texttt{in}'',
\\[0.2cm]
verwenden. Dabei  steht ``\texttt{==}'' fÃ¼r den Vergleich auf Gleichheit,
``\texttt{!=}'' fÃ¼r den Vergleich auf Ungleichheit.
FÃ¼r Zahlen fÃ¼hren die Operatoren ``\texttt{>}'',
``\texttt{<}'',
``\texttt{>=}'' und 
``\texttt{<=}''
dieselben GrÃ¶Ãenvergleiche durch wie in der Sprache \texttt{C}.  FÃ¼r Mengen
Ã¼berprÃ¼fen diese Operatoren analog, ob die entsprechenden Teilmengen-Beziehung
erfÃ¼llt ist.  Der Operator ``\texttt{in}'' Ã¼berprÃ¼ft, ob das erste Argument ein
...
definiert werden kÃ¶nnen, kÃ¶nnen mit Hilfe der Junktoren ``\texttt{\&\&}'' (logisches \emph{und}),
``\texttt{||}'' (logisches \emph{oder}) und ``\texttt{!}'' (logisches \emph{nicht})
komplexere Tests aufgebaut werden.  
Dabei bindet der Operator ``\texttt{||}'' am schwÃ¤chsten und der Operator ``\texttt{!}''
bindet am stÃ¤rksten.  Ein Ausdruck der Form
\\[0.2cm]
\hspace*{1.3cm}
\texttt{!a == b \&\& b < c || x >= y}
\\[0.2cm]
...
\texttt{((!(a == b)) \&\& b < c) || x >= y}
\\[0.2cm]
geklammert.  Damit haben die logischen Operatoren dieselbe PrÃ¤zedenz wie in der Sprache \texttt{C}.

\textsc{SetlX} unterstÃ¼tzt  die Verwendung von Quantoren.  Die Syntax fÃ¼r die Verwendung
des Allquantors ist
\\[0.2cm]
\hspace*{1.3cm}
\texttt{forall (} $x$ \texttt{in} $s$ \texttt{|} \textsl{cond} \texttt{)}
\\[0.2cm]
Hier ist $s$ eine Menge und \textsl{cond} ist ein Ausdruck, der von der Variablen $x$ abhÃ¤ngt und einen
Wahrheitswert als Ergebnis zurÃ¼ck liefert.  Die Auswertung des oben angegebenen Allquantors liefert genau
dann \texttt{true} wenn die Auswertung von \textsl{cond} fÃ¼r alle Elemente der Menge $s$ den Wert
...
\\[0.2cm]
trifft auf genau die Zahlen $p$ zu, fÃ¼r die gilt, dass alle Teiler Elemente der Menge $\{1,p\}$ sind.
...
\textbf{zusÃ¤tzlich} der Variablen $x$ ein Wert zugewiesen, fÃ¼r den die Bedingung \textsl{cond}
erfÃ¼llt ist.   Falls die Auswertung des Existenzquantors den Wert \texttt{false} ergibt,
Ã¤ndert sich der Wert von  $x$ nicht.


\subsubsection{\texttt{Switch}-BlÃ¶cke}
...
ausgewertet. FÃ¼r den ersten Test \textsl{test}$_i$, dessen
Auswertung den Wert \texttt{true} ergibt, wird der zugehÃ¶rige Block
\textsl{body}$_i$ ausgefÃ¼hrt.  Nur dann, wenn alle Tests 
\textsl{test}$_1$, $\cdots$, \textsl{test}$_n$
scheitern, wird der Block \textsl{body}$_{n+1}$ hinter dem SchlÃ¼sselwort
\texttt{default} ausgefÃ¼hrt.  Den selben Effekt kÃ¶nnte man natÃ¼rlich auch mit
...
bei dem es darum  geht, in AbhÃ¤ngigkeit von der letzten Ziffer einer Zahl eine Meldung auszugeben.  Bei
der Behandlung der Aussagenlogik werden wir noch realistischere Anwendungs-Beispiele fÃ¼r den
...
ausgefÃ¼hrt werden.  Dies ist in \textsc{SetlX} anders: Dort wird immer genau ein Block
ausgefÃ¼hrt.

Neben dem \texttt{switch}-Block gibt es noch dem \texttt{match}-Block, den wir allerdings
erst spÃ¤ter diskutieren werden.

\subsection{Schleifen}
Es gibt in \textsc{SetlX} Kopf-gesteuerte Schleifen (\texttt{while}-Schleifen)
und Schleifen, die Ã¼ber die Elemente einer Menge oder einer Liste  iterieren (\texttt{for}-Schleifen).
...
``\texttt{true}'', so wird anschlieÃend \textsl{body} ausgewertet.  Danach beginnt
die Auswertung der Schleife dann wieder von vorne, d.h.~es wird wieder
\textsl{test} ausgewertet und danach wird abhÃ¤ngig von dem Ergebnis dieser
...
Die von \textsc{SetlX} unterstÃ¼tzten \texttt{while}-Schleifen funktionieren genauso wie in
der Sprache \texttt{C}.
...
belegt und anschlieÃend wird mit dem jeweiligen Wert von $x$ der
Schleifenrumpf \textsl{body} ausgefÃ¼hrt.  Falls $s$ ein String ist, dann iteriert die Schleife Ã¼ber
...
$n$ berechnet werden, so wird zunÃ¤chst ein Tupel der LÃ¤nge $n$ gebildet, dessen
$i$-tes Element den Wert $i$ hat.  Das passiert in Zeile 2.  AnschlieÃend werden 
...
Dazu sind zwei Schleifen erforderlich: Die Ã¤uÃere \texttt{for}-Schleife iteriert
$i$ Ã¼ber alle Werte von 2 bis $n$.  Die innere \texttt{while}-Schleife iteriert dann fÃ¼r
gegebenes $i$ Ã¼ber alle Werte $i \cdot j$, fÃ¼r die das Produkt $i \cdot j \leq n$ ist.
SchlieÃlich werden in der letzten  Zeile alle die
Indizes $i$ ausgedruckt, fÃ¼r die $\mathtt{primes}[i]$  nicht auf 0 gesetzt worden ist,
...
\item Falls in der Ã¤uÃeren Schleife die Zahl $i$ keine Primzahl ist, so bringt es nichts mehr, die
      innere \texttt{while}-Schleife in den Zeilen 9 bis 11 zu durchlaufen, denn alle
      Indizes, fÃ¼r die dort \texttt{primes[i*j]} auf 0 gesetzt wird, sind schon bei dem
      vorherigen Durchlauf der Ã¤uÃeren Schleife, bei der \texttt{primes[i]}
      ...
      \texttt{primes[i] = 0} ist, zu Ã¼berspringen, haben wir den Befehl ``\texttt{continue}''
      benutzt.  Der Aufruf von ``\texttt{continue}'' bricht die Abarbeitung des
      Schleifen-Rumpfs fÃ¼r den aktuellen Wert von \texttt{i} ab, weist der Variablen
      \texttt{i} den nÃ¤chsten Wert aus \texttt{[1..n]} zu und fÃ¤hrt dann mit der
      Abarbeitung der Schleife in Zeile 4 fort.  Der Befehl ``\texttt{continue}'' verhÃ¤lt sich
...
lÃ¶sen.  Ein naives Verfahren, das hier zum Ziel fÃ¼hrt, basiert auf der
Beobachtung, dass die Folge $(x_n)_n$, die durch \\[0.2cm]
\hspace*{1.3cm} $x_0 := 0$ und $x_{n+1} := \mathtt{cos}(x_n)$ fÃ¼r alle $n \in \mathbb{N}$ \\[0.2cm]
definiert ist, gegen eine LÃ¶sung der obigen Gleichung konvergiert.  Damit fÃ¼hrt das in
...
\end{Verbatim} 
\vspace*{-0.3cm}
\caption{LÃ¶sung der Gleichung $x = cos(x)$ durch Iteration.}  \label{fig:solve.stlx}
\end{figure} %\$
...
Test kann aber am Anfang der Schleife noch gar nicht durchgefÃ¼hrt werden, weil
da die Variable \texttt{old\_x} noch gar keinen Wert hat.  Daher kÃ¶nnen wir diesen Test 
erst ausfÃ¼hren, wenn der nÃ¤chste Wert von \texttt{x} berechnet worden ist, denn erst dann 
haben wir zwei Werte, die wir vergleichen kÃ¶nnen.  Ist der Test erfolgreich, so brechen wir die
...
\begin{enumerate}
\item Das Argument \texttt{f} ist die Funktion, fÃ¼r die ein Fixpunkt berechnet werden soll.
...
lÃ¶sen.  In Zeile 12 lÃ¶sen wir die Gleichung 
\\[0.2cm]
\hspace*{1.3cm}
$x = \bruch{1}{1+x}$ 
\\[0.2cm]
die zu der quadratischen Gleichung $x^2 + x = 1$ Ã¤quivalent ist.  Beachten Sie, dass wir die
Funktion $x \mapsto \frac{1}{1+x}$ in \textsc{SetlX} durch den Ausdruck
\\[0.2cm]
\hspace*{1.3cm}
\texttt{x |-> 1.0/(1+x)}
\\[0.2cm]
definieren kÃ¶nnen.  Hier habe ich die FlieÃkomma-Zahl $1.0$ verwendet, da \textsc{SetlX} sonst mit
rationalen Zahlen rechnen wÃ¼rde, was wesentlich aufwendiger ist.



Ganz nebenbei zeigt das obige Beispiel auch, dass Sie in \textsc{SetlX} nicht
nur mit rationalen Zahlen, sondern auch mit reellen Zahlen rechnen kÃ¶nnen.  Eine Zahlen-Konstante,
die den Punkt ``\texttt{.}'' enthÃ¤lt, wird automatisch als reelle Zahl erkannt
...
\item Der Ausdruck $\texttt{sin}(x)$ berechnet den Sinus von $x$.
      AuÃerdem stehen die trigonometrischen Funktionen $\texttt{cos}(x)$ und 
      $\texttt{tan}(x)$ zur VerfÃ¼gung. Die Umkehr-Funktionen der trigonometrischen Funktionen sind
...
\item Der Ausdruck $\texttt{exp}(x)$ berechnet die Potenz zur Basis $e$, es gilt
      ...
      \hspace*{1.3cm}
      $\texttt{exp}(x) = e^x$.
\item Der Ausdruck $\texttt{log}(x)$ berechnet den natÃ¼rlichen Logarithmus von $x$.
       Der Logarithmus zur Basis 10 von $x$ wird durch $\mathtt{log10}(x)$ berechnet.
\item Der Ausdruck $\texttt{abs}(x)$ berechnet den Absolut-Betrag von $x$, wÃ¤hrend
...
\item Der Ausdruck $\texttt{ceil}(x)$ berechnet die kleinste ganze Zahl, die grÃ¶Ãer oder
...
\item Der Ausdruck $\texttt{floor}(x)$ berechnet die grÃ¶Ãte ganze Zahl, die kleiner oder
      gleich $x$ ist, es gilt
      \\[0.2cm]
      \hspace*{1.3cm}
...
\end{enumerate}
DarÃ¼ber hinaus unterstÃ¼tzt \textsc{SetlX} die Verwendung von rationalen Zahlen.  Die
Eingabe von
\\[0.2cm]
\hspace*{1.3cm}
\texttt{1/2 + 1/3;}
...
Zahlen keinen Ãberlauf geben.  Damit kÃ¶nnen wir die rationalen Zahlen benutzen, um mit beliebig
...
\\[0.2cm]
berechnet werden.  In \textsc{SetlX} kÃ¶nnen wir \texttt{e} mit Hilfe des Kommandos
\\[0.2cm]
\hspace*{1.3cm}
...
Der Interpreter bietet die MÃ¶glichkeit, komplexe Programme zu laden.  Der Befehl
\\[0.2cm]
\hspace*{1.3cm}
\texttt{load(\textsl{file});}
\\[0.2cm]
lÃ¤dt das Programm, dass sich in der Datei \textsl{file} befindet und fÃ¼hrt die in dem
Programm vorhandenen Befehle aus.  FÃ¼hren wir beispielsweise den Befehl
\\[0.2cm]
\hspace*{1.3cm}
\texttt{load(\symbol{34}primes-forall.stlx\symbol{34});}
\\[0.2cm]
im Interpreter aus und enthÃ¤lt die Datei ``\texttt{primes-forall.stlx}'' das in Abbildung
\ref{fig:primes-forall.stlx} auf Seite \pageref{fig:primes-forall.stlx} gezeigte Programm,
so kÃ¶nnen wir anschlieÃend mit den in dieser Datei definierten Variablen arbeiten.
...
aneinander zu hÃ¤ngen, der Ausdruck 
\\[0.2cm]
\hspace*{1.3cm}
...
\squote{abcuvw}.
\\[0.2cm]
ZusÃ¤tzlich kann eine natÃ¼rliche Zahl $n$ mit einem String $s$ Ã¼ber den
Multiplikations-Operator ``\texttt{*}'' verknÃ¼pft werden.  Der Ausdruck 
\\[0.2cm]
\hspace*{1.3cm}
\texttt{n * s;}
\\[0.2cm]
...
Wir wollen in diesem kurzen Abschnitt zeigen, wie sich Wahrscheinlichkeiten fÃ¼r die Poker-Variante
...
\\[0.2cm]
ist und auÃerdem einen Wert hat, der ein Element der Menge
\\[0.2cm]
\hspace*{1.3cm} $\textsl{values} = \{ 2, 3, 4, 5, 6, 7, 8, 9, 10, \textsl{Jack}, \textsl{Queen},
\textsl{King}, \textsl{Ace} \}$
\\[0.2cm]
ist.  Die einzelnen Karten kÃ¶nnen dann mathematisch als Paare dargestellt werden.  Die gesamte Menge von
Karten wÃ¤re dann beispielsweise wie folgt gegeben:
\\[0.2cm]
\hspace*{1.3cm} 
$\textsl{deck} = \{ \pair(v,s) \mid v \in \textsl{values} \wedge \textsl{s} \in \textsl{suits} \}$.
\\[0.2cm]
Jeder Spieler bekommt zunÃ¤chst zwei Karten.  Diese beiden Karten werden als \emph{Preflop}
oder \emph{Hole} bezeichnet.  Nach einer Bietphase werden anschlieÃend drei weitere
Karten, der sogenannte \textsl{Flop}, auf den Tisch gelegt.  Wir nehmen an, dass ein Spieler zunÃ¤chst die
Karten $\{ \pair(3, \club), \pair(3, \spade) \}$ bekommen hat und nun wissen mÃ¶chte, wie groÃ die
Wahrscheinlichkeit dafÃ¼r ist, dass beim Flop eine weitere 3 auf den Tisch gelegt wird.
...
gezeigte Programm fÃ¼hrt diese Berechnung durch.
...
\item In Zeile 1 definieren wir die Menge \texttt{suits} der Farben einer Karte.
      Dabei werden die folgenden AbkÃ¼rzungen verwendet:
      \begin{enumerate}
      \item ``\texttt{c}'' steht fÃ¼r $\club$, (Englisch: \emph{club}),
      \item ``\texttt{h}'' steht fÃ¼r $\heart$, (Englisch: \emph{heart}), 
      \item ``\texttt{d}'' steht fÃ¼r $\diamondsuit$, (Englisch: \emph{diamond}) und
      \item ``\texttt{s}'' steht fÃ¼r $\spade$, (Englisch: \emph{spade}). 
      \end{enumerate} 
\item In Zeile 2 definieren wir die Menge \texttt{values} der mÃ¶glichen Werte einer
      Karte.  Dabei wurden die folgenden AbkÃ¼rzungen verwendet:
      \begin{enumerate}
      \item ``\texttt{T}'' steht fÃ¼r ``\emph{Ten}'',
      \item ``\texttt{J}'' steht fÃ¼r ``\emph{Jack}'',
      \item ``\texttt{Q}'' steht fÃ¼r ``\emph{Queen}'',
      \item ``\texttt{K}'' steht fÃ¼r ``\emph{King}'' und
      \item ``\texttt{A}'' steht fÃ¼r ``\emph{Ace}''.
      \end{enumerate}
...
\item In Zeile 6 berechnen wir die Menge aller mÃ¶glichen Flops.  Da die Reihenfolge der Karten im
      Flop keine Rolle spielt, verwenden wir zur Darstellung der einzelnen Flops eine Menge.
      Dabei mÃ¼ssen wir aber darauf achten, dass der Flop auch wirklich aus drei
      \underline{verschiedenen}  Karten besteht.  Dazu mÃ¼ssen wir fordern, dass die Ungleichungen
      \\[0.2cm]
      \hspace*{1.3cm}
      $\mathtt{k1} \not= \mathtt{k2}$, \quad 
      ...
      $\mathtt{k2} \not= \mathtt{k3}$
      \\[0.2cm]
      erfÃ¼llt sind.  Dies ist aber genau dann der Fall, wenn die Menge
...
\item Die Teilmenge der Flops, in denen mindestens eine weitere 3 auftritt, wird in Zeile 8
      ...
      beim Spieler.
\item SchlieÃlich berechnet sich die Wahrscheinlichkeit fÃ¼r eine 3 im Flop als das VerhÃ¤ltnis der 
      Anzahl der gÃ¼nstigen FÃ¤lle zu der Anzahl der mÃ¶glichen FÃ¤lle.  Wir mÃ¼ssen also nur die Anzahl
      der Elemente der entsprechenden Mengen ins VerhÃ¤ltnis setzen.  

      Hier gibt es aber noch eine Klippe, die umschifft werden muss: Da die Auswertung der AusdrÃ¼cke
      ...
      Daher ist es erforderlich, zunÃ¤chst eine der beiden Zahlen in eine FlieÃkomma-Zahl zu konvertieren,
      damit der Quotienten als FlieÃkomma-Zahl berechnet wird.  Dies kÃ¶nnen wir beispielsweise dadurch
      erreichen, dass wir den Dividenden \texttt{\#trips} mit der FlieÃkomma-Zahl $1.0$
      multiplizieren.
\end{enumerate}
...
Weise nur dann mÃ¶glich, wenn die im Laufe der Berechnung auftretenden Mengen so klein
sind, dass sie sich explizit im Rechner darstellen lassen.  Wenn diese Voraussetzung nicht
mehr erfÃ¼llt ist, kÃ¶nnen die gesuchten Wahrscheinlichkeiten mit Hilfe der im zweiten Semester
...
\textsc{SetlX} abschlieÃen.  Dazu betrachten wir das Problem, Pfade in
...
Zur Vereinfachung wollen wir zunÃ¤chst annehmen, dass die einzelnen Punkte durch Zahlen
identifiziert werden.  Dann kÃ¶nnen wir eine direkte Verbindung zwischen zwei Punkten durch
ein Paar von Zahlen
...
\emph{Einbahn-StraÃen} sind:  Wir haben zwar eine Verbindung von \texttt{1} nach
...
zwischen zwei Punkten verzeichnet.  Es gibt aber unter UmstÃ¤nden auch noch
andere Verbindungen.  Beispielsweise gibt es eine unmittelbare Verbindung von
\texttt{1} nach \texttt{3}.  Es gibt darÃ¼ber hinaus noch einen
Pfad von \texttt{1} nach \texttt{3}, der Ã¼ber den Punkt \texttt{2} geht.  
Unser Ziel in diesem Abschnitt ist es einen Algorithmus zu entwickeln, der Ã¼berprÃ¼ft, ob
zwischen zwei Punkten eine Verbindung existiert und gegebenenfalls berechnet.
Dazu entwickeln wir zunÃ¤chst einen Algorithmus, der nur Ã¼berprÃ¼ft, ob es eine
Verbindung zwischen zwei Punkten gibt und erweitern diesen Algorithmus dann spÃ¤ter so, dass diese
Verbindung auch berechnet wird.

\subsection{Berechnung des transitiven Abschlusses einer Relation}
Als erstes bemerken wir, dass ein Graph $R$ nichts anderes ist als eine binÃ¤re Relation.
Um feststellen zu kÃ¶nnen, ob es zwischen zwei Punkten eine Verbindung gibt,
mÃ¼ssen wir den transitiven Abschluss $R^+$ der Relation $R$ bilden.  Wir haben bereits
...
lange rechnen mÃ¼ssten.  Aber versuchen wir einmal, diese Formel anschaulich zu
verstehen.  ZunÃ¤chst steht da $R$.  Das sind die Verbindungen, die unmittelbar  gegeben
sind.  Als nÃ¤chstes steht dort $R^2$ und das ist $R \circ R$.  Es gilt aber \\[0.2cm]
...
zusammengesetzt sind.  Allgemein lÃ¤sst sich durch Induktion sehen, dass $R^n$
alle die Pfade enthÃ¤lt, die aus $n$ direkten Verbindungen zusammengesetzt sind.  Nun
...
\\[0.2cm]
Diese Formel kÃ¶nnten wir tatsÃ¤chlich so benutzen.  Es ist aber noch effizienter,
einen Fixpunkt-Algorithmus zu verwenden.  Dazu zeigen wir zunÃ¤chst, dass der transitive
Abschluss $R^+$ die folgende Fixpunkt-Gleichung erfÃ¼llt:
\begin{equation}
  \label{fixpunkt}
  R^+ = R \cup R \circ R^+. 
\end{equation}
Wir erinnern hier daran, dass wir vereinbart haben, dass der Operator $\circ$ stÃ¤rker
bindet als der Operator $\cup$, so dass der Ausdruck $R \cup R \circ R^+$ als
$R \cup (R \circ R^+)$ zu lesen ist.
Die Fixpunkt-Gleichung \ref{fixpunkt} lÃ¤sst sich algebraisch beweisen.  Es gilt
\[
\begin{array}{cll}
...
\end{enumerate}
Die Relationen $T_n$ lassen sich auf die Relation $R$ zurÃ¼ckfÃ¼hren:
\begin{enumerate}
\item $T_1 = R$.
...
\item $\begin{array}[t]{lcl}
      ...
      $
\end{enumerate}
Allgemein kÃ¶nnen wir durch vollstÃ¤ndige Induktion Ã¼ber $n \in \mathbb{N}$ beweisen, dass
\[ T_n = \bigcup\limits_{i=1}^{n} R^i \]
gilt.  Der Induktions-Anfang folgt unmittelbar aus der Definition von $T_1$.  Um den 
Induktions-Schritt durchzufÃ¼hren, betrachten wir
\[ \begin{array}{lcll}
   ...
   \end{array}
\]
Die Folge $(T_n)_{n\in\mathbb{N}}$ hat eine weitere nÃ¼tzliche Eigenschaft: Sie ist 
...
\\[0.2cm]
gilt, wenn also die Mengen $X_n$ mit wachsendem Index $n$ immer grÃ¶Ãer werden.
...
$
\\[0.2cm]
und die letzte Formel ist offenbar wahr.  Ist nun die Relation $R$ endlich, so ist natÃ¼rlich 
auch $R^+$ eine endliche Menge.  Da die 
Folge $T_n$ aber in dieser Menge liegt, denn es gilt ja 
\\[0.2cm]
\hspace*{1.3cm}
$T_n = \bigcup\limits_{i=1}^{n} R^i \subseteq \bigcup\limits_{i=1}^{\infty} R^i = R^+$ \quad fÃ¼r alle $n \in \mathbb{N}$,
\\[0.2cm]
kÃ¶nnen die Mengen $T_n$ nicht beliebig groÃ werden.  Aufgrund der Monotonie der Folge
...
\\[0.2cm]
BerÃ¼cksichtigen wir die Gleichung $T_n = \bigcup_{i=1}^{n} R^i$, so haben wir 
\\[0.2cm]
\hspace*{1.3cm}
$T_n = \bigcup\limits_{i=1}^{n} R^i = \bigcup\limits_{i=1}^{k} R^i = T_k$ \quad fÃ¼r alle $n \geq k$.
\\[0.2cm]
Daraus folgt dann aber, dass
\\[0.2cm]
\hspace*{1.3cm}
$T_n = \bigcup\limits_{i=1}^{n} R^i = \bigcup\limits_{i=1}^{\infty} R^i = R^+$ 
\quad fÃ¼r alle $n \geq k$  
\\[0.2cm]
gilt.  Der Algorithmus zur Berechnung von $R^+$ sieht nun so aus, dass wir die Iteration
\[ T_{n+1} := R \cup R \circ T_n \]
solange durchfÃ¼hren bis $T_{n+1} = T_n$ gilt, denn dann gilt auch $T_n = R^+$.
...
\end{verbatim}
Der transitive Abschluss $R^+$ der Relation $R$ lÃ¤sst sich jetzt anschaulich
interpretieren:  Er enthÃ¤lt alle Paare $\pair(x,y)$, fÃ¼r die es einen \emph{Pfad} von
$x$ nach $y$ gibt.  Ein Pfad von $x$ nach $y$ ist dabei eine Liste der
Form \\[0.2cm]
\hspace*{1.3cm} $\bigl[ x_1, x_2, \cdots, x_n \bigr]$,
\\[0.2cm]
fÃ¼r die $x = x_1$ und $y = x_n$ gilt und fÃ¼r die auÃerdem 
\\[0.2cm]
\hspace*{1.3cm}
$\pair(x_i, x_{i+1}) \in R$ \quad fÃ¼r alle $i = 1, \cdots, n-1$ gilt.
\\[0.2cm]
...
Form der Mengen-Defi\-nition durch Iteratoren in \textsc{SetlX}.  Allgemein kÃ¶nnen wir eine Menge
durch den Ausdruck
\\[0.2cm]
\hspace*{1.3cm}
...
$
\\[0.2cm]
definieren.  Dabei muss $s_i$ fÃ¼r alle $i=1, \cdots, k$ eine Menge von Listen  der LÃ¤nge
$n(i)$ sein.  Bei der Auswertung dieses Ausdrucks werden fÃ¼r die Variablen 
$x^{(i)}_1, \cdots, x^{(i)}_{n(i)}$ die Werte eingesetzt, die die entsprechenden
Komponenten der Listen haben, die in der Menge $s_i$ auftreten.  Beispielsweise wÃ¼rde die
Auswertung von 
\begin{verbatim}
    s1 := { [ 1, 2, 3 ], [ 5, 6, 7 ] };
    s2 := { [ "a", "b" ], [ "c", "d" ] };
...
Als nÃ¤chstes wollen wir das Programm zur Berechnung des transitiven Abschlusses so
erweitern, dass wir nicht nur feststellen kÃ¶nnen, dass es einen Pfad zwischen zwei Punkten
gibt, sondern dass wir diesen auch berechnen kÃ¶nnen.  Die Idee ist, dass wir statt des
relationalen Produkts, das fÃ¼r zwei Relationen definiert ist, ein sogenanntes
\emph{Pfad-Produkt}, das auf Mengen von Pfaden definiert ist, berechnen.  Vorab fÃ¼hren wir
fÃ¼r Pfade, die wir ja durch Listen reprÃ¤sentieren,
drei Begriffe ein.
\begin{enumerate}
...
Damit kÃ¶nnen wir das Programm in Abbildung
\ref{fig:transitive-closure.stlx} so abÃ¤ndern, dass alle mÃ¶glichen Verbindungen zwischen zwei
...
Leider funktioniert das Programm dann nicht mehr, wenn der Graph Zyklen enthÃ¤lt.
Abbildung
\ref{fig:graph-zykl} zeigt einen Graphen, der einen Zyklus enthÃ¤lt.  In diesem Graphen
gibt es unendlich viele Pfade, die von dem Punkt 1 zu dem Punkt 2 fÃ¼hren: \\[0.2cm]
...
Abbildung \ref{fig:path-cyclic.stlx} zeigt einen Ausschnitt des geÃ¤nderten Programms
\href{https://github.com/karlstroetmann/Logik/blob/master/SetlX/path-cyclic.stlx}{\texttt{path-cyclic.stlx}},
das auch fÃ¼r zyklische Graphen funktioniert. 
\begin{enumerate}
\item In Zeile 2 berÃ¼cksichtigen wir nur die Pfade $x \oplus y$, die nicht zyklisch sind.
\item In Zeile 5 Ã¼berprÃ¼fen wir, ob die Konkatenation  $l_1 \oplus l_2$ zyklisch ist.  Die
      ...
      denn wir verknÃ¼pfen diese beiden Listen ja nur dann, wenn das letzte Element
      der Liste $l_1$ mit dem ersten Element der Liste $l_2$ Ã¼bereinstimmt.
      Wenn es nun noch einen weiteren Punkt geben wÃ¼rde, der sowohl in $l_1$ als auch in
      $l_2$ auftreten wÃ¼rde, dann wÃ¤re der Pfad $l_1 \oplus l_2$ zyklisch.
\end{enumerate}

In den meisten FÃ¤llen sind wird gar nicht daran interessiert, alle mÃ¶glichen Verbindungen
zwischen allen Punkten zu berechnen, das wÃ¤re nÃ¤mlich viel zu aufwendig, sondern wir
...
Prozedur $\texttt{reachable}(x, y, r)$, die Ã¼berprÃ¼ft, ob es in dem Graphen $r$ eine
Verbindung von $x$ nach $y$ gibt und die diese Verbindung berechnet.  Das vollstÃ¤ndige
...
\begin{enumerate}
\item In Zeile 2 initialisieren wir $p$ so, dass zunÃ¤chst nur der Pfad der LÃ¤nge 0,
      der mit dem Punkt $x$  startet, in $p$ liegt.
\item In Zeile 6 selektieren wir die Pfade aus $p$, die zum Ziel $y$ fÃ¼hren.
\item Wenn wir dann in Zeile 7 feststellen, dass wir einen solchen Pfad berechnet haben,
      geben wir einen dieser Pfade in Zeile 8 zurÃ¼ck.
\item Falls es nicht gelingt einen solchen Pfad zu berechnen und wir keine neuen
      Pfade mehr finden kÃ¶nnen, verlassen wir die Prozedur in Zeile 11
      mit dem Befehl \texttt{return}.  Da wir bei diesem \texttt{return}-Befehl
      keinen Wert zurÃ¼ckgeben, ist der RÃ¼ckgabewert der Prozedur in diesem Fall
      automatisch $\Omega$.
\end{enumerate}
...
Wir prÃ¤sentieren nun eine betriebswirtschaftliche Anwendung des oben entwickelten Algorithmus und
betrachten folgendes Problem.
\vspace*{0.3cm}

\begin{minipage}[c]{14cm}
{\sl
Ein AgrarÃ¶konom will mit einem Wolf, einer Ziege und einem Kohl Ã¼ber einen Fluss Ã¼bersetzen, um
...
Wenn der Bauer den Wolf mit der Ziege allein lÃ¤sst, dann frisst der Wolf die Ziege und wenn er die
Ziege mit dem Kohl allein lÃ¤sst, dann frisst die Ziege den Kohl. }
\end{minipage}
\vspace*{0.3cm}

\noindent
Wir wollen einen Fahrplan entwickeln, mit dem der AgrarÃ¶konom alle seine Waren unbeschadet zum
Markt bringen kann.  Dazu modellieren wir das RÃ¤tsel als Erreichbarkeits-Problem in einem
Graphen.  
Die Punkte des Graphen beschreiben dabei die einzelnen Situationen, die auftreten
kÃ¶nnen.  Wir definieren eine Menge\\[0.2cm]
\hspace*{1.3cm} 
...
Ufer ist.  Die Menge aller Punkte kÃ¶nnen wir dann definieren als \\[0.2cm]
\hspace*{1.3cm} 
...
Als nÃ¤chstes definieren wir den Graphen $r$, also die mÃ¶glichen Verbindungen zwischen
...
\end{verbatim}
Damit kÃ¶nnen wir eine Relation $r_1$ wie folgt definieren:
\\[0.2cm]
\hspace*{1.3cm} 
...
fÃ¤hrt und bei denen zusÃ¤tzlich sichergestellt ist, dass am linken Ufer
nach der Ãberfahrt kein Problem auftritt.  Die einzelnen Terme werden wie folgt
interpretiert:
\begin{enumerate}
\item $\pair(s_1,s_2)$ ist der Zustand vor der Ãberfahrt des Bootes,
...
\item $b$ ist der Inhalt des Bootes, daher beschreibt 
       $\pair(s_1 \backslash b, s_2 \cup b)$ den Zustand nach der Ãberfahrt des Bootes:
       Links sind nun nur noch die Objekte aus $s_1 \backslash b$, dafÃ¼r sind rechts dann 
       die Objekte $s_2 \cup b$.

       Die Bedingungen lassen sich wie folgt interpretieren.
\item $b \subseteq s_1$: Es kÃ¶nnen natÃ¼rlich nur solche Objekte ins Boot genommen werden,
       die vorher am linken Ufer waren.
\item $\squote{Bauer} \in b$: Der Bauer muss auf jeden Fall ins Boot, denn weder der Wolf
       noch die Ziege kÃ¶nnen rudern.
\item $\textsl{card}(b) \leq 2$: Die Menge der Objekte im Boot darf nicht mehr als zwei
       Elemente haben, denn im Boot ist nur fÃ¼r zwei Platz.
\item $\neg\textsl{problem}(s_1 \backslash b)$:  Am linken Ufer soll es nach der Ãberfahrt
       kein Problem geben, denn der Bauer ist ja hinterher am rechten Ufer.
\end{enumerate}
In analoger Weise definieren wir nun eine Relation $r_2$ die die Ãberfahrten vom rechten
...
Als nÃ¤chstes mÃ¼ssen wir den Start-Zustand modellieren.
...
$\bigl\langle \{\}, \{ \squote{Bauer}, \squote{Wolf}, \squote{Ziege},\squote{Kohl} \} \bigr\rangle$.
\\[0.3cm]
Damit haben wir das Problem in der Mengenlehre modelliert und kÃ¶nnen die im letzten
Abschnitt entwickelte Prozedur \texttt{reachable} benutzen, um das Problem zu lÃ¶sen.
...
in dem die oben ausgefÃ¼hrten Ãberlegungen in \textsc{SetlX} umgesetzt wurden.
Die von diesem Programm berechnete LÃ¶sung finden Sie in Abbildung \ref{fig:wolf-ziege-solution}.
...
\end{Verbatim} 
\vspace*{-0.3cm}
\caption{Ein Fahrplan fÃ¼r den Bauern}  
...
Datenstruktur, die sogenannten \emph{Terme}, die insbesondere nÃ¼tzlich ist, wenn wir
...
Im nÃ¤chsten Unterabschnitt werden wir zunÃ¤chst \emph{Terme} zusammen mit den in \setl\
vordefinierten Funktionen vorstellen, die zur Verarbeitung von Termen benutzt werden kÃ¶nnen.
AnschlieÃend stellen wir das sogenannte \emph{Matching} vor, mit dessen Hilfe sich Terme
...
beginnen Funktionen-Zeichen im Gegensatz zu einem Variablen-Namen daher mit einem groÃen
Buchstaben.  Auf den GroÃbuchstaben kÃ¶nnen  dann beliebig viele Buchstaben, Ziffern und
der Unterstrich ``\texttt{\_}'' folgen.
ZusÃ¤tzlich gibt es noch Funktionszeichen, die mit dem Zeichen
``\texttt{\symbol{94}}''
beginnen.  Solche Funktions-Zeichen werden intern von \setl\ verwendet um Operator-Symbole
wie ``\texttt{+}'' oder ``\texttt{*}'' darzustellen.
Die folgenden Strings kÃ¶nnen beispielsweise  als Funktions-Zeichen vewendet werden:
\\[0.2cm]
\hspace*{1.3cm}
...
$F(t_1, t_2, \cdots, t_n)$
\\[0.2cm]
ein Term.  Beachten Sie, dass Terme ganz Ã¤hnlich aussehen wie die Aufrufe von Funktionen.
Terme und Aufrufe von Funktionen unterscheiden sich nur dadurch, dass bei einem Term links
vor der ersten Ã¶ffnenden Klammer ein Funktions-Zeichen steht, wÃ¤hrend bei einem
Funktions-Aufruf dort statt dessen eine Variable steht, der eine Funktions-Definition zugewiesen
worden ist.

\examples
\begin{enumerate}
\item \texttt{Adresse(\symbol{34}RotebÃ¼hlplatz 41\symbol{34}, 70178, \symbol{34}Stuttgart\symbol{34})}

      ist ein Term, der eine Adresse reprÃ¤sentiert.
\item \texttt{Product(Variable(\symbol{34}x\symbol{34}), Sin(Variable(\symbol{34}x\symbol{34})))}

      ist ein Term, der einen arithmetischen Ausdruck reprÃ¤sentiert, den Sie mathematisch
      als $x \cdot \sin(x)$ schreiben wÃ¼rden.  \eox
\end{enumerate}
...
Genausowenig wie Sie die Zahl \texttt{42} auswerten mÃ¼ssen, mÃ¼ssen Sie einen Term auswerten.

Nehmen wir einmal an,
dass es in \setl\ keine Listen geben wÃ¼rde.  Dann kÃ¶nnten wir Listen als Terme darstellen.  ZunÃ¤chst
wÃ¼rden wir ein Funktions-Zeichen benÃ¶tigen, mit dem wir die leere Liste darstellen kÃ¶nnten.  Wir
wÃ¤hlen dazu das Funktions-Zeichen \texttt{Nil}.  Damit haben wir dann also die Entsprechung
\\[0.2cm]
\hspace*{1.3cm}
$\texttt{Nil}() \;\widehat{=}\; \texttt{[]}$.
\\[0.2cm]
\textbf{Beachten} Sie hier, dass die Klammern hinter dem Funktions-Zeichen \texttt{Nil} nicht
weggelassen werden dÃ¼rfen!  Um nun eine Liste darzustellen, deren erstes Element $x$ ist und deren
...
\\[0.2cm]
Konkret kÃ¶nnen wir nun die Liste \texttt{[1,2,3]} durch den Term
\\[0.2cm]
\hspace*{1.3cm}
\texttt{Cons(1, Cons(2, Cons(3, Nil())))}
\\[0.2cm]
darstellen.  In der Sprache \textsl{Prolog}, werden Listen intern in Ã¤hnlicher Form als Terme dargestellt.

Es gibt zwei vordefinierte Funktionen in \setl, mit denen wir auf die Komponenten eines Terms
zugreifen kÃ¶nnen und es gibt eine weitere Funktion, mit deren Hilfe wir Terme konstruieren kÃ¶nnen.
\begin{enumerate}
...
\item Ist ein Funktions-Zeichen $f$ und eine Liste $l$ von Argumenten gegeben, so erzeugt die
      ...
      $\texttt{makeTerm}(f,l)$
      \\[0.2cm]
      einen Term $t$ mit dem Funktions-Zeichen $f$ und der Argument-Liste $l$, fÃ¼r $t$ gilt also
      \\[0.2cm]
      \hspace*{1.3cm}
      ...
      \texttt{Cons(1,Nil())}.
      \\[0.2cm]
      Diesen Term hÃ¤tten wir natÃ¼rlich auch unmittelbar hinschreiben kÃ¶nnen.
\end{enumerate}
...
\end{Verbatim}
\vspace*{-0.3cm}
\caption{EinfÃ¼gen eines Elements am Ende einer Liste.}
...
deren Aufgabe es ist, ein Element $x$ am Ende einer Liste $l$ einzufÃ¼gen, wobei vorausgesetzt ist,
dass die Liste $l$ als Term mit Hilfe der Funktions-Zeichen ``\texttt{Cons}'' und ``\texttt{Nil}''
dargestellt wird.
\begin{enumerate}
\item ZunÃ¤chst wird in Zeile 2 Ã¼berprÃ¼ft, ob die Liste $l$ leer ist.  Die Liste $l$ ist genau dann
      leer, wenn $l = \texttt{Nil()}$ gilt.  Daher kÃ¶nnen wir einfach das Funktions-Zeichen von dem
      Term $l$ testen um herauszufinden, ob $l$ die leere Liste reprÃ¤sentiert.
\item Falls $l$ nicht leer ist, muss $l$ die Form
      ...
      die Liste der restlichen Elemente.  In diesem Fall mÃ¼ssen wir $x$ rekursiv in die Liste
      \textsl{tail} einfÃ¼gen.  Als Ergebnis wird in Zeile 6 dann eine neue Liste erzeugt, deren
      erstes Element \textsl{head} ist, wÃ¤hrend die Liste der restlichen Elemente durch den
      rekursiven Aufruf von \textsl{append} berechnet wird.
\end{enumerate}
In manchen FÃ¤llen ist es sehr unbequem, dass Funktions-Zeichen immer mit einem groÃen Buchstaben
beginnen mÃ¼ssen.  Deswegen gibt es in \setl\ einen Escape-Mechanismus, der es erlaubt, auch
...
algebraische AusdrÃ¼cke darzustellen, die trigonometrische Funktionen enthalten, 
so kÃ¶nnen wir einen Ausdruck der Form $\sin(x)$ durch den Term
\\[0.2cm]
\hspace*{1.3cm}
...
\subsection{Matching}
Der Umgang mit Termen wÃ¤re sehr mÃ¼hsam, wenn wir die Terme jedesmal mit Hilfe der Funktionen
\texttt{fct} und \texttt{args} auseinander nehmen mÃ¼ssten.  Abbildung \ref{fig:append-match.stlx} 
...
and ``\texttt{args}'' verwendet haben.  In Zeile 3 wird  Ã¼berprÃ¼ft, ob die Liste $l$ leer ist.
Die wahre StÃ¤rke des Matchings sehen wir allerdings ist in Zeile 4, denn dort wird nicht nur
Ã¼berprÃ¼ft, ob die Liste $l$ die Form
\\[0.2cm]
\hspace*{1.3cm}
\texttt{Cons(\textsl{head},\textsl{tail})}
\\[0.2cm]
hat, sondern gleichzeitig werden die Variablen \textsl{head} and \textsl{tail} so gesetzt, dass
anschlieÃend die Gleichung
\\[0.2cm]
\hspace*{1.3cm}
$l = \texttt{Cons(\textsl{head},\textsl{tail})}$
\\[0.2cm]
...
Im Allgemeinen ist  ein \texttt{match}-Block so Ã¤hnlich aufgebaut wie ein
...
Die AusdrÃ¼cke $t_1$, $\cdots$, $t_n$ sind sogenannte \emph{Muster}, die freie
...
mit $\textsl{body}_i$ bezeichnete Gruppe von Befehlen ausgefÃ¼hrt.  Andernfalls
versucht \textsc{SetlX} das nÃ¤chste Muster $t_{i+1}$ mit $e$ zur Deckung zu bringen.
Falls keines der Muster $t_1$, $\cdots$, $t_n$ mit $e$ zur Deckung zu bringen ist, wird
ersatzweise $\textsl{body}_{n+1}$ ausgefÃ¼hrt.  
...
\begin{enumerate}
\item Das erste  Argument $t$ ist ein Term, der einen arithmetischen Ausdruck reprÃ¤sentiert.
...
$x \mapsto x^x$
\\[0.2cm]
nach $x$ ableiten, so kÃ¶nnen wir die Funktion \texttt{diff} wie folgt aufrufen.
\\[0.2cm]
\hspace*{1.3cm}
...
zunÃ¤chst den \texttt{match}-Befehl in  Abbildung \ref{fig:diff.stlx}.  
In Zeile  3 hat der zu differenzierende Ausdruck die Form
\texttt{t1 + t2}.  Um einen solchen Ausdruck nach einer Variablen $x$ zu differenzieren, mÃ¼ssen wir
...
In den Zeilen  12 und 16 haben wir den Funktions-Zeichen ``\texttt{exp}'' und ``\texttt{ln}'' den
 Operator ``\texttt{\symbol{64}}'' vorangestellen mÃ¼ssen, denn sonst wÃ¼rden die Strings
...
Um einen Ausdruck der Form $\ln(f)$ abzuleiten, mÃ¼ssen
...
Um das Beispiel in Abbildung \ref{fig:diff.stlx} besser zu verstehen mÃ¼ssen wir wissen, 
...
\\[0.2cm]
umgewandelt.  Dies erklÃ¤rt die Zeilen 19 und 21 von Abbildung \ref{fig:diff.stlx}.

Wir kÃ¶nnen die interne Darstellung eines Terms mit Hilfe der Funktion
...
\texttt{case n:}
\\[0.2cm]
passt zunÃ¤chst auf jeden Term.  Allerdings wollen wir in Zeile 21 nur Zahlen matchen.  Daher haben
wir an dieses Muster mit Hilfe des Operators ``\texttt{|}'' noch die Bedingung \texttt{isNumber(n)}
angehÃ¤ngt, mit der wir sicherstellen, dass $n$ tatsÃ¤chlich eine Zahl ist.


\subsection{Ausblick}
Wir konnten in diesem einfÃ¼hrenden Kapitel nur einen Teil der Sprache \textsc{SetlX}
behandeln.  Einige weitere Features
der Sprache \textsc{SetlX} werden wir noch in den folgenden Kapiteln diskutieren.
ZusÃ¤tzlich finden Sie
