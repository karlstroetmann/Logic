...
solche Formeln, die man nicht in kleinere Formeln zerlegen kann, atomare Formeln enthalten also weder Junktoren noch Quantoren.
\begin{Definition}[Atomare Formeln]
{\em
...
Setzen wir das obige Beispiel fort, so können wir sehen, dass \\[0.2cm]
...
\hspace*{1.3cm}  $\int\limits_{0}^{2} y \cdot  t\, d t = \frac{1}{2} 2^2 \cdot  y$ \\[0.2cm]
...
\hspace*{1.3cm} $\int\limits_{0}^{x} y \cdot  u\, d u = \frac{1}{2} x^2 \cdot  y$ \\[0.2cm]
und das ist die selbe Aussage wie oben.  Das funktioniert allerdings nicht mit jeder Variablen. Setzen wir
...
wir \\[0.2cm]
\hspace*{1.3cm}  $\int\limits_{0}^{x} x^2 \cdot  t\, d t = \frac{1}{2} x^2 \cdot  x^2$ \\[0.2cm]
...
\begin{Definition}[$\Sigma$-Formel, gebundene und freie Variablen] \hspace*{\fill} \\
{\em \label{praedikaten-formel}
    ...
    \item Sind $F, G \in \mathbb{F}_\Sigma$ und gilt außerdem \\[0.2cm]
          \hspace*{1.3cm} $\FV(F) \cap \BV(G) = \{\}$ \quad und \quad
                          $\FV(G) \cap \BV(F) = \{\}$, \\[0.2cm]
          so gilt auch
          \begin{enumerate}
...
der Formeln läßt sich zeigen, dass für alle $F \in \mathbb{F}_\Sigma$ folgendes gilt:
\[ \FV(F) \cap \BV(F) = \{\}. \]


\noindent
\textbf{Beispiel}: Setzen wir das oben begonnene Beispiel fort, so  sehen wir, dass \\[0.2cm]
...
Wenn wir Formeln immer in dieser Form anschreiben würden, dann würde die Lesbarkeit unverhältnismäßig leiden.
Zur Abkürzung vereinbaren wir, dass in der Prädikatenlogik die selben Regeln zur Klammer-Ersparnis
gelten sollen,  die wir schon in der Aussagenlogik verwendet haben.  Zusätzlich werden
gleiche Quantoren zusammengefaßt: Beispielsweise schreiben wir  
...
$\forall x\el M: F$ oder $\exists x\el M: F$.  Hierbei handelt es sich um Abkürzungen, die
...
Als nächstes legen wir die Bedeutung der Formeln fest.  Dazu definieren wir mit Hilfe der
Mengenlehre den Begriff einer \emph{$\Sigma$-Struktur}.  Eine solche Struktur legt fest, wie die
...
\noindent
\textbf{Beispiel}: Wir geben ein Beispiel für eine $\Sigma_{\mathrm{arith}}$-Struktur
...
\item Die Abbildung $\mathcal{J}_{\mathrm{arith}}$ legen wir dadurch fest, dass die
      ...
      und die Kleiner-Gleich-Relation interpretiert werden.
\end{enumerate}

\noindent
\textbf{Beispiel}: Wir geben ein weiteres  Beispiel.
...
\noindent
\textbf{Beispiel}: Mit der oben definieren $\Sigma_\mathrm{arith}$-Struktur
...
\noindent
\textbf{Beispiel}:  In Fortführung des obigen Beispiels gilt: \\[0.2cm]
...
\textsc{SetlX} implementieren läßt.  Dadurch gelingt es, diesen Begriff zu veranschaulichen.  Als konkretes
...
\item Die Interpretation $j$ wird dann in Zeile 7 mit dem
      Universum $u$ zu der Struktur $s$ zusammen gefasst.  
...
$\texttt{evalFormula}(f, \mathcal{S}, \mathcal{I})$ benutzen läßt um zu überprüfen, ob die
...
\begin{Satz}
{\em Es gelten die folgenden Äquivalenzen:
  \begin{enumerate}
...
\item Falls $t$ eine Variable ist, gibt es zwei Fälle:
  ...
  \end{enumerate}
\item Andernfalls muß $t$ die Form $t= f(s_1,\cdots,s_m)$ haben. Dann können wir $t\sigma$ durch \\[0.2cm]
...
\noindent
\textbf{Beispiel}:  Wir führen das obige Beispiel fort und setzen \\[0.2cm]
...
$s \doteq t$, wobei einer der beiden folgenden Fälle vorliegen muß:
\begin{enumerate}
...
\noindent
\textbf{Beispiel}: Wir verdeutlichen die bisher eingeführten Begriffe anhand eines
Beispiels.  
...
\hspace*{1.3cm} $p(x_2, x_3)\sigma \;\quad = p(x_2, f(x_4))$. 
\vspace{0.3cm}
...
$\langle E, []\rangle$  , so läßt sich immer eine der
...
\noindent
\textbf{Beispiel}: Wir wenden das oben dargestellte Verfahren an, um die syntaktische Gleichung \\[0.2cm]
\hspace*{1.3cm}  $p(x_1, f(x_4)) \doteq p( x_2, x_3)$  \\[0.2cm]
...
des anfangs gegebenen syntaktischen Gleichungs-Systems gefunden.  \hspace*{\fill} $\Box$
...
besteht aus zwei Schluß-Regeln, die wir jetzt definieren.
...
\[ p(x) \doteq p(f(x)) \]
ist unlösbar.  Das liegt daran, dass \textbf{zufällig} in beiden Klauseln die selbe Variable
...
{\em
  ...
  den \emph{Allabschluß}  $\forall(k)$  der Klausel k als \\[0.2cm]
  \hspace*{1.3cm} $\forall(k) := \forall x_1, \cdots,x_n \colon k$.
}
\end{Definition}

\noindent
Die für uns wesentlichen Eigenschaften des Beweis-Begriffs $M \vdash k$ werden in den folgenden
beiden Sätzen zusammengefaßt.
...
{\em 
    ...
    so ist $k$ tatsächlich eine Folgerung aus $M$.
    \hspace*{\fill} $\Box$
}
\end{Satz}
...
\end{enumerate}
Zum Abschluß demonstrieren wir das skizzierte Verfahren an einem Beispiel.
...
Der im letzten Abschnitt beschriebene Kalkül läßt sich automatisieren und bildet die Grundlage moderner
automatischer Beweiser.  Gleichzeitig läßt sich auch die Suche nach Gegenbeispielen automatisieren.
