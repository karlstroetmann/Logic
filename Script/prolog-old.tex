...
Wir definieren den reflexiven und transitiven Abschluß $\leadsto_\mathcal{P}^*$ der Relation $\leadsto_\mathcal{P}$ in der üblichen Art und Weise:
\begin{enumerate}
...
Mit der atomaren Formel $\textsl{plus}(s(0), 0, v)$ läßt sich jetzt nur der Kopf der
...
beschreiben muß, wie das Problem im Detail zu lösen ist, sondern nur noch eine präzise
...
Prolog kann aufgefaßt werden als eine Implementierung der Relation
...
\end{enumerate}
An dieser Stelle muß der Prolog--Interpreter sich für eine der beiden Möglichkeiten
...
$H_1 \leftarrow B^{(1)}_1 \wedge \cdots \wedge B^{(1)}_{n(1)}$ die erste Klausel, so wird
 ...
 Falls allerdings im Anschluß daran die Reduktion des Zieles
...
\item Die Ziel--Liste kann jetzt nicht leer sein, sie hat also die Form \\[0.1cm]
      ...
      \item Andernfalls werden alle mögliche Reduktionen des Ziels $Z$ durchgeführt.  Falls \\[0.1cm]
            ...
            Im Anschluß daran gehen wir wieder zu Schritt 1.
      \end{enumerate}
\end{enumerate}
...
Möglichkeit gibt, ein Ziel zu \emph{lösen}, dass heißt auf ein Ziel der Form 
 ...
 werden.  Leider ist dies ein Trugschluß.  Der Grund ist, dass die Reduktions--Relation
$\leadsto_\mathcal{P}$ nicht terminieren muß, es kann durchaus sein, dass es für ein
...
Funktionen zu interpretieren.  Was damit gemeint ist, sehen wir am besten an Hand von
...
an die Reihe kommen.  Sind schließlich alle $B_i$ abgearbeitet, so sind zum Schluß auch die Ausgabe--Variablen von
...
\item \texttt{insert} hat die E/A--Spezifikationen \texttt{insert(+,+,-)}.  Es erwartet
      ...
      die zusätzlich noch in aufsteigender Reihenfolge sortiert sein muß.
...
Zum Abschluß geben wir noch ein Programm an, mit dem es möglich ist, symbolisch zu differenzieren.
...
Regel zum symbolischen Differenzieren aufgefaßt werden kann.
\begin{enumerate}
