...
Da auﬂerdem $\mu_1 \restriction \{u\} = [ u \mapsto s(z)]$ ist haben wir also \\[0.1cm]
...
Prolog ist (leider) eine ungetypte Sprache, das heiﬂt dass es neben dem Typ--Bezeichner
...
bestehen und die zus\"{a}tzlich mit einem Groﬂbuchstaben oder mit dem Zeichen ``\texttt{\_}''
...
bestehen und die mit einem kleinen Buchstaben anfangen.  Auﬂerdem sind W\"{o}rter, die in
...
``\texttt{=<}'',
``\texttt{>=}'' \\[0.1cm]
als Pr\"{a}dikats--Zeichen benutzt.  Schlieﬂlich k\"{o}nnen als 0--stellige Funktions--Zeichen auch
...
Symbol ``$\wedge$'' wird einfach ein Komma geschrieben.  Auﬂerdem ist noch wichtig, dass
...
``\texttt{pl}''.  Wir werden mit einem Prompt der folgenden Form begr\"{u}ﬂt:
\begin{verbatim}
...
\end{verbatim}
Wir k\"{o}nnen diese beiden Klauseln folgendermaﬂen in die Umgangssprache \"{u}bersetzen:
\begin{enumerate}
...
\begin{verbatim}
    ...
    L = [1, 2, 3, a, b, c] 
\end{verbatim}
Die obige Interpretation des gegebenen Prolog--Programms ist \emph{funktional}, dass heiﬂt 
...
gesetzt, dass heiﬂt die Ziel--Liste enth\"{a}lt am Anfang genau das instantiierte Ziel $\langle G, [] \rangle$.  Dann l\"{a}uft der
...
M\"{o}glichkeit gibt, ein Ziel zu \emph{l\"{o}sen}, dass heiﬂt auf ein Ziel der Form 
...
\begin{verbatim}
  ...
  loop.
\end{verbatim}
Wird dieses Programm geladen und wird anschlieﬂend versucht, das Ziel \texttt{loop} zu
...
das heiﬂt der Algorithmus zum L\"{o}sen syntaktischer Gleichungen, ist (aus Effizienzgr\"{u}nden)
...
$\tau$ \emph{instantiiert}.  Auﬂerdem bezeichnen wir die Instantiierung $G\tau$ als \emph{L\"{o}sung} des Ziels $G$.
...
an die Reihe kommen.  Sind schlieﬂlich alle $B_i$ abgearbeitet, so sind zum Schluss auch die Ausgabe--Variablen von
...
{\em
    ...
    \hspace*{1.3cm} $\langle G, [] \rangle \leadsto_\mathcal{P}^* \langle \verum, \tau\rangle$, \\[0.1cm]
    so gilt $\FV(G\tau) = \emptyset$.  Auﬂerdem gen\"{u}gen dann auch alle Ziele $G_i$, die bei dieser Rechnung abgearbeitet werden,
...
Wir schlieﬂen diesen Abschnitt mit einer Definition ab.
...
\item Die zweite Klausel greift, wenn die eingegebene Liste nicht leer ist und wenn auﬂerdem
...
\item Die dritte Klausel greift, wenn die eingegebene Liste nicht leer ist und wenn auﬂerdem
      das als erstes Argument eingegebene Element \texttt{X} gr\"{o}ﬂer oder gleich dem ersten Element \texttt{Head}
...
beliebige Liste ist.  Die Aufgabe von $\texttt{split}$ ist es, diese Liste in zwei etwa gleich groﬂe 
Listen aufzuspalten.  Die Korrektheit der ersten beiden Klauseln von \texttt{split} ist offensichtlich.
Die letzte Klausel ist rekursiv: Um eine Liste der Form \texttt{[ X, Y | Ys ]} in etwa gleich groﬂe 
Listen aufzuspalten, wird zun\"{a}chst die Liste \texttt{Ys} in zwei etwa gleich groﬂe Teile aufgespalten.
...
das erste Element \texttt{X} der ersten Liste \texttt{[ X | Xs ]} gr\"{o}ﬂer als das zweite  Element \texttt{Y} 
...
Zun\"{a}chst wird die zu sortierende Liste mit Hilfe von \texttt{split} in zwei etwa gleich groﬂe Listen aufgeteilt.
...
so w\"{u}rde das entstehende Programm sicherlich wesentlich gr\"{o}ﬂer.
