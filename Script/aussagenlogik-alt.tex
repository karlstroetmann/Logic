...
der Wahrheitswert zusammengesetzter Aussagen aus dem Wahrheitswert der einzelnen Teilaussagen berechnen lässt.  Darauf
...
\begin{enumerate}
\item Einer der drei Verdächtigen muss die Tat begangen haben: \\[0.1cm]
      \hspace*{1.3cm} 
      $f_1 := a \vee b \vee c$.
...
$\falsum$ herleiten
lässt, dann folgt $f$ aus $M$: \\[0.1cm]
\hspace*{1.3cm} $\models k_1 \wedge \cdots \wedge k_n \rightarrow f$ \quad g.d.w. \quad
...
die nur noch $m$ Aussage-Variablen enthält.  Außerdem muss diese Transformation so beschaffen sein,
...
{\em
  ...
  \begin{enumerate}
  \item Wenn die Klausel $l$ enthält, dann muss auch die Klausel wahr sein, denn die
...
Das steht im Widerspruch zu der Voraussetzung $M \models \falsum$.  Damit muss die Annahme, dass $\textsl{Redukt}(M,l)$ erfüllbar ist,
...
\item Fall: $f \el \textsl{Redukt}(M,l)$.
      ...
      Dann muss es eine Klausel $k \el M$ geben, so dass $f = \textsl{redukt}(k,l)$ ist.
...
\item Induktions-Anfang: $n=0$.  
      ...
      muss dann die leere Klausel in $M$ auftreten und damit gilt  $M \gentzen \{\}$.  
...
einer Diagonale steht höchstens eine Dame'' auf dasselbe Grundmuster zurückführen:
