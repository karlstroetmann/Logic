...
\item Ein automatischer Beweiser (eine sogenannte \emph{Inferenz-Maschine})
      erschlieﬂt aus diesen Fakten und Regeln
...
\begin{enumerate}
\item Wenn ein String mit einem groﬂen Buchstaben oder aber mit dem Unterstrich
...
\item Zahlen bezeichnen 0-stellige Funktions-Zeichen. In \textsl{Prolog} k\"{o}nnen Sie sowohl
      ganze Zahlen als auch Flieﬂkomma-Zahlen benutzen.  Die Syntax f\"{u}r Zahlen ist \"{a}hnlich
...
Wenn das \textsl{Prolog}-System installiert ist, begr\"{u}ﬂt uns das System wie folgt:
\begin{verbatim}
...
Fakten und Regeln erstellt, die das Problem vollst\"{a}ndig beschreiben.  Anschlieﬂend
...
um.  Auﬂerdem bezeichnen wir bei einer Klausel \\[0.2cm]
...
\item Gibt es mehrere solche Regeln, so
      ...
      \item Auﬂerdem setzen wir an dieser Stelle einen Auswahl-Punkt (\emph{Choice-Point}),
...
\item Nun   bilden wir die  Anfrage \\[0.2cm]
      ...
      \item Sonst beantworten wir rekursiv die Anfrage $B_1\mu, \cdots, B_m\mu, Q_2\mu, \cdots, Q_n\mu$            . 
            ...
            gemacht.  Anschlieﬂend versuchen wir, mit der n\"{a}chsten m\"{o}glichen Regel
...
als Bl\"{a}tter, die am weitesten unten stehen.  Suchb\"{a}ume stehen also gewissermaﬂen auf dem Kopf: Die
...
\item Die zweite Klausel greift, wenn die Liste, in die die Zahl  \texttt{X} eingef\"{u}gt
      werden soll, nicht leer ist und wenn auﬂerdem
...
\item Die dritte Klausel greift, wenn die Liste, in die die Zahl  \texttt{X} eingef\"{u}gt
      werden soll  nicht leer ist und wenn auﬂerdem \texttt{X} gr\"{o}ﬂer als das erste
...
k\"{o}nnen und wenn auﬂerdem zu jeder Eingabe h\"{o}chstens eine Ausgabe berechnet wird.
...
augenf\"{a}llig sein.  Auﬂerdem ist offensichtlich, dass die obigen Gleichungen 
...
anschlieﬂend die Umsetzung dieser Gleichungen in \textsl{Prolog} angeben.
...
\begin{enumerate}
\item In der ersten Phase wird die zu sortierende Liste in zwei etwa gleich groﬂe
...
\item $\mathtt{length}(l) \geq 2 \rightarrow \texttt{merge\_sort}(l) = \texttt{mix}(
  ...
  \texttt{merge\_sort}(\texttt{even}(l)))$.
      ...
      diese Listen und mischen anschlieﬂend die sortierten Teillisten.
\end{enumerate}
...
\item Zahlen sind arithmetische Ausdr\"{u}cke.
      ...
      Sowohl die ganzen Zahlen als auch die Flieﬂkomma-Zahlen sind Bestandteil der Sprache
...
\item Die beiden Klauseln in den Zeilen 3 -- 9 zeigen, wie eine Variable differenziert werden kann.
      ...
      handelt.  Anschlieﬂend \"{u}berpr\"{u}fen wir in den Zeilen 5 bzw.~9, ob diese Variable mit der
