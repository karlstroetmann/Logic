...
aus dem Wahrheitswert der einzelnen Teilaussagen berechnen läßt.  Darauf
...
\item Analyse und Design digitaler Schaltungen.
      ...
      Gattern\footnote{
        Die Version des Pentium 4 Prozessors mit dem \textsl{Northwood}
        Kernel enthält etwa 55 Millionen Transistoren.}.  
...
\item Erstellung von Verschlußplänen für die Weichen und Signale von Bahnhöfen.
      ...
      Die einzelnen Fahrstraßen werden durch sogenannte \emph{Verschlußpläne} beschrieben.
      Die Korrektheit solcher Verschlußpläne kann durch aussagenlogische Formeln ausgedrückt werden.
...
wie sich diese Semantik in \textsc{SetlX} implementieren läßt.
...
\item Eine aussagenlogische Variable $p \in \mathcal{P}$ repräsentieren wir 
      ...
      aussagenlogische Formel umwandeln wollen, so müssen wir Variablen in dieser Form repräsentieren.
...
es auch wichtig, dass die Repräsentation für die zu entwickelnden Algorithmen \underline{adäquat}
...
aussagenlogischen Formeln die selben Operatoren verwenden, die auch in \textsc{SetlX} selber
...
\\[0.2cm]
\hspace*{1.3cm}
\texttt{\symbol{94}implication(\symbol{94}variable(\symbol{34}p\symbol{34}), \symbol{94}disjunction(\symbol{94}variable(\symbol{34}p\symbol{34}), \symbol{94}negation(\symbol{94}variable(\symbol{34}q\symbol{34}))))}
\\[0.2cm]
Wir erkennen beispielsweise, dass in \textsc{SetlX} der Operator ``\texttt{=>}'' intern
...
Paaren \texttt{[ $p$, $b$ ]}, für die $p$ eine Aussage-Variable ist und für die zusätzlich
$b \in \mathbb{B}$ gilt:
\[ \mathcal{I} \subseteq \mathcal{P} \times \mathbb{B}. \]
Damit können wir jetzt eine einfache Funktion schreiben, dass den Wahrheitswert
einer aussagenlogischen Formel $f$ unter einer gegebenen aussagenlogischen
Interpretation ${\cal I}$ berechnet. Die Funktion
\href{http://wwwlehre.dhbw-stuttgart.de/stroetmann/Logic/SetlX/evaluate.stlx}{\texttt{evaluate.stlx}}
...
\href{http://wwwlehre.dhbw-stuttgart.de/stroetmann/Logic/SetlX/watson.stlx}{\texttt{watson.stlx}}.
...
\begin{Verbatim}[ frame         = lines, 
    ...
    // This procedure turns a subset m of set set of all variables 
    ...
    s  := { i in b | forall (f in fs | evaluate(f, i)) };
...
gezeigte Funktion
\href{http://wwwlehre.dhbw-stuttgart.de/stroetmann/Logic/SetlX/tautology.stlx}{\texttt{tautology}}
testet, ob die als Argument übergebene
aussagenlogische Formel $f$ allgemeingültig ist.   
Die Prozedur verwendet die Funktion 
\href{http://wwwlehre.dhbw-stuttgart.de/stroetmann/Logic/SetlX/evaluate.stlx}{\texttt{evaluate}}
...
$\bigl\{ \{p, q, \neg r \},\, \{ p, \neg q, \neg r \} \bigr\}$.
\\[0.2cm]
Wir stellen nun ein Verfahren vor, mit dem sich jede Formel in KNF transformieren läßt.  Nach
...
Prozedur 
\href{http://wwwlehre.dhbw-stuttgart.de/stroetmann/Logic/SetlX/knf.stlx}{\texttt{elimGdw}}.
...
\begin{Verbatim}[ frame         = lines, 
    ...
    elimGdw := procedure(f) {
        match (f) {
            ...
            case g || h   : return elimGdw(g) || elimGdw(h);
            case g => h   : return elimGdw(g) => elimGdw(h);
            case g <==> h : return (elimGdw(g) => elimGdw(h)) && (elimGdw(h) => elimGdw(g));
            default       : return f; 
        }
    };
\end{Verbatim}
...
\href{http://wwwlehre.dhbw-stuttgart.de/stroetmann/Logic/SetlX/knf.stlx}{\texttt{elimFolgt}}.
...
\href{http://wwwlehre.dhbw-stuttgart.de/stroetmann/Logic/SetlX/knf.stlx}{\texttt{nnf}} und
\href{http://wwwlehre.dhbw-stuttgart.de/stroetmann/Logic/SetlX/knf.stlx}{\texttt{neg}},
...
\href{http://wwwlehre.dhbw-stuttgart.de/stroetmann/Logic/SetlX/knf.stlx}{\texttt{knf}}.
\begin{enumerate}
...
\\[0.2cm]
\hspace*{1.3cm}
\verb'{ args(l)[1] : l in c | fct(l) == "^negation" }'
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
\texttt{\symbol{94}negation(p)}
\\[0.2cm]
dargestellt.  Falls also das Literal $l$ die Form $l = \neg p$ hat, so hat der Term $l$
das Funktions-Zeichen \texttt{\symbol{94}negation}.  Die Anwendung der Funktion
...
\texttt{\symbol{94}negation} und diese Liste enthält als einziges Element gerade die
...
\href{http://wwwlehre.dhbw-stuttgart.de/stroetmann/Logic/SetlX/knf.stlx}{\texttt{knf.stlx}}
auf meiner Webseite.
...
dar. Wir numerieren die Zeilen dabei von oben beginnend von 1 bis 8 durch, während die
...
\end{itemize}
auf das selbe Grundmuster zurückführen:
...
\href{http://wwwlehre.dhbw-stuttgart.de/stroetmann/Logic/SetlX/queens.stlx}{\texttt{queens.stlx}}.
