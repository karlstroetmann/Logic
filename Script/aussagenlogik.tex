...
\section{Ãberblick}
Die Aussagenlogik beschÃ¤ftigt sich mit der VerknÃ¼pfung einfacher Aussagen durch
...
Einfache Aussagen sind dabei SÃ¤tze, die 
\begin{itemize}
\item einen Tatbestand ausdrÃ¼cken, der entweder wahr oder falsch ist und 
\item selber keine Junktoren enthalten.
\end{itemize}
Beispiele fÃ¼r einfache Aussagen sind
\begin{enumerate}
...
\emph{zusammengesetzten Aussagen} verknÃ¼pfen.  
Ein Beispiel fÃ¼r eine zusammengesetzte Aussage wÃ¤re \\[0.2cm]
...
aus dem Wahrheitswert der einzelnen Teilaussagen berechnen lÃ¤sst.  Darauf
aufbauend wird dann gefragt, in welcher Art und Weise wir aus gegebenen Aussagen neue 
Aussagen logisch folgern kÃ¶nnen.

Um die Struktur komplexerer Aussagen Ã¼bersichtlich werden zu lassen, fÃ¼hren wir in der Aussagenlogik zunÃ¤chst sogenannte
\emph{Aussage-Variablen} ein.  Diese stehen fÃ¼r atomare Aussagen.
ZusÃ¤tzlich fÃ¼hren wir fÃ¼r die Junktoren
``\textsl{nicht}'', ``\textsl{und}'', ``\textsl{oder}'', ``\textsl{wenn, $\cdots$ dann}'', und ``\textsl{genau dann, wenn}'' die
folgenden AbkÃ¼rzungen ein:
\begin{enumerate}
\item $\neg a$ \quad\quad\ fÃ¼r \quad \textsl{nicht} $a$ 
\item $a \wedge b$ \,\quad\ fÃ¼r \quad $a$ \textsl{und} $b$
\item $a \vee b$ \,\quad\ fÃ¼r \quad $a$ \textsl{oder} $b$
\item $a \rightarrow b$   \quad fÃ¼r \quad \textsl{wenn} $a$, \textsl{dann} $b$
\item $a \leftrightarrow b$ \quad fÃ¼r \quad  $a$ \textsl{genau dann, wenn} $b$
\end{enumerate}
Aussagenlogische Formeln werden aus Aussage-Variablen mit Hilfe von Junktoren aufgebaut
und kÃ¶nnen beliebig komplex sein.  Die Aussage (1) kÃ¶nnen wir mit Hilfe der Junktoren  kÃ¼rzer als
 \\[0.2cm]
...
\hspace*{1.3cm}  \texttt{Regenbogen}. \\[0.2cm]
 Um Beweis-Prinzipien Ã¼bersichtlicher angeben zu kÃ¶nnen,
fÃ¼hren wir die folgende Notation ein: 
...
$$
Die Aussagen Ã¼ber dem Bruchstrich bezeichnen wir als \emph{PrÃ¤missen}, die Aussage unter dem Bruchstrich ist die \emph{Konklusion}.
Statt Beweis-Prinzip sagen wir oft auch \emph{Schluss-Regel}.

 Wir stellen fest, dass die obige Schluss-Regel unabhÃ¤ngig von dem Wahrheitswert der Aussagen in dem folgenden Sinne
gÃ¼ltig ist: Wenn alle PrÃ¤missen gÃ¼ltig sind, dann folgt aus logischen GrÃ¼nden auch die GÃ¼ltigkeit der Konklusion.
 Um dieses weiter formalisieren zu kÃ¶nnen, ersetzen wir die Aussage-Variablen
 \texttt{SonneScheint}, \texttt{EsRegnet} und \texttt{Regenbogen} durch die
 \emph{Meta-Variablen} $p$, $q$ und $r$, die fÃ¼r beliebige aussagenlogische Formeln stehen.
...
\textsl{Wenn es regnet, ist die StraÃe nass.  Es regnet nicht.  Also ist die StraÃe nicht nass. \eox} 
\end{minipage}  
\end{center}
...
auf die StraÃe kippen.  \qed
\vspace*{0.1cm}

Dadurch, dass wir ausgehend von Beobachtungen und als wahr erkannten Tatsachen und ZusammenhÃ¤ngen
mehrere \emph{logische SchlÃ¼sse} aneinander fÃ¼gen, erhalten wir einen \emph{Beweis}. 
...
\item $\vdash$ bezeichnet ein System von Schluss-Regeln.  Eine solches System bezeichnen wir auch als \emph{KalkÃ¼l}.
\item $r$ ist eine Aussage.  
\end{itemize}
Die Schreibweise $M \vdash r$ wÃ¤re dann als \\[0.2cm]
...
Anwenden verschiedener Schluss-Regeln die Aussage $r$ beweisen kÃ¶nnen.
Das Zeichen $\vdash$ steht fÃ¼r die Menge aller Schluss-Regeln und symbolisiert damit den
\emph{Herleitungs-Begriff}, den wir auch als \emph{KalkÃ¼l} bezeichnen.  Wir werden  in einem
spÃ¤teren Abschnitt den KalkÃ¼l formal definieren.
...
Das kÃ¶nnen wir anders auch so
formulieren:  Immer wenn alle Aussagen aus $M$ wahr sind, dann ist auch die Aussage $r$ wahr.
Wir kÃ¶nnen den Begriff der \emph{logischen Folgerung} aber
erst dann prÃ¤zise definieren, wenn wir die Semantik der Junktoren mathematisch festgelegt haben.

Ziel der Aussagenlogik ist es, einen
Herleitungsbegriff zu finden, der die folgenden beiden Bedingungen erfÃ¼llt:
\begin{enumerate}
\item Der Herleitungsbegriff sollte {\bf korrekt} sein, es sollte also nicht mÃ¶glich sein,
      Unsinn zu beweisen.  Es sollte also gelten \\[0.2cm]
      \hspace*{1.3cm} Aus $M \vdash r$ folgt $M \models r$. 
      \\[0.2cm]
      Wenn wir die Aussage $r$ aus den Axiomen der Menge $M$ herleiten kÃ¶nnen, dann 
      soll $r$ auch aus $M$ folgen.
\item Der Herleitungsbegriff sollte {\bf vollstÃ¤ndig} sein, d.h.~wenn eine Aussage $r$
      ...
      hergeleitet werden kÃ¶nnen.
\end{enumerate}

Bestimmte aussagenlogische Formeln sind offenbar immer wahr, egal was
 wir fÃ¼r die einzelnen Teilaussagen einsetzen.  Beispielsweise ist eine Formel der Art
\\[0.2cm]
\hspace*{1.3cm}
$p \vee \neg p$
\\[0.2cm]
unabhÃ¤ngig von dem Wahrheitswert der Aussage $p$ immer wahr.  Eine aussagenlogische
...
$p \wedge \neg p$
\\[0.2cm]
immer falsch.  Eine Formel heiÃt \emph{erfÃ¼llbar}, wenn es wenigstens eine MÃ¶glichkeit
gibt, bei der die Formel wahr wird.  Im Rahmen der Vorlesung werden wir verschiedene Verfahren
entwickeln, mit denen es mÃ¶glich ist zu entscheiden, ob eine aussagenlogische Formel eine
Tautologie ist oder ob Sie wenigstens erfÃ¼llbar ist.  Solche Verfahren spielen
in der Praxis eine wichtige Rolle.

\section{Anwendungen der Aussagenlogik}
Die Aussagenlogik bildet nicht nur die Grundlage fÃ¼r die PrÃ¤dikatenlogik, sondern sie hat auch wichtige praktische
Anwendungen.  Aus der groÃen Zahl der industriellen Anwendungen mÃ¶chte ich stellvertretend vier Beispiele nennen:
\begin{enumerate}
\item Analyse und Design digitaler Schaltungen.
      ...
      Gattern\footnote{
        ...
        enthÃ¤lt mehr als 4 Milliarden Transistoren.}.  
      ...
      etc.~auf elektronischer Ebene reprÃ¤sentiert. 
  
      Die KomplexitÃ¤t solcher Schaltungen wÃ¤re ohne den Einsatz
      rechnergestÃ¼tzter Verfahren zur Verifikation nicht mehr beherrschbar.  Die
      ...
      AnschlieÃend wird versucht, mit aussagenlogischen Mitteln die Ãquivalenz dieser
      Formeln zu zeigen. Software-Werkzeuge, die fÃ¼r die Verifikation digitaler
      Schaltungen eingesetzt werden, kosten heutzutage Ã¼ber $100\,000\,\symbol{36}$\footnote{
      ...
      Eine solche Lizenz ist dann drei Jahre lang gÃ¼ltig.}.
      Dies zeigt die wirtschaftliche Bedeutung der Aussagenlogik.

\item Erstellung von EinsatzplÃ¤nen (\emph{crew sheduling}).

      International tÃ¤tige Fluggesellschaften mÃ¼ssen bei der Einteilung ihrer Crews
      einerseits gesetzlich vorgesehene Ruhezeiten einhalten, wollen aber ihr Personal
      mÃ¶glichst effizient einsetzen.  Das fÃ¼hrt zu Problemen, die sich mit Hilfe
      aussagenlogischer Formeln beschreiben und lÃ¶sen lassen.
\item Erstellung von VerschlussplÃ¤nen fÃ¼r die Weichen und Signale von BahnhÃ¶fen.

      Bei einem grÃ¶Ãeren Bahnhof gibt es einige hundert Weichen und Signale, die stÃ¤ndig
      neu eingestellt werden mÃ¼ssen, um sogenannte \emph{FahrstraÃen} fÃ¼r die ZÃ¼ge zu
      realisieren.  Verschiedene FahrstraÃen dÃ¼rfen sich aus SicherheitsgrÃ¼nden nicht kreuzen.  
      Die einzelnen FahrstraÃen werden durch sogenannte \emph{VerschlussplÃ¤ne} beschrieben.
      Die Korrektheit solcher VerschlussplÃ¤ne kann durch aussagenlogische Formeln ausgedrÃ¼ckt werden.
\item Eine Reihe kombinatorischer Puzzles lassen sich als aussagenlogische Formeln
      kodieren und kÃ¶nnen dann mit Hilfe aussagenlogischer Methoden lÃ¶sen.  Als ein
      Beispiel werden wir in der Vorlesung das 8-Damen-Problem behandeln.  Dabei geht es um die Frage,
      ob 8 Damen so auf einem Schachbrett angeordnet werden kÃ¶nnen, dass keine der Damen
      eine andere Dame bedroht.
\end{enumerate}

\section{Formale Definition der aussagenlogischen Formeln}
Wir behandeln zunÃ¤chst die \emph{Syntax} der Aussagenlogik und besprechen anschlieÃend die
...
wie sich diese Semantik in \textsc{SetlX} implementieren lÃ¤sst.
...
Aussagenlogische Formeln sind dann WÃ¶rter, die aus dem Alphabet
...
\item $\verum \in \mathcal{F}$ und $\mathtt{\falsum} \in \mathcal{F}$.
      ...
      Hier steht $\verum$ fÃ¼r die Formel, die immer wahr ist, wÃ¤hrend $\falsum$ fÃ¼r die 
      Formel steht, die immer falsch ist.  Die Formel $\verum$ trÃ¤gt auch den Namen \textsl{Verum},
      fÃ¼r $\falsum$ sagen wir auch \textsl{Falsum}.
...
gebildeten WÃ¶rter, die den oben aufgestellten Forderungen genÃ¼gt.
...
\begin{enumerate}
\item ÃuÃere Klammern werden weggelassen, wir schreiben also beispielsweise \\[0.2cm]
...
\item Die Junktoren $\vee$ und $\wedge$ binden stÃ¤rker als $\rightarrow$, wir schreiben
      also \\[0.2cm]
      ...
      anders als in der Sprache \textsc{SetlX}, denn dort bindet der Operator ``\texttt{\&\&}'' stÃ¤rker als
      der Operator ``\texttt{||}''.
\item Der Junktor $\rightarrow$ bindet stÃ¤rker als $\leftrightarrow$, wir schreiben
      also \\[0.2cm]
      \hspace*{1.3cm} $p \rightarrow q \leftrightarrow r$ \quad statt \quad $(p \rightarrow q) \leftrightarrow r$.
\end{enumerate}

\remark
Wir werden im Rest dieser Vorlesung eine Reihe von Beweisen fÃ¼hren, bei denen es darum geht,
mathematische Aussagen Ã¼ber Formeln nachzuweisen.  Bei diesen Beweisen werden wir natÃ¼rlich
...
\item Bei den Beweisen, die wir Ã¼ber aussagenlogische Formeln fÃ¼hren, schreiben wir fÃ¼r diesen
...
Um aussagenlogischen Formeln einen Wahrheitswert zuordnen zu kÃ¶nnen, definieren wir
zunÃ¤chst die Menge $\mathbb{B}$ der Wahrheitswerte:  \\[0.2cm] 
\hspace*{1.3cm} $\mathbb{B} := \{ \mathtt{true}, \mathtt{false} \}$. \\[0.2cm]
Damit kÃ¶nnen wir nun
...
Um nicht nur Variablen sondern auch aussagenlogische Formel interpretieren zu kÃ¶nnen, 
benÃ¶tigen wir eine
...
mit deren Hilfe wir die aussagenlogischen Junktoren interpretieren kÃ¶nnen:
\begin{enumerate}
...
aufgefasst werden kÃ¶nnen.  Die Funktion $\circneg$ dreht die Wahrheits-Werte um und
...
Wir kÃ¶nnten auch die Funktionen $\circwedge$, $\circvee$, $\circright$ und $\circleftright$ als
...
\end{table}
Nun kÃ¶nnen wir den Wert, den eine aussagenlogische Formel $f$ unter einer gegebenen
...
\item $\widehat{\mathcal{I}}(p) := \mathcal{I}(p)$ fÃ¼r alle $p \in \mathcal{P}$.
\item $\widehat{\mathcal{I}}(\neg f) := \circneg\;\bigl(\widehat{\mathcal{I}}(f)\bigr)$ fÃ¼r alle $f \in \mathcal{F}$.
\item $\widehat{\mathcal{I}}(f \wedge g) := \circwedge\;\bigl(\widehat{\mathcal{I}}(f), \widehat{\mathcal{I}}(g)\bigr)$ 
      fÃ¼r alle $f, g \in \mathcal{F}$.
...
\item $\widehat{\mathcal{I}}(f \leftrightarrow g) := \circleftright\;\bigl(\widehat{\mathcal{I}}(f), \widehat{\mathcal{I}}(g)\bigr)$ 
      fÃ¼r alle $f, g \in \mathcal{F}$.
\end{enumerate}
Um die Schreibweise nicht Ã¼bermÃ¤Ãig kompliziert werden zu lassen, unterscheiden wir in
Zukunft nicht  mehr zwischen $\widehat{\mathcal{I}}$ und $\mathcal{I}$, wir werden das HÃ¼tchen Ã¼ber dem
$\mathcal{I}$ also weglassen.

\noindent
\textbf{Beispiel}: Wir zeigen, wie sich der Wahrheits-Wert der Formel
$$  (p \rightarrow q) \rightarrow (\neg p \rightarrow q) \rightarrow q $$
fÃ¼r die aussagenlogische Interpretation $\mathcal{I}$, die durch 
...
eben durchgefÃ¼hrte Rechnung fÃ¼r die Praxis zu umstÃ¤ndlich.  Stattdessen wird der Wert
...
$$  (p \rightarrow q) \rightarrow (\neg p \rightarrow q) \rightarrow q $$
fÃ¼r beliebige Belegungen $\mathcal{I}$ Ã¼ber diese Tabelle berechnen kÃ¶nnen.
 Um nun die Wahrheitswerte 
dieser Formel unter einer gegebenen Belegung der Aussage-Variablen bestimmen zu kÃ¶nnen,
 bauen wir eine  Tabelle auf, die fÃ¼r jede in der Formel
auftretende Teilformel eine Spalte enthÃ¤lt.  Tabelle \ref{tab:tautologie} auf Seite
...
$(p \rightarrow q) \rightarrow (\neg p \rightarrow q) \rightarrow q $
fÃ¼r jede aussagenlogische Belegung $\mathcal{I}$ den Wert \texttt{true}.  
...
Wir erlÃ¤utern die Aufstellung dieser Tabelle anhand der zweiten Zeile.  In dieser Zeile sind zunÃ¤chst die
...
\end{enumerate}
FÃ¼r komplexe Formeln ist die Auswertung von Hand viel zu mÃ¼hsam und
fehleranfÃ¤llig um praktikabel zu sein.  Wir zeigen deshalb im Ã¼bernÃ¤chsten Abschnitt, wie
sich dieser Prozess automatisieren lÃ¤sst.
...
\\[0.2cm]
berechnen wollen, so mÃ¼ssen wir die Details der Teilformeln $f$ und $g$ nicht kennen, es reicht,
...
``\textsl{wenn $\cdots$, dann}'' nur eine Approximation der umgangssprachlichen Interpretation, die sich fÃ¼r die
...
Um die bisher eingefÃ¼hrten Begriffe nicht zu abstrakt werden zu lassen,
...
mÃ¼ssen wir uns als erstes fragen, wie wir die Argumente der zu implementierenden Funktion und die
Ergebnisse dieser Funktion in der verwendeten Programmier-Sprache darstellen kÃ¶nnen.
In diesem Fall mÃ¼ssen wir uns also Ã¼berlegen, wie wir eine
aussagenlogische Formel in \textsc{SetlX} reprÃ¤sentieren kÃ¶nnen, denn Ergebnisswerte
\texttt{true} und \texttt{false} stehen ja als Wahrheitswerte unmittelbar zur VerfÃ¼gung.
Zusammengesetzte Daten-Strukturen kÃ¶nnen in \textsc{SetlX} am einfachsten als
Terme dargestellt werden und das ist auch der Weg, den wir fÃ¼r die aussagenlogischen
Formeln  beschreiten werden.  Wir definieren die ReprÃ¤sentation von
...
``\texttt{<==>}'' benutzen, denn damit kÃ¶nnen wir die aussagenlogischen Formeln in sehr natÃ¼rlicher
Weise darstellen.
\begin{enumerate}
\item $\verum$ wird reprÃ¤sentiert durch den Wahrheitswert \texttt{true}.
      \\[0.2cm]
      \hspace*{1.3cm}
      $\textsl{rep}(\verum) := \mathtt{true}$
\item $\falsum$  wird reprÃ¤sentiert durch den Wahrheitswert \texttt{false}.
      \\[0.2cm]
      \hspace*{1.3cm}
      $\textsl{rep}(\falsum) := \mathtt{false}$
\item Eine aussagenlogische Variable $p \in \mathcal{P}$ reprÃ¤sentieren wir 
      durch einen Term der Form
      \\[0.2cm]
      \hspace*{1.3cm}
      \texttt{\symbol{94}variable(p)}.
      \\[0.2cm]
      Der Grund fÃ¼r diese zunÃ¤chst seltsam anmutende Darstellung der Variable liegt darin,
      dass \textsc{SetlX} intern Variablen in der obigen Form darstellt.  Wenn wir spÃ¤ter
      ...
      \\[0.2cm]
      \hspace*{1.3cm}
      $\textsl{rep}(p) := \texttt{\symbol{94}variable}(p)$ \quad fÃ¼r alle $p \in \mathcal{P}$.
\item Ist $f$ eine aussagenlogische Formel, so reprÃ¤sentieren wir $\neg f$ mit Hilfe des Operators
      ``\texttt{!}'': \\[0.2cm]
      \hspace*{1.3cm} 
      $\textsl{rep}(\neg f) := \texttt{!} \textsl{rep}(f)$.
\item Sind $f_1$ und $f_2$ aussagenlogische Formel, so reprÃ¤sentieren wir $f_1 \vee f_2$ mit
      Hilfe des Operators  ``\texttt{||}'': \\[0.2cm]
      \hspace*{1.3cm} 
      $\textsl{rep}(f \vee g) := \textsl{rep}(f) \;\texttt{||}\; \textsl{rep}(g)$.
\item Sind $f_1$ und $f_2$ aussagenlogische Formel, so reprÃ¤sentieren wir $f_1 \wedge f_2$ mit
      Hilfe des Operators ``\texttt{\&\&}'': \\[0.2cm]
      \hspace*{1.3cm} 
      $\textsl{rep}(f \wedge g) := \textsl{rep}(f) \;\texttt{\&\&}\; \textsl{rep}(g)$.
\item Sind $f_1$ und $f_2$ aussagenlogische Formel, so reprÃ¤sentieren wir $f_1 \rightarrow f_2$ mit Hilfe
      des Operators  ``\texttt{=>}'': \\[0.2cm]
      \hspace*{1.3cm} 
      $\textsl{rep}(f \rightarrow g) := \textsl{rep}(f) \;\texttt{=>}\; \textsl{rep}(g)$.
\item Sind $f_1$ und $f_2$ aussagenlogische Formel, so reprÃ¤sentieren wir 
      $f_1 \leftrightarrow f_2$ mit Hilfe des Operators ``\texttt{<==>}'': \\[0.2cm] 
      \hspace*{1.3cm} 
      $\textsl{rep}(f \leftrightarrow g) := 
       \textsl{rep}(f) \;\texttt{<==>}\; \textsl{rep}(g)$.
\end{enumerate}
Bei der Wahl der ReprÃ¤sentation, mit der wir eine Formel in \setl\ repÃ¤sentieren,
sind wir weitgehend frei.  Wir hÃ¤tten oben sicher auch eine andere ReprÃ¤sentation
verwenden kÃ¶nnen.  Beispielsweise wurden in einer frÃ¼heren Version dieses Skriptes die
aussagenlogischen Formeln als Listen reprÃ¤sentiert.
Eine gute ReprÃ¤sentation sollte einerseits mÃ¶glichst intuitiv sein, andererseits ist
es auch wichtig, dass die ReprÃ¤sentation fÃ¼r die zu entwickelnden Algorithmen \underline{adÃ¤}q\underline{uat}
ist.  Im wesentlichen heiÃt dies, dass es einerseits einfach sein sollte, auf
die Komponenten einer Formel zuzugreifen, andererseits sollte es auch leicht sein,
die entsprechende ReprÃ¤sentation zu erzeugen.  Da wir zur Darstellung der
aussagenlogischen Formeln dieselben Operatoren verwenden, die auch in \textsc{SetlX} selber
benutzt werden, kÃ¶nnen wir die in \textsc{SetlX} vordefinierte Funktion \texttt{parse}
...
\texttt{p => p || !q}.
\\[0.2cm]
Mit Hilfe der \textsc{SetlX}-Funktion \texttt{canonical} kÃ¶nnen wir uns anschauen, wie die
...
der Operator ``\texttt{\&\&}'' wird durch ``\texttt{\symbol{94}conjunction}'' reprÃ¤sentiert und
der Operator ``\texttt{!}'' wird intern als  ``\texttt{\symbol{94}not}'' geschrieben.

Als nÃ¤chstes geben wir an, wie wir eine aussagenlogische Interpretation in \textsc{SetlX}
...
die auch in der Formel $f$ auftreten.  Wir kÃ¶nnen daher die Interpretation
${\cal I}$ durch eine funktionale Relation darstellen, also durch eine Menge von
Paaren der Form \texttt{[ $p$, $b$ ]}, fÃ¼r die $p$ eine Aussage-Variable ist und fÃ¼r die zusÃ¤tzlich
$b \in \mathbb{B}$ gilt:
\[ \mathcal{I} \subseteq \mathcal{P} \times \mathbb{B}. \]
Damit kÃ¶nnen wir jetzt eine einfache Funktion schreiben, die den Wahrheitswert
...
\noindent
Wir diskutieren jetzt die Implementierung der Funktion \texttt{evaluate()} Zeile fÃ¼r
Zeile:
\begin{enumerate}
\item Falls die Formel $f$ den Wert \texttt{true} hat, so repÃ¤sentiert $f$ die Formel $\verum$.
      Also ist das Ergebnis der Auswertung unabhÃ¤ngig von der aussagenlogischen
      Interpretation $i$ immer \texttt{true}. 
\item Falls die Formel $f$ den Wert \texttt{false} hat, so repÃ¤sentiert $f$ die Formel
      $\falsum$.  Also ist das Ergebnis der
      Auswertung unabhÃ¤ngig von der aussagenlogischen Interpretation $i$ immer \texttt{false}.
\item In Zeile 5 betrachten wir den Fall, dass das Argument $f$ eine aussagenlogische
      Variable reprÃ¤sentiert.   

      In diesem Fall mÃ¼ssen wir die Belegung $i$, die ja eine Funktion
      ...
      kÃ¶nnen wir diese Relation durch den Ausdruck $i[p]$ sehr einfach fÃ¼r die Variable 
      $p$ auswerten.
...
\item In Zeile 7 betrachten wir den Fall, dass $f$ die Form 
      ...
      Formel $g_1 \wedge g_2$ reprÃ¤sentiert.
      In diesem Fall werten wir zunÃ¤chst $g_1$ und $g_2$ unter der Belegung $i$ 
      aus und verknÃ¼pfen  das Ergebnis mit dem Operator ``\texttt{\&\&}''.
\item In Zeile 8 betrachten wir den Fall, dass $f$ die Form 
      $g_1 \;\texttt{||}\; g_2$ hat und folglich die Formel $g_1 \vee g_2$ reprÃ¤sentiert.
      In diesem Fall werten wir zunÃ¤chst $g_1$ und $g_2$ unter der Belegung $i$ 
      aus und verknÃ¼pfen  das Ergebnis mit dem Operator ``\texttt{||}''.
\item In Zeile 9 betrachten wir den Fall, dass $f$ die Form 
       $g_1 \;\texttt{=>}\; g_2$ hat und folglich die 
      Formel $g_1 \rightarrow g_2$       reprÃ¤sentiert.
      In diesem Fall werten wir zunÃ¤chst $g_1$ und $g_2$ unter der Belegung $i$ 
      aus und benutzen dann den Operator ``\texttt{=>}'' der Sprache \textsc{SetlX}.
\item In Zeile 10 fÃ¼hren wir die Auswertung einer Formel $g_1 \;\texttt{<==>}\; g_2$
      auf die Gleichheit zurÃ¼ck: Die Formel $f \leftrightarrow g$ ist genau dann wahr,
      wenn $f$ und $g$ den selben Wahrheitswert haben.
\item Wenn keiner der vorhergehenden FÃ¤lle greift, liegt ein Syntax-Fehler vor, 
...
einem JuweliergeschÃ¤ft gerufen, in das eingebrochen worden ist.
In der unmittelbaren Umgebung werden drei VerdÃ¤chtige Anton, Bruno und Claus festgenommen.
Die Auswertung der Akten ergibt folgendes:
\begin{enumerate}
\item Einer der drei VerdÃ¤chtigen muss die Tat begangen haben: \\[0.2cm]
      \hspace*{1.3cm} 
      $f_1 := a \vee b \vee c$.
\item Wenn Anton schuldig ist, so hat er genau einen Komplizen. 

      Diese Aussage zerlegen wir zunÃ¤chst in zwei Teilaussagen:
      \begin{enumerate}
      \item Wenn Anton schuldig ist, dann hat er mindestens einen Komplizen: \\[0.2cm]
            \hspace*{1.3cm} $f_2 := a \rightarrow b \vee c$ 
      \item Wenn Anton schuldig ist, dann hat er hÃ¶chstens einen Komplizen: \\[0.2cm]
...
\item Wenn genau zwei schuldig sind, dann ist Claus einer von ihnen.
      ...
      formulieren lÃ¤sst.  Wir behelfen uns mit einem Trick und Ã¼berlegen uns, wann die
...
Wir fragen uns nun, fÃ¼r welche Belegungen $\mathcal{I}$ alle Formeln aus $F$ wahr werden.
Wenn es genau eine Belegungen gibt, fÃ¼r die dies der Fall ist, dann liefert uns die
Belegung den oder die TÃ¤ter.  Eine Belegung entspricht dabei 1-zu-1 der Menge der TÃ¤ter.
HÃ¤tten wir beispielsweise \\[0.2cm]
\hspace*{1.3cm} 
$\mathcal{I} = \bigl\{ \pair(a,\mathtt{false}), \pair(b,\mathtt{false}), \pair(c,\mathtt{true}) \bigr\}$,
\\[0.2cm]
so wÃ¤re Claus der alleinige TÃ¤ter.  Diese Belegung lÃ¶st unser Problem offenbar
nicht, denn Sie widerspricht der dritten Aussage: Da Bruno unschuldig wÃ¤re, wÃ¤re dann auch
Claus unschuldig.  Da es zu zeitraubend ist, alle Belegungen von Hand auszuprobieren,
schreiben wir besser ein Programm, das fÃ¼r uns die notwendige Berechnung durchfÃ¼hrt.
...
Wir diskutieren diese Programm nun Zeile fÃ¼r Zeile.
...
\end{Verbatim}
\vspace*{-0.3cm}
  \caption{Programm zur AufkÃ¤rung des Einbruchs.}
  \label{fig:watson.stlx}
\end{figure}

\begin{enumerate}
\item In den Zeilen 6 -- 16 definieren wir die Formeln $f_1$, $\cdots$, $f_6$      .
      Wir mÃ¼ssen hier die Formeln in die \textsc{SetlX}-ReprÃ¤sentation bringen.
      Diese Arbeit wird uns durch die Benutzung der Funktion \texttt{parse} leicht gemacht.
\item Als nÃ¤chstes mÃ¼ssen wir uns Ã¼berlegen, wie wir alle Belegungen aufzÃ¤hlen kÃ¶nnen. 
      Wir hatten oben schon beobachtet, dass die Belegungen 1-zu-1 zu den mÃ¶glichen Mengen der TÃ¤ter
      korrespondieren.  Die Mengen der mÃ¶glichen TÃ¤ter sind aber alle Teilmengen der Menge
      \\[0.2cm]
      \hspace*{1.3cm}
      $\{ \squote{a}, \squote{b}, \squote{c} \}$. 
      \\[0.2cm]
      Wir berechnen daher in Zeile 18 zunÃ¤chst die Menge aller dieser Teilmengen.
\item Wir brauchen jetzt eine MÃ¶glichkeit, eine Teilmenge in eine Belegung umzuformen.
      ...
      $. 
      \\[0.2cm]
      Das allgemeine Prinzip ist offenbar, dass fÃ¼r eine aussagenlogische Variable
      ...
      in $\mathcal{I}$.  Damit kÃ¶nnten wir die Menge aller Belegungen, die genau die
      Elemente aus $m$ wahrmachen, wie folgt schreiben:
      \\[0.2cm]
      \hspace*{1.3cm}      
      \texttt{\{ [ x, true ] : x in m \} + \{ [ x, false ] : x in A | !(x in m) \}}
      \\[0.2cm]
      Es geht aber einfacher, denn wir kÃ¶nnen beide FÃ¤lle zusammenfassen, indem wir fordern,
      dass das Paar $\pair(x, x \el m)$ ein Element der Belegung $\mathcal{I}$ ist. Genau
      das steht in Zeile 4.
\item In Zeile 22 sammeln wir in der Menge $b$ alle mÃ¶glichen Belegungen auf.
\item In Zeile 23 berechnen wir die Menge $s$ aller der Belegungen $i$, fÃ¼r die alle 
      Formeln aus der Menge $\textsl{fs}$ wahr werden. 
\item Falls es genau eine Belegung gibt, die alle Formeln wahr macht, 
      dann haben wir das Problem lÃ¶sen kÃ¶nnen.  In diesem Fall
      extrahieren wir in Zeile 26 diese Belegungen aus der Menge $s$ und geben
      anschlieÃemd die Menge der TÃ¤ter aus.
\end{enumerate}
Lassen wir das Programm laufen, so erhalten wir als Ausgabe
\begin{verbatim}
    Set of offenders: {"b", "c"}
\end{verbatim}
Damit liefern unsere ursprÃ¼nglichen Formeln ausreichende Information um die TÃ¤ter zu Ã¼berfÃ¼hren:
...
$$  (p \rightarrow q) \rightarrow (\neg p \rightarrow q) \rightarrow q $$
fÃ¼r jede aussagenlogische Interpretation wahr ist, denn in der letzten Spalte dieser Tabelle steht immer der
...
\begin{Definition}[Tautologie]
  ...
  \hspace*{1.3cm} $\mathcal{I}(f) = \mathtt{true}$ \quad fÃ¼r jede aussagenlogische Interpretation $\mathcal{I}$, \\[0.2cm]
...
\end{enumerate}
Wir kÃ¶nnen die Tatsache, dass es sich bei diesen Formeln um Tautologien handelt, durch
...
einfach, allerdings zu ineffizient, wenn die Anzahl der aussagenlogischen Variablen groÃ
ist.  Ziel dieses Kapitels ist daher die Entwicklung besserer Verfahren.

Die letzten beiden Beispiele in der obigen AufzÃ¤hlung geben Anlass zu einer neuen Definition.
\begin{Definition}[Ãquivalent]
  Zwei Formeln $f$ und $g$ heiÃen \emph{Ã¤quivalent} g.d.w.  \\[0.2cm]
...
\models \neg \falsum \leftrightarrow \verum & \models \neg \verum \leftrightarrow \falsum &  \\[0.2cm]
 ...
 \models p \wedge q \leftrightarrow q \wedge p & \models p \vee   q \leftrightarrow q \vee p & \mbox{KommutativitÃ¤t} \\[0.2cm]
 \models (p \wedge q) \wedge r \leftrightarrow p \wedge (q \wedge r) & \models (p \vee   q) \vee r \leftrightarrow p \vee   (q \vee r)  &
 \mbox{AssoziativitÃ¤t} \\[0.2cm]
 ...
 \models p \vee   (q \wedge r) \leftrightarrow (p \vee q)   \wedge (p \vee   r) & \mbox{DistributivitÃ¤t} \\[0.2cm]
...
\end{array}$ \\[0.3cm]
Wir kÃ¶nnen diese Ãquivalenzen nachweisen, indem wir in einer Tabelle sÃ¤mtliche Belegungen
durchprobieren.  Eine solche Tabelle heiÃt auch \emph{Wahrheits-Tafel}.
...
stehen.  Daher sind die Formeln, die zu diesen Spalten gehÃ¶ren, Ã¤quivalent.

\subsection{Testen der AllgemeingÃ¼ltigkeit in \textsc{SetlX}}
\noindent
 Die manuelle ÃberprÃ¼fung der Frage, ob eine gegebene Formel $f$ eine Tautologie ist, 
lÃ¤uft auf die Erstellung umfangreicher Wahrheitstafeln heraus.   Solche Wahrheitstafeln
...
obige Frage automatisch beantworten kÃ¶nnen.   Die Grundidee ist, dass wir die zu untersuchende
Formel fÃ¼r alle mÃ¶glichen Belegungen auswerten und Ã¼berprÃ¼fen, dass sich bei der Auswertung
jedesmal der Wert \texttt{true} ergibt.  Dazu mÃ¼ssen wir zunÃ¤chst einen Weg finden, alle
mÃ¶glichen Belegungen einer Formel zu berechnen.  Wir haben frÃ¼her schon gesehen, dass
Belegungen $\mathcal{I}$ zu Teilmengen $M$ der Menge der aussagenlogischen Variablen
$\mathcal{P}$ korrespondieren, denn fÃ¼r jedes $M \subseteq \mathcal{P}$ kÃ¶nnen wir eine
...
$
\\[0.2cm]
Um die aussagenlogische Belegung $\mathcal{I}$ in \textsc{SetlX} darstellen zu kÃ¶nnen,
...
$.
\\[0.2cm]
Dies lÃ¤sst sich noch zu
\\[0.2cm]
\hspace*{1.3cm}
$\mathcal{I} = \bigl\{ \langle p, p \el M\rangle \mid  p \in \mathcal{P} \bigr\}$
\\[0.2cm]
vereinfachen.  Mit dieser Idee kÃ¶nnen wir nun eine Prozedur implementieren, die fÃ¼r eine
...
\end{Verbatim}
\vspace*{-0.3cm}
  \caption{ÃberprÃ¼fung der AllgemeingÃ¼ltigkeit einer aussagenlogischen Formel.}
...
testet, ob die als Argument Ã¼bergebene
aussagenlogische Formel $f$ allgemeingÃ¼ltig ist.   
...
Wir diskutieren die Definition der Funktion $\textsl{tautology}$ nun Zeile fÃ¼r Zeile:
\begin{enumerate}
\item In Zeile 2 sammeln wir alle aussagenlogischen Variablen auf, die in der zu
      Ã¼berprÃ¼fenden Formel auftreten.  Die dazu benÃ¶tigte Prozedur \texttt{collectVars}
      ist in den Zeilen 11 -- 23 gezeigt.  Diese Prozedur ist durch Induktion Ã¼ber den
      ...
      Es ist klar, das bei der Berechnung von ${\cal I}(f)$ fÃ¼r eine Formel $f$
      und eine aussagenlogische Interpretation ${\cal I}$ nur die Werte von
      ${\cal I}(p)$ eine Rolle spielen, fÃ¼r die die Variable $p$ in $f$
      auftritt.  Zur Analyse von $f$ kÃ¶nnen wir uns also auf aussagenlogische 
...
\item In Zeile 4 berechnen wir die Menge aller aussagenlogischen
      Interpretationen Ã¼ber der Menge $\mathcal{P}$ der aussagenlogischen Variablen.  
      Wir berechnen fÃ¼r eine Menge $m$ von aussagenlogischen Variablen
      ...
      damit haben wir alle mÃ¶glichen Interpretationen der Variablen $p$ und $q$. 
\item In Zeile 5 testen wir, ob die Formel $f$ fÃ¼r alle mÃ¶glichen Interpretationen $i$
      aus der Menge $a$ aller Interpretationen wahr ist.  Ist dies der Fall,
      so geben wir die leere Menge als Ergebnis zurÃ¼ck.

      Falls es allerdings eine Belegungen $i$ in der Menge $a$ gibt, fÃ¼r die die
      Auswertung von $f$ den Wert \textsl{false} liefert, so bilden wir in Zeile 8 die
      Menge aller solcher Belegungen und wÃ¤hlen mit Hilfe der Funktion
      $\textsl{arb}$ eine beliebige Belegungen aus dieser Menge aus, die wir dann
      als Gegenbeispiel zurÃ¼ck geben.
\end{enumerate}

\subsection{Nachweis der AllgemeingÃ¼ltigkeit durch Ãquivalenz-Umformungen}
Wollen wir nachweisen, dass eine Formel eine Tautologie ist, kÃ¶nnen wir uns prinzipiell
...
Eine andere MÃ¶glichkeit nachzuweisen, dass eine Formel eine Tautologie ist, ergibt sich dadurch, dass wir
die Formel mit Hilfe der oben aufgefÃ¼hrten Ãquivalenzen \emph{vereinfachen}.  Wenn es gelingt, eine Formel $F$ unter Verwendung
dieser Ãquivalenzen zu $\verum$ zu vereinfachen, dann ist gezeigt, dass $F$ eine Tautologie ist.  
Wir demonstrieren das Verfahren zunÃ¤chst an einem Beispiel. 
...
Ãquivalenz-Umformungen einsehen kÃ¶nnen:\\[0.2cm]
\hspace*{1.3cm} 
$ 
\begin{array}[c]{lcr}
 ...
 \leftrightarrow & (p \wedge \neg q) \vee ((\neg p \wedge \neg q) \vee q)              & \quad(\mbox{DistributivitÃ¤t}) \\
 \leftrightarrow & (p \wedge \neg q) \vee ((\neg p \vee q) \wedge (\neg q \vee q))     & \quad(\mbox{Tertium-non-Datur})\\               
 \leftrightarrow & (p \wedge \neg q) \vee ((\neg p \vee q) \wedge \verum)                & \quad(\mbox{Neutrales Element})\\                 
 \leftrightarrow & (p \wedge \neg q) \vee (\neg p \vee q)                              & \quad(\mbox{DistributivitÃ¤t})\\                   
 \leftrightarrow & (p \vee (\neg p \vee q)) \wedge (\neg q \vee (\neg p \vee q))       & \quad(\mbox{AssoziativitÃ¤t}) \\                   
 ...
 \leftrightarrow & \neg q \vee (\neg p \vee q)                                         & \quad(\mbox{AssoziativitÃ¤t})\\                    
 \leftrightarrow & (\neg q \vee \neg p) \vee q                                         & \quad(\mbox{KommutativitÃ¤t})\\                    
 \leftrightarrow & (\neg p \vee \neg q) \vee q                                         & \quad(\mbox{AssoziativitÃ¤t})\\                    
 \leftrightarrow & \neg p \vee (\neg q  \vee q)                                        & \quad(\mbox{Tertium-non-Datur})\\               
...
Die Umformungen in dem obigen Beweis sind nach einem bestimmten System durchgefÃ¼hrt worden.  Um dieses System
prÃ¤zise formulieren zu kÃ¶nnen, benÃ¶tigen wir noch einige Definitionen.

\begin{Definition}[Literal]
  Eine aussagenlogische Formel $f$ heiÃt \emph{Literal} g.d.w. einer der folgenden FÃ¤lle vorliegt:
  \begin{enumerate}
...
SpÃ¤ter werden wird noch den Begriff des \emph{Komplements} eines Literals benÃ¶tigen.
...
\end{enumerate}
Wir sehen, dass das Komplement $\komplement{\,l\,}$ eines Literals $l$ Ã¤quivalent zur
...
\begin{Definition}[Klausel]
  ...
  hat, wobei $l_i$ fÃ¼r alle $i=1,\cdots,r$ ein Literal ist.  Eine Klausel ist also eine
...
der Literale in der Disjunktion.  Dies ist mÃ¶glich aufgrund der AssoziativitÃ¤t, KommutativitÃ¤t und
Idempotenz des Junktors ``$\vee$''.  FÃ¼r die Klausel $l_1 \vee \cdots \vee l_r$ schreiben
wir also in Zukunft auch 
\\[0.2cm]
\hspace*{1.3cm} $\{ l_1, \cdots, l_r \}$.
\\[0.2cm]
Das folgende Beispiel illustriert die NÃ¼tzlichkeit der Mengen-Schreibweise von Klauseln.
Wir betrachten die beiden Klauseln
\\[0.2cm]
\hspace*{1.3cm}
$p \vee q \vee \neg r \vee p$ \quad und \quad $\neg r \vee q \vee \neg r \vee p$. 
\\[0.2cm]
Die beiden Klauseln sind zwar Ã¤quivalent, aber die Formeln sind verschieden.
ÃberfÃ¼hren wir die beiden Klauseln in Mengen-Schreibweise, so erhalten wir
\\[0.2cm]
\hspace*{1.3cm}
$\{p, q, \neg r \}$ \quad und \quad $\{ \neg r, q, p \}$. 
\\[0.2cm]
In einer Menge kommt jedes Element hÃ¶chstens einmal vor und die Reihenfolge, in der die
...
abstrahieren, implementiert die Mengen-Schreibweise die AssoziativitÃ¤t, KommutativitÃ¤t und
...
\\[0.2cm]
Dies zeigt, dass wir das Element $\falsum$ in einer Klausel getrost weglassen kÃ¶nnen.
Betrachten wir die letzten Ãquivalenz fÃ¼r den Fall, dass $r=0$ ist, so haben wir
\\[0.2cm]
\hspace*{1.3cm}
...
\begin{Definition}
  Eine Klausel $k$ ist \emph{trivial}, wenn einer der beiden folgenden FÃ¤lle vorliegt:
  \begin{enumerate}
  \item $\verum \in k$.
  \item Es existiert $p \in \mathcal{P}$ mit $p \in k$ und $\neg p \in k$.

        In diesem Fall bezeichnen wir $p$ und $\neg p$ als \emph{komplementÃ¤re Literale}.
        \eox
\end{enumerate}
\end{Definition}
...
\end{Satz}
\textbf{Beweis}:  Wir nehmen zunÃ¤chst an, dass die Klausel $k$ trivial ist.
Falls nun $\verum \el k$ ist, dann gilt wegen der GÃ¼ltigkeit der Ãquivalenz 
...
Wir nehmen nun an, dass die Klausel $k$ eine Tautologie ist.  Wir fÃ¼hren den Beweis
indirekt und nehmen an, dass $k$ nicht trivial ist.  Damit gilt  $\verum \notin k$ und
$k$ kann auch keine komplementÃ¤ren Literale enthalten.  Damit hat $k$ dann die Form
\\[0.2cm]
\hspace*{1.3cm} 
$k = \{ \neg p_1, \cdots, \neg p_m, q_1, \cdots, q_n \}$ \quad mit $p_i
\not= q_j$ fÃ¼r alle $i \in \{ 1,\cdots,m\}$ und $j \in \{1, \cdots, n\}$.
\\[0.2cm]
Dann kÃ¶nnten wir eine Interpretation $\mathcal{I}$ wie folgt definieren:
\begin{enumerate}
\item $\mathcal{I}(p_i) = \mathtt{true}$ fÃ¼r alle $i = 1, \cdots, m$ und
\item $\mathcal{I}(q_j) = \mathtt{false}$ fÃ¼r alle $j = 1, \cdots, n$,
\end{enumerate}
Mit dieser Interpretation wÃ¼rde offenbar $\mathcal{I}(k) = \mathtt{false}$ gelten und damit kÃ¶nnte $k$ keine
...
\begin{Definition}[Konjunktive Normalform]  
  ...
  wobei die $k_i$ fÃ¼r alle $i=1,\cdots,n$ Klauseln sind. \eox
\end{Definition}
...
\begin{Korollar} \label{korollar:knf}
  ...
  \hspace*{1.3cm} $\models f$ \quad genau dann, wenn \quad $\models k_i$ \quad fÃ¼r alle $i=1,\cdots,n$. \qed
\end{Korollar}

Damit kÃ¶nnen wir fÃ¼r eine Formel $f = k_1 \wedge \cdots \wedge k_n$ in konjunktiver
...
Da fÃ¼r die Konjunktion genau wie fÃ¼r die Disjunktion Assoziativit-Gesetz, Kommutativ-Gesetz und
Idempotenz-Gesetz gilt, ist es zweckmÃ¤Ãig, auch fÃ¼r Formeln in konjunktiver Normalform eine
Mengen-Schreibweise einzufÃ¼hren.  Ist also die Formel
\\[0.2cm]
\hspace*{1.3cm} $f = k_1 \wedge \cdots \wedge k_n$
\\[0.2cm]
in konjunktiver Normalform, so reprÃ¤sentieren wir diese
...
$\bigl\{ \{p, q, \neg r \},\, \{ p, \neg q, \neg r \} \bigr\}$.
\\[0.2cm]
Wir stellen nun ein Verfahren vor, mit dem sich jede Formel in KNF transformieren lÃ¤sst.  Nach
dem oben Gesagten kÃ¶nnen wir dann leicht entscheiden, ob $f$ eine Tautologie ist.
\begin{enumerate}
...
\item Stehen in der Formel jetzt ``$\vee$''-Junktoren Ã¼ber ``$\wedge$''-Junktoren, so kÃ¶nnen wir durch
...
\item In einem letzten Schritt Ã¼berfÃ¼hren wir die Formel nun in Mengen-Schreibweise, indem
      wir zunÃ¤chst die Disjunktionen aller Literale als Mengen zusammenfassen und anschlieÃend
...
$f \vee (g \wedge h) \leftrightarrow (f \vee g) \wedge (f \vee h)$ zweimal auftritt, wÃ¤hrend sie
links nur einmal      vorkommt. 

Wir demonstrieren das Verfahren am Beispiel der Formel\\[0.2cm]
\hspace*{1.3cm} $(p \rightarrow q) \rightarrow (\neg p \rightarrow \neg q)$.
\begin{enumerate}
\item Da die Formel den Junktor ``$\leftrightarrow$'' nicht enthÃ¤lt,
...
\item Die ÃberfÃ¼hrung in die Mengen-Schreibweise ergibt zunÃ¤chst als Klauseln die beiden Mengen \\[0.2cm]
      \hspace*{1.3cm} $\{p, p, \neg q\}$ \quad und \quad $\{\neg q,  p,  \neg q\}$. \\[0.2cm]
      Da die Reihenfolge der Elemente einer Menge aber unwichtig ist und auÃerdem eine Menge
      jedes Element nur einmal enthÃ¤lt, stellen wir fest, dass diese beiden Klauseln gleich sind.
      Fassen wir jetzt die Klauseln noch in einer Menge zusammen, so erhalten wir \\[0.2cm]
      \hspace*{1.3cm} $\bigl\{ \{p, \neg q\} \bigr\}$. \\[0.2cm]
      Beachten Sie, dass sich die Formel durch die ÃberfÃ¼hrung in 
      Mengen-Schreibweise noch einmal deutlich vereinfacht hat.
\end{enumerate}
Damit ist die Formel in KNF Ã¼berfÃ¼hrt.
...
Formel $f$ in konjunktive Normalform Ã¼berfÃ¼hren lÃ¤sst.  Wir beginnen mit einer
Prozedur 
\\[0.2cm]
\hspace*{1.3cm}
$\texttt{elimGdw}: \mathcal{F} \rightarrow \mathcal{F}$
\\[0.2cm]
die die Aufgabe hat, eine vorgegebene aussagenlogische Formel $f$ in eine Ã¤quivalente Formel
umzuformen, die den Junktor ``$\leftrightarrow$'' nicht mehr enthÃ¤lt.  Die Funktion
$\texttt{elimGdw}(f)$ wird durch Induktion Ã¼ber den Aufbau der aussagenlogischen Formel $f$ definiert.
Dazu stellen wir zunÃ¤chst rekursive Gleichungen auf,
...
\item Wenn $f$ eine
      ...
      \\[0.2cm]
      \hspace*{1.3cm}
      $\mathtt{elimGdw}(p) = p$ \quad fÃ¼r alle $p \in \mathcal{P}$.
...
Als nÃ¤chstes betrachten wir die Prozedur zur Elimination des Junktors ``$\rightarrow$''. 
Abbildung
...
benutzen.  AuÃerdem kÃ¶nnen wir schon voraussetzen, dass der Junktor ``$\leftrightarrow$''
bereits vorher eliminiert wurde.  Dadurch entfÃ¤llt ein Fall.
...
Als nÃ¤chstes zeigen wir die Routinen zur Berechnung der Negations-Normalform.
Abbildung
...
\item Falls die Formel $f$, die wir in KNF transformieren wollen, die Form \\[0.2cm]
      ...
      Negations-Normalform.  Damit kÃ¶nnen wir $f$ in eine Klausel transformieren, 
      indem wir $\{\neg g\}$, also $\{f\}$ schreiben.  
      Da eine KNF eine Menge von Klauseln ist, ist dann 
      $\bigl\{\{f\}\bigr\}$ das Ergebnis, das wir in Zeile 3 zurÃ¼ck geben.
\item Falls $f= f_1 \wedge f_2$ ist, transformieren wir zunÃ¤chst $f_1$ und $f_2$ in KNF.
...
\item Falls $f= f_1 \vee f_2$ ist, transformieren wir zunÃ¤chst $f_1$ und $f_2$ in KNF.
      Dabei erhalten wir \\[0.2cm]
      \hspace*{1.3cm} 
      $\mathtt{knf}(f_1) = \{ h_1, \cdots, h_m \}$ \quad und \quad
      $\mathtt{knf}(f_2) = \{ k_1, \cdots, k_n \}$. \\[0.2cm]
      ...
      \end{array}
      $$
      BerÃ¼cksichtigen wir noch, dass Klauseln in der Mengen-Schreibweise als Mengen von
      Literalen aufgefasst werden, die implizit disjunktiv verknÃ¼pft werden, so kÃ¶nnen wir
      fÃ¼r $h_i \vee k_j$ auch $h_i \cup k_j$ schreiben.  
...
\item Falls die Formel $f$, die wir in KNF transformieren wollen, eine Aussage-Variable
      ist, so transformieren wir $f$ zunÃ¤chst in eine Klausel. Das liefert $\{f\}$.  
      Da eine KNF eine Menge von Klauseln ist, ist die KNF dann $\bigl\{\{f\}\bigr\}$.
      Dieses Ergebnis geben wir in Zeile 6 zurÃ¼ck.
\end{enumerate}
...
zunÃ¤chst die Junktoren ``$\leftrightarrow$'' und ``$\rightarrow$'' und bringt die Formel
...
Mengen-Schreibweise Ã¼berfÃ¼hrt wird.  Im letzten Schritt entfernt die Funktion
...
in $k$ der Menge $k$ auftritt.  Daher berechnet die Funktion \texttt{isTrivial} fÃ¼r eine
Klausel $c$ zunÃ¤chst die Menge
\\[0.2cm]
\hspace*{1.3cm}
\verb'{ p in c | fct(p) == "^variable" }'
\\[0.2cm]
aller Variablen, die in der Klausel $c$ auftreten.  AnschlieÃend wird die Menge
\\[0.2cm]
\hspace*{1.3cm}
\verb'{ args(l)[1] : l in c | fct(l) == "^not" }'
\\[0.2cm]
berechnet.  Diese Menge enthÃ¤lt alle die aussagenlogischen Variablen $p$, fÃ¼r die $\neg p$
...
\texttt{\symbol{94}not} und diese Liste enthÃ¤lt als einziges Element gerade die
aussagenlogische Variable $p$, die wir daher durch den Ausdruck
\\[0.2cm]
\hspace*{1.3cm}
\texttt{args(l)[1]}
\\[0.2cm]
aus dem Literal $l$ extrahieren kÃ¶nnen.  Falls nun die beiden oben berechneten Mengen ein
gemeinsames Element haben, so ist die Klausel $c$ trivial.  Dies wird dadurch geprÃ¼ft,
dass der Schnitt der beiden Mengen berechnet wird.
Das vollstÃ¤ndige Programm zur Berechnung der konjunktiven Normalform finden Sie als die Datei
...
kÃ¶nnen wir uns fragen, ob  die  Formel $g$ aus $f_1$, $\cdots$, $f_n$ \emph{folgt}, ob
also 
\[ \models f_1 \wedge \cdots \wedge f_n \rightarrow g \]
gilt.
Es gibt verschiedene MÃ¶glichkeiten, diese Frage zu beantworten.  Ein Verfahren kennen wir
schon: ZunÃ¤chst Ã¼berfÃ¼hren wir die Formel  $f_1 \wedge \cdots \wedge f_n \rightarrow g$ in
...
\\[0.2cm] 
Ã¤quivalent ist.  Diese Formel ist nun genau dann eine Tautologie, wenn
...
\]
und erhalten nach mÃ¼hsamer Rechnung
\[
   (p \vee \neg p \vee r \vee \neg r) \wedge (\neg q \vee \neg p \vee r \vee \neg r) \wedge
   (\neg q \vee \neg p \vee q \vee r) \wedge (p \vee \neg p \vee q \vee r). 
\]
Zwar kÃ¶nnen wir jetzt sehen, dass die Formel $h$ eine Tautologie ist, aber angesichts der
Tatsache, dass wir mit bloÃem Auge sehen, dass  $p \rightarrow r$ aus den Formeln $p \rightarrow q$ und
$q \rightarrow r$ folgt, ist die Rechnung  doch  sehr mÃ¼hsam.

Wir stellen daher nun eine weiteres Verfahren vor, mit dessen Hilfe wir entscheiden
kÃ¶nnen, ob eine Formel aus einer gegebenen Menge von Formeln folgt.  Die Idee bei diesem Verfahren
...
\begin{Definition}[Schluss-Regel]
    ...
    \emph{PrÃ¤missen}, die Formel $k$ heiÃt die \emph{Konklusion} der Schluss-Regel.
    Ist das Paar 
...
\noindent
\textbf{Beispiele} fÃ¼r Schluss-Regeln: 
\\[0.2cm]
\hspace*{1.3cm}            
...
\noindent
Die Definition der Schluss-Regel schrÃ¤nkt zunÃ¤chst die Formeln, die als PrÃ¤missen
bzw.~Konklusion verwendet werden kÃ¶nnen, nicht weiter ein.  Es ist aber sicher nicht
sinnvoll, beliebige Schluss-Regeln zuzulassen.  Wollen wir Schluss-Regeln in Beweisen
verwenden, so sollten die Schluss-Regeln in dem in der folgenden Definition erklÃ¤rten
...
Schluss-Regeln korrekt sind, wÃ¤hrend die als  ``\emph{Modus Tollendo Tollens}'' bezeichnete
Schluss-Regel nicht korrekt ist.

Im Folgenden gehen wir davon aus, dass alle Formeln Klauseln sind.  Einerseits ist dies
keine echte EinschrÃ¤nkung, denn wir kÃ¶nnen ja jede Formel in eine Ã¤quivalente Menge von
...
sowohl die PrÃ¤missen als auch die Konklusion Klauseln sind.
...
\noindent
Die Schnitt-Regel ist sehr allgemein.  Setzen wir in der obigen Definition fÃ¼r $k_1 =
\{\}$ und  $k_2 = \{q\}$ 
...
Wenn wir jetzt noch berÃ¼cksichtigen, dass die Formel $\neg p \vee q$ Ã¤quivalent ist zu der
...
\end{Satz}
\textbf{Beweis}:  Wir mÃ¼ssen zeigen, dass \\[0.2cm]
\hspace*{1.3cm} $\models (k_1 \vee p) \wedge (\neg p \vee k_2) \rightarrow k_1 \vee k_2$ \\[0.2cm]
gilt.  Dazu Ã¼berfÃ¼hren wir die obige Formel in konjunktive Normalform:
$$
\begin{array}{ll}
  & (k_1 \vee p) \wedge (\neg p \vee k_2) \rightarrow k_1 \vee k_2  \\[0.2cm]
\leftrightarrow  & 
...
\begin{Definition}[$\vdash$]
    ...
    \item Sind $k_1 \cup \{p\}$ und $\{ \neg p \} \cup k_2$ Klauseln, die aus $M$
          hergeleitet werden kÃ¶nnen, so kann mit der Schnitt-Regel auch die Klausel $k_1 \cup k_2$ aus $M$
...
\noindent
\textbf{Beweis}:  Der Beweis verlÃ¤uft durch eine Induktion nach der Definition der Relation $\vdash$. 
\begin{enumerate}
...
\noindent
Die Umkehrung dieses Satzes gilt leider nur in abgeschwÃ¤chter Form und zwar dann, wenn $k$
die leere Klausel ist, also im Fall $k = \falsum$.
\begin{Satz}[Widerlegungs-VollstÃ¤ndigkeit] \label{widerlegungs-vollstaendig}
...
\subsection{Beweis der Widerlegungs-VollstÃ¤ndigkeit}
Der Beweis der Widerlegungs-VollstÃ¤ndigkeit der Aussagenlogik benÃ¶tigt den  Begriff der
\emph{ErfÃ¼llbarkeit}, den wir jetzt formal einfÃ¼hren. 

\begin{Definition}[ErfÃ¼llbarkeit]
  Es sei $M$ eine Menge von aussagenlogischen Formeln.
  Falls es eine aussagenlogische Interpretation $\I$ gibt, die alle Formeln aus $M$ erfÃ¼llt, nennen
  wir $M$ \emph{erfÃ¼llbar}. 

  Wir sagen, dass $M$ \emph{unerfÃ¼llbar} ist und schreiben 
  \\[0.2cm]
  \hspace*{1.3cm}
  $M \models \falsum$
  \\[0.2cm]
  wenn es keine aussagenlogische Interpretation $\I$ gibt, die alle Formel aus $M$ erfÃ¼llt.
...
\remark 
Ist $M = \{ k_1, \cdots, k_n \}$ eine Menge von Klauseln, so kÃ¶nnen Sie sich leicht Ã¼berlegen, dass
$M$ genau dann nicht erfÃ¼llbar ist, wenn
\\[0.2cm]
\hspace*{1.3cm}
...
Wir fÃ¼hren den Bewies der Widerlegungs-VollstÃ¤ndigkeit mit Hilfe eines Programms, das in den
...
alle Klauseln abzuleiten, die Ã¼berhaupt aus $M$ ableitbar sind.  Wenn wir dabei auch
die leere Klausel ableiten, dann ist $M$ aufgrund der Korrektheit der Schnitt-Regel offenbar
unerfÃ¼llbar.  Falls es uns aber nich  t gelingt, die leere Klausel aus $M$ abzuleiten, dann konstruieren wir
aus der Menge aller Klauseln, die wir aus $M$ abgeleitet haben, eine aussagenlogische Interpretation
$\I$, die alle Klauseln aus $M$ erfÃ¼llt.

Wir diskutieren zunÃ¤chst die Hilfsprozeduren, die in Abbildung \ref{fig:completeness.stlx-1} gezeigt
sind. 
\begin{enumerate}
\item Die Funktion \texttt{complement} erhÃ¤lt als Argument ein Literal $l$ und berechnet das
...
\item Die Funktion \texttt{extractVar} extrahiert die aussagenlogische Variable aus einem Literal $l$.
      Die Implementierung verlÃ¤uft analog zur Implementierung der Funktion \texttt{complement} Ã¼ber eine
      Fallunterscheidung, bei der wir berÃ¼cksichtigen, dass $l$ entweder die Form $\neg p$ oder die
      Form $p$ hat, wobei $p$ eine aussagenlogische Variable ist.
\item Die Funktion \texttt{collectVars} erhÃ¤lt als Argument eine Menge $m$ von Klauseln, wobei die
      ...
      wir zunÃ¤chst Ã¼ber die Klauseln $c$ der Menge $m$ und dann fÃ¼r jede Klausel $c$ Ã¼ber die in $c$
...
\item Die Funktion \texttt{cutRule} erhÃ¤lt als Argumente zwei Klauseln $c_1$ und $c_2$ und berechnet
      alle die Klauseln, die mit Hilfe der Schnittregel aus $c_1$ und $c_2$ gefolgert werden
      kÃ¶nnen.  Beispielsweise kÃ¶nnen wir aus den beiden Klauseln
      \\[0.2cm]
      \hspace*{1.3cm}
      $\{ p, q \}$ \quad und \quad $\{ \neg p, \neg q \}$ 
      \\[0.2cm]
...
Abbildung \ref{fig:completeness.stlx-2} zeigt die Funktion \texttt{saturate}.  Die Funktion erhÃ¤lt
...
kÃ¶nnen.  Genauer sagen wir, dass die Menge $S$ der Klauseln, die von der Funktion \texttt{saturate}
zurÃ¼ck gegeben wird, unter Anwendung der Schnitt-Regel \emph{saturiert} ist, was formal wie folgt
definiert ist:
\begin{enumerate}
\item Falls $S$ die leere Klausel $\{\}$ enthÃ¤lt, dann ist $S$ saturiert.
\item Andernfalls muss \texttt{clauses} eine Teilmenge von $S$ sein und es muss zusÃ¤tzlich Folgendes
...
\end{enumerate}
Wir erlÃ¤utern nun die Implementierung der Funktion \texttt{saturate}.
\begin{enumerate}
\item Die \texttt{while}-Schleife, die in Zeile 22 beginnt, hat die Aufgabe, die Schnitt-Regel
      solange wie mÃ¶glich anzuwenden um mit Hilfe der Schnitt-Regel neue Klauseln aus den gegebenen
      Klauseln herzuleiten.  Da die Bedingung dieser Schleife den Wert \texttt{true} hat, kann diese
      Schleife nur durch die AusfÃ¼hrung eines \texttt{return}-Befehls abgebrochen werden.
\item In Zeile 23 wird die Menge \texttt{derived} dadurch berechnet, dass alle Klauseln berechnet
      ...
      gefolgert werden kÃ¶nnen.
\item Falls die Menge \texttt{derived} die leere Klausel enthÃ¤lt, dann ist die Menge
      \texttt{clauses} widersprÃ¼chlich und die Funktion \texttt{saturate} gibt als Ergebnis die
      Menge $\bigl\{ \{\} \bigr\}$ zurÃ¼ck.
\item Andernfalls ziehen wir in Zeile 27 von der Menge \texttt{derived} zunÃ¤chst die Klauseln ab, die wir schon
      vorher hatten, denn es geht uns darum festzustellen, ob wir im letzten Schritt tatsÃ¤chlich
...
\item Falls wir nun in Zeile 28 feststellen, dass wir keine neuen Klauseln hergeleitet haben,
      dann ist die Menge \texttt{clauses} saturiert und wir geben diese Menge in Zeile 29 zurÃ¼ck.
\item Andernfalls fÃ¼gen wir in Zeile 31 die Klauseln, die wir neu gefunden haben, zu der Menge
      \texttt{clauses} hinzu und setzen die \texttt{while}-Schleife fort.
\end{enumerate}
An dieser Stelle mÃ¼ssen wir uns Ã¼berlegen, dass die \texttt{while}-Schleife tatsÃ¤chlich irgendwann
abbricht.  Das hat zwei GrÃ¼nde:  
\begin{enumerate}
\item In jeder Iteration der Schleife wird die Anzahl der Elemente der Menge \texttt{clauses}
      mindestens um Eins erhÃ¶ht, denn wir wissen ja, dass die Menge \texttt{derived}, die wir zu
      \texttt{clauses} hinzufÃ¼gen, einerseits nicht leer ist und andererseits auch nur solche
      Klauseln enthÃ¤lt, die nicht bereits in \texttt{clauses} auftreten.
\item Die Menge \texttt{clauses}, mit der wir ursprÃ¼nglich starten, enthÃ¤lt eine bestimmte Anzahl $n$
      ...
      \texttt{clauses} auftreten, immer gleich.  Damit ist natÃ¼rlich auch die Anzahl der Literale,
      die in \texttt{clauses} auftreten, beschrÃ¤nkt: Wenn es nur $n$ aussagenlogische Variablen gibt,
      dann kann es auch hÃ¶chstens $2 \cdot n$ Literale geben.  Jede Klausel aus \texttt{clauses} ist
      aber eine Teilmenge der Menge aller Literale.  Da eine Menge mit $k$ Elementen insgesamt $2^k$
      Teilmengen hat, gibt es hÃ¶chstens $2^{2 \cdot n}$ verschiedene Klauseln, die in
      \texttt{clauses} auftreten kÃ¶nnen.  
\end{enumerate}
Aus den beiden oben angegebenen GrÃ¼nden kÃ¶nnen wir schlieÃen, dass die \texttt{while}-Schleife in
Zeile 22 nach spÃ¤testens $2^{2 \cdot n}$ Iterationen abgebrochen wird. 
...
Als nÃ¤chstes diskutieren wir die Implementierung der Funktion \texttt{findValuation}, die in
Abbildung \ref{fig:completeness.stlx-3} gezeigt ist.  Diese Funktion erhÃ¤lt als Eingabe eine Menge
\texttt{clauses} von Klauseln.  Falls diese Menge widersprÃ¼chlich ist, soll die Funktion
das Ergebnis \texttt{false} zurÃ¼ck geben.  Andernfalls soll eine aussagenlogische Belegung $\I$ berechnet werden,
unter der alle Klauseln aus der Menge \texttt{clauses} erfÃ¼llt sind.  Im Detail arbeitet die
Funktion \texttt{findValuation} wie folgt.
\begin{enumerate}
\item ZunÃ¤chst berechnen wir in Zeile 35 die Menge aller aussagenlogischen Variablen, die in
      der Menge \texttt{clauses} auftreten.  Wir benÃ¶tigen diese Menge, denn wir mÃ¼ssen diese
...
\item In Zeile 36 saturieren wir die Menge \texttt{clauses} und berechnen alle Klauseln, die aus der
      ursprÃ¼nglich gegebenen Menge von Klauseln mit Hilfe der Schnitt-Regel abgeleitet werden
      kÃ¶nnen.  Hier kÃ¶nnen zwei FÃ¤lle auftreten:
      \begin{enumerate}
      \item Falls die leere Klausel abgeleitet werden kann, dann ist die ursprÃ¼nglich gegebene Menge
            von Klauseln widersprÃ¼chlich und wir geben als Ergebnis an Stelle einer Belegung den
            Wert \texttt{false} zurÃ¼ck, denn eine widersprÃ¼chliche Menge von Klauseln ist sicher
            nicht erfÃ¼llbar.
      \item Andernfalls berechnen wir nun eine aussagenlogische Belegung, unter der alle Klauseln aus
            \texttt{clauses} wahr werden.  Zu diesem Zweck berechnen wir zunÃ¤chst eine Menge von
            ...
            Zeile 48 die Menge zurÃ¼ck, bei der wir jedes Literal $l$ aus der Menge \texttt{literals}
            durch sein Komplement $\komplement{\,l\,}$ ersetzen.  

            Im engeren Sinne ist diese Menge $K$
            keine aussagenlogische Belegung, aber wir kÃ¶nnen aus dieser Menge leicht eine
            ...
            $
      \end{enumerate}
\item Die Berechnung der Menge \texttt{literals} erfolgt nun Ã¼ber eine \texttt{for}-Schleife.
      Dabei ist die Idee, dass wir fÃ¼r eine aussagenlogische Variable $p$ genau dann das Literal
      $\neg p$ zu der Menge \texttt{literals} hinzufÃ¼gen, wenn die Belegung $\I$ die Variable $p$
      auf \texttt{true} abbilden muss um die Klauseln zu erfÃ¼llen.  

      Die Bedingung dafÃ¼r ist wie folgt: Angenommen, wir haben bereits Werte fÃ¼r die Variablen
      ...
      \\[0.2cm]
      \hspace*{1.3cm}
      $\I(p_i) = \mathtt{false}$ \quad fÃ¼r alle $i \in \{1,\cdots,n\}$
      \\[0.2cm]
      und damit ist $p$ das einzige Literal in der Klausel $c$, das wir mit Hilfe der Belegung $\I$ Ã¼berhaupt noch wahr machen
      kÃ¶nnen.  In diesem Fall fÃ¼gen wir also das Literal $\neg p$ in die Menge \texttt{literals} ein.

      Falls wir keine solche Klausel $c$ finden, dann setzen wir $\mathtt{I}(p) := \mathtt{false}$
      und fÃ¼gen daher das Literal $p$ in die Menge \texttt{literals} ein.
\end{enumerate}
Der entscheidende Punkt ist nun der Nachweis, dass die Funktion \texttt{findValuation} in dem Falle,
dass in Zeile 38 nicht der Wert \texttt{false} zurÃ¼ck gegeben wird, eine aussagenlogische Belegung $\I$ berechnet, bei der
...
\\[0.2cm]
und zeigen durch Induktion nach $n$, dass nach $n$ DurchlÃ¤ufen der Schleife fÃ¼r jede Klausel $d \in \mathtt{clauses}$, in der nur
...
\item[I.A.:] $n = 0$.
             ...
             Die einzige Klausel, in der Ã¼berhaupt keine Variablen vorkommen, ist die leere Klausel.
             Da wir aber vorausgesetzt haben, dass \texttt{clauses} die leere Klausel nicht enthÃ¤lt,
...
\item[I.S.:] $n \mapsto n+1$.
             ...
             \item Es gibt ein $i \in \{1,\cdots,n\}$, so dass $l_i \in d$ und  $\I(l_i) =
               \mathtt{true}$ ist.  
                   ...
                   $\I(d) = \mathtt{true}$ unabhÃ¤ngig davon, ob $\I(l_{n+1})$ den Wert \texttt{true} oder
                   \texttt{false} hat.
             \item FÃ¼r alle $i \in \{1,\cdots,n\}$ mit $l_i \in d$ gilt $\I(l_i) = \mathtt{false}$ und es gilt $l_{n+1} = p_{n+1}$.
                   ...
                   \\[0.2cm]
                   und daher wird in Zeile 43 der Funktion \texttt{findValuation} das Literal $\neg p_{n+1}$ 
                   zu der Menge Literals hinzugefÃ¼gt.  Nach Definition der Belegung $\I$, 
                   die von der Funktion \texttt{findValuation} zurÃ¼ck gegeben wird, heiÃt dies
                   gerade, dass 
                   \\[0.2cm]
                   \hspace*{1.3cm}
                   $\I(p_{n+1}) = \mathtt{true}$
                   \\[0.2cm]
                   ist und dann gilt natÃ¼rlich auch $\I(d) = \mathtt{true}$.
             \item FÃ¼r alle $i \in \{1,\cdots,n\}$ mit $l_i \in d$ gilt $\I(l_i) = \mathtt{false}$ und es gilt $l_{n+1} = \neg p_{n+1}$.
                   ...
                   \item Es gibt eine Klausel $c$ in der Menge \texttt{clauses}, so dass
                         ...
                         in diesem Fall wÃ¼rde ja das Literal $\neg p_{n+1}$ zur Menge
                         \texttt{literals} hinzugefÃ¼gt und damit wÃ¤re zunÃ¤chst $\I(p_{n+1}) = \mathtt{true}$ 
                         und damit $\I(\neg p_{n+1}) = \mathtt{false}$, woraus insgesamt 
                         $\I(d) = \mathtt{false}$ folgern wÃ¼rde.  In diesem Fall wÃ¼rden aber
                         ...
                         $c = c' \cup \{p_{n+1}\}$, \quad $d = d' \cup \{ \neg p_{n+1} \}$, 
                         \\[0.2cm]
                         gefolgert werden kann.  Das heiÃt dann aber, dass die Klausel $c' \cup d'$ ein
                         Element der Menge \texttt{clauses} sein muss, denn die Menge
                         \texttt{clauses} ist ja saturiert!  Da die Klausel $c' \cup d'$ auÃerdem
                   ...
                   \item Es gibt \underline{keine} Klausel $c$ in der Menge \texttt{clauses}, so dass
                         ...
                         hinzugefÃ¼gt und damit gilt zunÃ¤chst $\I(p_{n+1}) = \mathtt{false}$ und folglich
                         $\I(\neg p_{n+1}) = \mathtt{true}$, woraus schlieÃlich $\I(d) = \mathtt{true}$ folgt.
                   \end{enumerate}
...
\noindent
\textbf{Beweis der Widerlegungs-VollstÃ¤ndigkeit der Schnitt-Regel:}
Wir haben nun alles Material zusammen um zeigen zu kÃ¶nnen, dass die Schnitt-Regel
widerlegungs-vollstÃ¤ndig ist.  Wir nehmen also an, dass $M$ eine endliche Menge von Klauseln ist,  die nicht
erfÃ¼llbar ist, was wir als
\\[0.2cm]
\hspace*{1.3cm}
$M \models \falsum$ 
\\[0.2cm]
schreiben.  Wir rufen die Funktion \texttt{findValuation} mit dieser Menge $M$ als Argument auf.
Jetzt gibt es zwei MÃ¶glichkeiten:
\begin{enumerate}
\item Fall: Die Funktion \texttt{findValuation} liefert als Ergebnis \texttt{false}.  Nach
      ...
      Schnitt-Regel ableiten lÃ¤sst.  Dann haben wir also
      \\[0.2cm]
      \hspace*{1.3cm}
      $M \vdash \{\}$,
      \\[0.2cm]
...
\item Fall: Die Funktion \texttt{findValuation} liefert als Ergebnis eine aussagenlogische Belegung
      $\I$.  Bei der Diskussion der Funktion \texttt{findValuation} haben wir gezeigt, dass fÃ¼r
      ...
      Menge $M$ erfÃ¼llbar ist.  Dies steht im Widerspruch zu $M \models \falsum$ und damit kann der
      zweite Fall gar nicht auftreten. $\lightning$
\end{enumerate}
Folglich liefert die Funktion \texttt{findValuation} fÃ¼r eine unerfÃ¼llbare Menge von Klauseln immer
das Ergebnis \texttt{false}, was impliziert, dass $M \vdash \{\}$ gilt.  \qed

\section{Das Verfahren von Davis und Putnam}
In der Praxis stellt sich oft die Aufgabe, fÃ¼r eine gegebene Menge von Klauseln $K$ eine
Belegung $\I$ der Variablen zu berechnen, so dass 
\\[0.2cm]
\hspace*{1.3cm} $\mathtt{eval}(k,\I) = \mathtt{true}$ \quad fÃ¼r alle $k\in K$ \\[0.2cm]
gilt.  In diesem Fall sagen wir auch, dass die Belegung $\I$ eine \emph{LÃ¶sung} der
 Klausel-Menge $K$ ist.  Im letzten Abschnitt haben wir bereits die Prozedur \texttt{findValuation}
 kennengelernt, mit der wir eine solche Belegung berechnen kÃ¶nnten.
Bedauerlicherweise ist diese Prozedur fÃ¼r eine praktische Anwendung nicht effizient genug.
Wir werden daher in diesem Abschnitt ein Verfahren vorstellen, mit dem die Berechnung einer LÃ¶sung
einer aussagenlogischen Klausel-Menge auch in der Praxis mÃ¶glich ist.
Dieses Verfahren geht auf Davis und Putnam
\cite{davis62} zurÃ¼ck.  Verfeinerungen dieses Verfahrens werden beispielsweise
eingesetzt, um die Korrektheit digitaler elektronischer Schaltungen nachzuweisen.  

Um das Verfahren zu motivieren Ã¼berlegen wir zunÃ¤chst, bei welcher Form der Klausel-Menge $K$
unmittelbar klar ist, ob es eine Belegung gibt, die $K$ lÃ¶st und wie diese Belegung
...
$p \wedge \neg q \wedge r \wedge \neg s \wedge \neg t$.
\\[0.2cm]
Daher ist $K_1$ lÃ¶sbar und die Belegung  \\[0.2cm]
\hspace*{1.3cm} 
$\I = \bigl\{\; \pair(p, \mathtt{true}),\; \pair(q, \mathtt{false}),\;\pair(r, \mathtt{true}),\; \pair(s, \mathtt{false}),\; \pair(t, \mathtt{false})\;\}$
\\[0.2cm]
ist eine LÃ¶sung.  Betrachten wir eine weiteres Beispiel: \\[0.2cm]
\hspace*{1.3cm} 
$K_2 = \bigl\{\; \{\}, \{p\},\; \{\neg q\},\; \{r\}\; \bigr\}$ 
\\[0.2cm]
...
\\[0.2cm]
Offensichtlich ist $K_2$ unlÃ¶sbar.  Als letztes Beispiel betrachten wir 
\\[0.2cm]
...
$p \wedge \neg q \wedge \neg p $
\\[0.2cm]
Offenbar ist $K_3$ ebenfalls unlÃ¶sbar, denn eine LÃ¶sung $\I$ mÃ¼sste $p$ gleichzeitig
wahr und falsch machen.
Wir nehmen die an den letzten drei Beispielen gemachten Beobachtungen zum Anlass fÃ¼r zwei Definitionen.

\begin{Definition}[Unit-Klausel]
  Eine Klausel $k$ heiÃt \emph{Unit-Klausel}, wenn $k$ nur aus einem Literal besteht.
  Es gilt dann \\[0.2cm]
  \hspace*{1.3cm} $k = \{p\}$ \quad oder \quad $k = \{\neg p\}$ \\[0.2cm]
  fÃ¼r eine Aussage-Variable $p$. \eox
\end{Definition}

\begin{Definition}[Triviale Klausel-Mengen]
  Eine Klausel-Menge $K$ heiÃt \emph{trivial} wenn einer der beiden folgenden FÃ¤lle
  vorliegt.
  \begin{enumerate}
  \item $K$ enthÃ¤lt die leere Klausel: $\{\} \el K$.

        In diesem Fall ist $K$ offensichtlich unlÃ¶sbar.
  \item $K$ enthÃ¤lt nur Unit-Klausel mit verschiedenen Aussage-Variablen.
        ...
        $
        \\[0.2cm]
        eine LÃ¶sung von $K$. \eox
  \end{enumerate}
\end{Definition}


Wie kÃ¶nnen wir nun eine Menge von Klauseln so vereinfachen, dass die Menge schlieÃlich nur
noch aus Unit-Klauseln besteht?  Es gibt drei
MÃ¶glichkeiten, Klauselmengen zu vereinfachen:
\begin{enumerate}
\item Schnitt-Regel,
\item Subsumption und
\item Fallunterscheidung.
\end{enumerate}
Wir betrachten diese MÃ¶glickeiten jetzt der Reihe nach.
...
als die PrÃ¤missen $k_1 \cup \{p\}$ und $\bigl\{\neg p\} \cup k_2$.  EnthÃ¤lt die
Klausel $k_1 \cup \{p\}$ insgesamt $m+1$ Literale und enthÃ¤lt die Klausel
$\bigl\{\neg p\} \cup k_2$ insgesamt $n+1$ Literale, so kann die Konklusion $k_1 \cup k_2$ 
insgesamt $m + n$ Literale enthalten.  NatÃ¼rlich kÃ¶nnen es auch weniger Literale 
sein, und zwar dann, wenn es Literale gibt, die sowohl in $k_1$ als auch in $k_2$
auftreten.  Im allgemeinen ist $m + n$ grÃ¶Ãer als $m + 1$ und als $n + 1$.  Die
...
\emph{Unit-Schnitte}.  Um alle mit einer gegebenen Unit-Klausel $\{l\}$ mÃ¶glichen Schnitte
durchfÃ¼hren zu kÃ¶nnen, definieren wir eine Funktion
\\[0.2cm]
\hspace*{1.3cm}
$\textsl{unitCut}: 2^\mathcal{K} \times \mathcal{L} \rightarrow 2^\mathcal{K}$
\\[0.2cm]
so, dass fÃ¼r eine Klausel-Menge $K$ und ein Literal $l$ die Funktion
$\textsl{unitCut}(K,l)$ die Klausel-Menge $K$ soweit wie mÃ¶glich mit Unit-Schnitten mit der Klausel
$\{l\}$ vereinfacht:
\\[0.2cm]
\hspace*{1.3cm}
$\textsl{unitCut}(K,l) = \Bigl\{ k \backslash \bigl\{ \komplement{\,l\,} \bigr\} \;\Big|\; k \in K \Bigr\}$.
\\[0.2cm]
Beachten Sie, dass die Menge $\textsl{unitCut}(K,l)$ genauso viele Klauseln enthÃ¤lt wie die Menge
$K$.  Allerdings sind die Klauseln aus der Menge $K$, die das Literal $\komplement{\,l\,}$
enthalten, verkÃ¼rzt worden. 

\subsection{Vereinfachung durch Subsumption}
Das Prinzip der Subsumption demonstrieren wir zunÃ¤chst an einem Beispiel.
...
$\{p\}$ erfÃ¼llt ist, ist automatisch auch $\{q, p, \neg r\}$ erfÃ¼llt.  Das liegt daran, dass 
\\[0.2cm]
...
$k$ durch $u$ subsumiert, so kÃ¶nnen wir $K$ durch Unit-Subsumption zu $K - \{k\}$
vereinfachen, indem wir die Klausel $k$ aus $K$ lÃ¶schen.  Allgemein definieren wir eine Funktion
\\[0.2cm]
\hspace*{1.3cm}
$\textsl{subsume}: 2^\mathcal{K} \times \mathcal{L} \rightarrow 2^\mathcal{K}$
\\[0.2cm]
die eine gegebene Klauselmenge $K$, welche die Unit-Klausel $\{l\}$ enthÃ¤lt, mittels Subsumption 
dadurch vereinfacht, dass alle durch $\{l\}$ subsumierten Klauseln aus $K$ gelÃ¶scht werden.
Die Unit-Klausel $\{l\}$ behalten wir natÃ¼rlich.  Daher definieren wir:
\\[0.2cm]
\hspace*{1.3cm}
$\textsl{subsume}(K, l) := 
...
\\[0.2cm]
In der obigen Definition muss $\{l\}$ in das Ergebnis eingefÃ¼gt werden, weil die Menge
$\bigl\{ k \in K \mid l \not\in k \bigr\}$ die Unit-Klausel $\{l\}$ nicht enthÃ¤lt.


\subsection{Vereinfachung durch Fallunterscheidung}
Ein KalkÃ¼l, der nur mit Unit-Schnitten und Subsumption arbeitet, ist nicht 
widerlegungs-vollstÃ¤ndig.  Wir brauchen 
daher eine weitere MÃ¶glichkeit, Klausel-Mengen zu vereinfachen.
Eine solche MÃ¶glichkeit bietet das Prinzip der
...
\begin{Satz}
  ...
  so ist $K$ genau dann erfÃ¼llbar, wenn $K \cup \bigl\{\{p\}\bigr\}$ oder 
  $K \cup \bigl\{\{\neg p\}\bigr\}$ erfÃ¼llbar ist.  
\end{Satz}
Beweis: 
Ist $K$ erfÃ¼llbar durch eine
Belegung $\I$, so gibt es fÃ¼r  $\I(p)$ zwei MÃ¶glichkeiten:  Falls $\I(p) = \mathtt{true}$ ist, ist
damit auch die Menge $K \cup \bigl\{\{p\}\bigr\}$ erfÃ¼llbar, andernfalls ist
$K \cup \bigl\{\{\neg p\}\bigr\}$ erfÃ¼llbar. 

Da $K$ sowohl eine Teilmenge von $K \cup \bigl\{\{p\}\bigr\}$ als auch von 
$K \cup \bigl\{\{\neg p\}\bigr\}$ ist, ist klar, dass $K$ erfÃ¼llbar
ist, wenn eine dieser Mengen erfÃ¼llbar sind.  
\qed

Wir kÃ¶nnen nun eine Menge $K$ von Klauseln dadurch vereinfachen, dass wir eine
aussagenlogische Variable $p$ wÃ¤hlen, die in $K$ vorkommt.
AnschlieÃend bilden wir die Mengen \\[0.2cm]
\hspace*{1.3cm} $K_1 := K \cup \bigl\{\{p\}\bigr\}$ \quad und \quad $K_2 := K \cup
\bigl\{\{\neg p\}\bigr\}$
\\[0.2cm]
und untersuchen rekursiv ob $K_1$ erfÃ¼llbar ist.  Falls wir eine LÃ¶sung fÃ¼r $K_1$ finden,
ist dies auch eine LÃ¶sung fÃ¼r die ursprÃ¼ngliche Klausel-Menge $K$ und wir haben unser Ziel
erreicht.
Andernfalls untersuchen wir rekursiv ob $K_2$ erfÃ¼llbar ist.
Falls wir nun eine LÃ¶sung finden, ist dies auch eine LÃ¶sung von $K$ und wenn wir fÃ¼r $K_2$
keine LÃ¶sung finden, dann hat auch $K$ keine LÃ¶sung.
Die rekursive Untersuchung von $K_1$ bzw.~$K_2$ ist leichter,
weil ja wir dort mit den Unit-Klausel $\{p\}$ bzw.~$\{\neg p\}$
zunÃ¤chst Unit-Subsumption und anschlieÃend Unit-Schnitte durchfÃ¼hren kÃ¶nnen.


\subsection{Der Algorithmus}
Wir kÃ¶nnen jetzt den Algorithmus von Davis und Putnam skizzieren.
Gegeben sei eine Menge $K$ von Klauseln.  Gesucht ist dann eine LÃ¶sung von $K$.  Wir
suchen  also eine Belegung $\I$, so dass gilt: \\[0.2cm]
\hspace*{1.3cm} $\I(k) = \mathtt{true}$ \quad fÃ¼r alle $k \in K$.\\[0.2cm]
Das Verfahren von Davis und Putnam besteht nun aus den folgenden Schritten.
\begin{enumerate}
\item FÃ¼hre alle Unit-Schnitte aus, die mit Klauseln aus $K$ mÃ¶glich sind und fÃ¼hre
      zusÃ¤tzlich alle Unit-Subsumptionen aus.
\item Falls $K$ nun trivial ist, sind wir fertig.
\item Andernfalls wÃ¤hlen wir eine aussagenlogische Variable $p$, die in $K$ auftritt.
      \begin{enumerate}
      \item Jetzt versuchen  wir rekursiv  die Klausel-Menge \\[0.2cm]
            \hspace*{1.3cm}  $K \cup \bigl\{\{p\}\bigr\}$ \\[0.2cm]
            zu lÃ¶sen. Falls diese gelingt, haben wir eine LÃ¶sung von $K$.
      \item Andernfalls versuchen wir  die Klausel-Menge \\[0.2cm]
            \hspace*{1.3cm} $K \cup \bigl\{\{\neg p\}\bigr\}$ \\[0.2cm]
            zu lÃ¶sen.  Wenn auch dies fehlschlÃ¤gt, ist $K$ unlÃ¶sbar, andernfalls
            haben wir eine LÃ¶sung von $K$.
      \end{enumerate}
\end{enumerate}
FÃ¼r die Implementierung ist es zweckmÃ¤Ãig, die beiden oben definierten Funktionen $\textsl{unitCut}()$ und
...
$
\\[0.2cm]
Die Menge enthÃ¤lt also einerseits die Ergebnisse von Schnitten mit
...
gilt.  AuÃerdem fÃ¼gen wir auch noch die Unit-Klausel $\{l\}$ hinzu.
Dadurch erreichen wir, dass die beiden Mengen $K$ und $\textsl{reduce}(K,l)$
logisch Ã¤quivalent sind, wenn wir dieses Mengen als Formeln in konjunktiver Normalform
interpretieren.  
...
\hspace*{2.5cm} $\{\neg s, p\},\; \{\neg p, \neg q, s, \neg r\},\; \{p, \neg q, s\},\; \{\neg r, \neg s\},\; \{\neg p, \neg s\} \Big\}  $. \\[0.2cm]
Wir zeigen nun mit dem Verfahren von Davis und Putnam, dass $K$ nicht lÃ¶sbar ist.  Da die
Menge $K$ keine Unit-Klauseln enthÃ¤lt, ist im ersten Schritt nichts zu tun.  Da $K$ nicht
trivial ist, sind wir noch nicht fertig.  Also gehen wir jetzt zu Schritt 3 und wÃ¤hlen
eine aussagenlogische Variable, die in $K$ auftritt.  An dieser Stelle ist es sinnvoll
eine Variable zu wÃ¤hlen, die in mÃ¶glichst vielen Klauseln von $K$ auftritt.  Wir wÃ¤hlen
daher die aussagenlogische Variable $p$.
\begin{enumerate}
\item ZunÃ¤chst bilden wir die Menge \\[0.2cm]
      \hspace*{1.3cm} $K_0 := K \cup \bigl\{ \{p\} \bigr\}$       \\[0.2cm]
      und versuchen, diese Menge zu lÃ¶sen.  Dazu bilden wir \\[0.2cm]
      \hspace*{0.3cm} 
      $K_1 := \textsl{reduce}\bigl(K_0,p\bigr) = 
          \Big\{ \{r, \neg t\},\; \{r, s\},\; \{\neg r, q\},\; \{\neg q, s, \neg r\},\; \{\neg r, \neg s\},\; \{ \neg s\},\;\{p\}\, \Big\}$.
      \\[0.2cm]
      Die Klausel-Menge $K_1$ enthÃ¤lt die Unit-Klausel $\{\neg s\}$,
      so dass wir als nÃ¤chstes mit dieser Klausel reduzieren kÃ¶nnen: \\[0.2cm]
      \hspace*{1.3cm} 
      $K_2 := \textsl{reduce}\bigl(K_1,\;\neg s\bigr) = 
              \Big\{ \{r, \neg t\},\; \{r\},\; \{\neg r, q\},\; \{\neg q, \neg r\},\; \{ \neg s\},\; \{p\} \Big\}$.
      \\[0.2cm]
      Hier haben wir nun die neue Unit-Klausel $\{r\}$, mit der wir als nÃ¤chstes reduzieren:
      \\[0.2cm]
      \hspace*{1.3cm} 
      $K_3 := \textsl{reduce}\bigl(K_2,\; r\bigr) = 
              \Big\{ \{r\},\; \{q\},\; \{\neg q\},\; \{ \neg s\},\; \{p\} \Big\}$
      \\[0.2cm]
      Da $K_3$ die Unit-Klausel $\{q\}$ enthÃ¤lt, reduzieren wir jetzt mit $q$: \\[0.2cm]
      \hspace*{1.3cm} 
      $K_4 := \textsl{reduce}\bigl(K_2,\;q\bigr) = 
              \Big\{ \{r\},\; \{q\},\; \{\},\; \{ \neg s\},\; \{p\} \Big\}$.
      \\[0.2cm]
      Die Klausel-Menge $K_4$ enthÃ¤lt die leere Klausel und ist damit unlÃ¶sbar.
     
\item Also bilden wir jetzt die Menge \\[0.2cm]
      \hspace*{1.3cm} $K_5 := K \cup \bigl\{ \{\neg p\} \bigr\}$ \\[0.2cm]
      und versuchen, diese Menge zu lÃ¶sen.  Dazu bilden wir
      \\[0.2cm]
      \hspace*{1.3cm} 
      $K_6 = \textsl{reduce}\bigl(K_5,\; \neg p\bigr) =\Big\{ \{q, s\},\; \{r, s\},\;\{\neg s\},\; \{\neg q, s\},\; \{\neg r, \neg s\},\;\{\neg p\}\, \Big\}$.
      \\[0.2cm]
      Die Menge $K_6$ enthÃ¤lt die  Unit-Klausel $\{\neg s\}$.  Wir bilden daher \\[0.2cm]
      \hspace*{1.3cm} 
      $K_7 = \textsl{reduce}\bigl(K_6,\; \neg s\bigr) =\Big\{ \{q\},\; \{r\},\;\{\neg s\},\; \{\neg q\},\;\{\neg p\}\, \Big\}$.
      \\[0.2cm]
      Die Menge $K_7$ enthÃ¤lt die neue Unit-Klausel $\{q\}$, mit der wir als nÃ¤chstes reduzieren:\\[0.2cm]
      \hspace*{1.3cm} 
      $K_8 = \textsl{reduce}\bigl(K_7,\; q \bigr) =\Big\{ \{q\},\; \{r\},\;\{\neg s\},\; \{\},\;\{\neg p\}\, \Big\}$.
      \\[0.2cm]
      Da $K_8$ die leere Klausel enthÃ¤lt, ist $K_8$ und damit auch die ursprÃ¼nglich
      gegebene Menge $K$ unlÃ¶sbar.
\end{enumerate}
Bei diesem Beispiel hatten wir GlÃ¼ck, denn wir mussten nur eine einzige Fallunterscheidung
durchfÃ¼hren. Bei komplexeren Beispielen ist es hÃ¤ufig so, dass wir mehrere Fallunterscheidungen
...
mit der die Frage, ob eine Menge von Klauseln erfÃ¼llbar ist, beantwortet werden kann. Die
Implementierung ist in Abbildung \ref{fig:davisPutnam} auf Seite \pageref{fig:davisPutnam}
gezeigt.  Die Prozedur erhÃ¤lt zwei Argumente: \texttt{clauses} und \texttt{literals}.
\texttt{clauses} ist eine Menge von Klauseln und \texttt{literals} ist eine Menge von
Literalen.  Falls  die Vereinigung dieser beiden Mengen erfÃ¼llbar ist, so liefert
der Aufruf 
\\[0.2cm]
\hspace*{1.3cm}
...
dass jede Belegung $\I$, die alle Unit-Klauseln aus \texttt{r} erfÃ¼llt, auch die
Menge $\mathtt{clauses} \cup \mathtt{literals}$ erfÃ¼llt.  Falls die Menge
$\mathtt{clauses} \cup \mathtt{literals}$ nicht erfÃ¼llbar ist, liefert der Aufruf
\\[0.2cm]
\hspace*{1.3cm}
\texttt{davisPutnam(clauses, literals)} 
\\[0.2cm]
als Ergebnis die Menge $\bigl\{ \{\} \bigr\}$ zurÃ¼ck,
denn die leere Klausel reprÃ¤sentiert die unerfÃ¼llbare Formel $\falsum$.
...
merken mÃ¼ssen, welche Literale wir schon benutzt haben.  Diese Literale sammeln wir in der
...
\item In Zeile 2 reduzieren wir mit Hilfe der Methode \texttt{saturate} 
      solange wie mÃ¶glich die gegebene Klausel-Menge \texttt{clauses} mit Hilfe
      von Unit-Schnitten und entfernen alle Klauseln die durch Unit-Klauseln
      subsumiert werden.
\item AnschlieÃend testen wir in Zeile 3, ob die so vereinfachte Klausel-Menge
      die leere Klausel enthÃ¤lt und geben in diesem Fall als Ergebnis die Menge 
      $\bigl\{\{\}\bigr\}$ zurÃ¼ck.
\item Dann testen wir in Zeile 6, ob bereits alle Klauseln $c$ aus der Menge
      ...
      dann ist die Menge \texttt{clauses} trivial und wir geben diese Menge als Ergebnis zurÃ¼ck.
\item Andernfalls wÃ¤hlen wir in Zeile 9 ein Literal $l$ aus der Menge \texttt{clauses}, 
      ...
      \\[0.2cm]
      lÃ¶sbar ist.  Dabei gibt es zwei FÃ¤lle:
      \begin{enumerate}
      \item Falls diese Menge lÃ¶sbar ist, geben wir die LÃ¶sung dieser Menge als Ergebnis zurÃ¼ck.

      \item Sonst  prÃ¼fen wir rekursiv, ob die Menge \\[0.2cm]
...
Als erstes besprechen wir die Funktion \texttt{saturate}.  Diese Prozedur erhÃ¤lt eine
Menge $s$ von Klauseln als Eingabe und fÃ¼hrt alle mÃ¶glichen Unit-Schnitte und
...
\begin{enumerate}
\item ZunÃ¤chst berechnen wir in Zeile 2 die Menge \texttt{units} aller Unit-Klauseln.  
\item Dann initialisieren wir in Zeile 3 die Menge \texttt{used} als die leere Menge.
      In dieser Menge merken wir uns, welche Unit-Klauseln wir schon fÃ¼r Unit-Schnitte und
      Subsumptionen benutzt haben.
\item Solange die Menge \texttt{units} der Unit-Klauseln nicht leer ist, wÃ¤hlen wir in Zeile 5
      eine beliebige Unit-Klausel \texttt{unit} aus der Menge \texttt{units} aus.
\item In Zeile 6 fÃ¼gen wir die Klausel \texttt{unit} zu der Menge
...
\item In Zeile 8 wird  die eigentliche Arbeit durch einen Aufruf der Prozedur
      ...
      Unit-Klausel $\{\texttt{l}\}$ mÃ¶glich sind und entfernt darÃ¼ber hinaus alle Klauseln, die
      durch die Unit-Klausel $\{\texttt{l}\}$ subsumiert werden.
\item Wenn die Unit-Schnitte mit der Unit-Klausel $\{\texttt{l}\}$ berechnet werden, kÃ¶nnen neue
...
\item Am Ende geben wir die verbliebende Klauselmenge als Ergebnis zurÃ¼ck.
\end{enumerate}
...
Die Implementierung des Algorithmus von Davis und Putnam benutzt auÃer den bisher diskutierten Prozeduren
...
\begin{enumerate}
\item Die Prozedur \texttt{selectLiteral} wÃ¤hlt ein beliebiges Literal aus 
      einer gegeben Menge $s$ von Klauseln aus, das auÃerdem nicht in der Menge
      ...
      resultierenden Menge wird mit Hilfe der Funktion $\texttt{arb}()$ ein Literal ausgewÃ¤hlt.
...
Die oben dargestellte Version des Verfahrens von Davis und Putnam lÃ¤sst sich in vielerlei
Hinsicht verbessern.  Aus ZeitgrÃ¼nden kÃ¶nnen wir auf solche Verbesserungen leider nicht
...
Probleme umformuliert werden kÃ¶nnen.  Diese
kÃ¶nnen dann anschlieÃend mit dem Algorithmus von Davis und Putnam bzw.~mit 
Verbesserungen dieses Algorithmus gelÃ¶st werden.  Als konkretes
...
Als erstes Ã¼berlegen wir uns, wie wir ein Schach-Brett mit den darauf
positionierten Damen aussagenlogisch reprÃ¤sentieren kÃ¶nnen.  Eine MÃ¶glichkeit besteht darin, 
fÃ¼r jedes Feld eine aussagenlogische Variable einzufÃ¼hren.  Diese Variable drÃ¼ckt
...
dar. Wir nummerieren die Zeilen dabei von oben beginnend von 1 bis 8 durch, wÃ¤hrend die
...
Als nÃ¤chstes Ã¼berlegen wir uns, wie wir die einzelnen Bedingungen des 8-Damen-Problems 
als aussagenlogische
Formeln kodieren kÃ¶nnen.  Letztlich lassen sich alle Aussagen der Form
\begin{itemize}
\item ``in einer Zeile steht hÃ¶chstens eine Dame'', 
\item ``in einer Spalte steht hÃ¶chstens eine Dame'', oder 
\item ``in einer Diagonale steht hÃ¶chstens eine Dame'' 
\end{itemize}
auf dasselbe Grundmuster zurÃ¼ckfÃ¼hren:
Ist eine Menge von aussagenlogischen Variablen \\[0.2cm]
\hspace*{1.3cm} $V = \{ x_1, \cdots, x_n \}$ \\[0.2cm]
gegeben, so brauchen wir eine Formel die aussagt, dass \textbf{\emph{hÃ¶chstens}} eine der Variablen aus
$V$ den Wert \texttt{true} hat.  Das ist aber gleichbedeutend damit, dass fÃ¼r jedes Paar
$x_i, x_j \in V$ mit $x_i \not= x_j$ die folgende Formel gilt: \\[0.2cm]
\hspace*{1.3cm} $\neg (x_i \wedge x_j)$. \\[0.2cm]
Diese Formel drÃ¼ckt aus, dass die Variablen $x_i$ und $x_j$ nicht gleichzeitig den Wert
...
\\[0.2cm]
\hspace*{1.3cm}  $\{\neg x_i, \neg x_j \}$. \\[0.2cm]
Die Formel, die fÃ¼r eine Variablen-Menge $V$ ausdrÃ¼ckt, dass keine zwei verschiedenen
...
Klauseln.  Diese Klauseln sind genau dann wahr, wenn hÃ¶chstens eine der Variablen aus $V$
...
Mit Hilfe der Prozedur \texttt{atMostOne} kÃ¶nnen wir nun die Prozedur
\texttt{atMostOneInRow} implementieren.  Der Aufruf \\[0.2cm]
\hspace*{1.3cm} \texttt{atMostOneInRow}(\textsl{row}, n) \\[0.2cm]
berechnet fÃ¼r eine gegebene Zeile \textsl{row} bei einer BrettgrÃ¶Ãe von $n$ eine Formel,
die ausdrÃ¼ckt, dass in der Zeile \textsl{row} hÃ¶chstens eine Dame steht.
...
Als nÃ¤chstes berechnen wir eine Formel die aussagt, dass mindestens eine Dame in einer gegebenen
Spalte steht.  FÃ¼r die erste Spalte hÃ¤tte diese Formel im Falle eine $8 \times 8$-Bretts die Form 
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
Abbildung \ref{fig:oneInColumn} zeigt eine \textsc{SetlX}-Prozedur, die fÃ¼r eine gegebene Spalte
\texttt{column} und eine gegebene BrettgrÃ¶Ãe \texttt{n }die entsprechende Klausel-Menge berechnet.
Der Schritt, von einer einzelnen Klausel 
zu einer Menge von Klauseln Ã¼berzugehen ist notwendig, da unsere Implementierung des Algorithmus von
...
ausdrÃ¼cken, dass in einer gegebenen Spalte hÃ¶chstens eine Dame steht und dass in jeder
Reihe mindestens eine Dame steht.
Solche Formeln sind aber unnÃ¶tig, denn wenn wir wissen, dass in jeder Spalte mindestens
eine Dame steht, so wissen wir bereits, dass auf dem Brett mindestens 8 Damen stehen.
Wenn wir nun zusÃ¤tzlich wissen, dass in jeder Zeile hÃ¶chstens eine Dame steht, so ist
...
dass es insgesamt nicht mehr als 8 Damen sind, dass in jeder Spalte hÃ¶chstens eine Dame stehen kann.

Als nÃ¤chstes Ã¼berlegen wir uns, wie wir die Variablen, die auf derselben Diagonale
stehen, charakterisieren kÃ¶nnen.  Es gibt grundsÃ¤tzlich zwei verschiedene Arten von
Diagonalen: absteigende Diagonalen und aufsteigende Diagonalen.  Wir betrachten zunÃ¤chst
die aufsteigenden Diagonalen.  Die lÃ¤ngste aufsteigende Diagonale, wir sagen dazu auch
...
\\[0.2cm]
Die Indizes $i$ und $j$ der Variablen $\mathtt{Var}(i,j)$ erfÃ¼llen offenbar
die Gleichung \\[0.2cm]
\hspace*{1.3cm} $i + j = 9$. \\[0.2cm]
Allgemein erfÃ¼llen die Indizes der Variablen einer aufsteigenden Diagonale die Gleichung \\[0.2cm]
...
charakterisiert werden kÃ¶nnen, betrachten wir die fallende Hauptdiagonale, die aus den
Variablen \\[0.2cm]
\hspace*{1.3cm} 
$\texttt{Var}(1,1),\; \texttt{Var}(2,2),\; \texttt{Var}(3,3),\; \texttt{Var}(4,4),\; \texttt{Var}(5,5),\; 
 \texttt{Var}(6,6),\; \texttt{Var}(7,7),\; \texttt{Var}(8,8)$ 
\\[0.2cm]
besteht. Die Indizes  $i$ und $j$ dieser Variablen erfÃ¼llen offenbar
die Gleichung \\[0.2cm]
\hspace*{1.3cm} $i - j = 0$. \\[0.2cm]
Allgemein erfÃ¼llen die Indizes der Variablen einer absteigenden Diagonale die Gleichung \\[0.2cm]
\hspace*{1.3cm} $i - j = k$, \\[0.2cm]
...
Jetzt sind wir in der Lage, unsere Ergebnisse zusammen zu fassen:  Wir kÃ¶nnen eine
Menge von Klauseln konstruieren, die das 8-Damen-Problem vollstÃ¤ndig beschreibt.
...
rechnet fÃ¼r ein Schach-Brett der GrÃ¶Ãe $n$ eine Menge von Klauseln aus, die
genau dann erfÃ¼llt sind, wenn auf dem Schach-Brett
\begin{enumerate}
\item in jeder Zeile hÃ¶chstens eine Dame steht (Zeile 2),
\item in jeder absteigenden Diagonale hÃ¶chstens eine Dame steht (Zeile 3),
\item in jeder aufsteigenden Diagonale hÃ¶chstens eine Dame steht (Zeile 4) und
\item in jeder Spalte mindestens eine Dame steht (Zeile 5).
\end{enumerate}
Die AusdrÃ¼cke in den einzelnen Zeilen liefern Mengen, deren Elemente
...
\texttt{solve}, mit der wir das 8-Damen-Problem lÃ¶sen kÃ¶nnen.
Hierbei ist $\texttt{printBoard}()$ eine Prozedur, welche die LÃ¶sung in lesbarere Form als Schachbrett
ausdruckt.  Das funktioniert allerdings nur, wenn ein Font verwendet wird, bei dem alle Zeichen die
selbe Breite haben.  Diese Prozedur ist der VollstÃ¤ndigkeit halber in Abbildung \ref{fig:printBoard}
gezeigt, wir wollen die Implementierung aber nicht weiter diskutieren.
Das vollstÃ¤ndige Programm finden Sie auf meiner Webseite unter dem Namen
...
berechnete Menge \texttt{solution} enthÃ¤lt fÃ¼r jede der Variablen $\mathtt{Var}(i,j)$
...
\vbox{\offinterlineskip
  ...
  \caption{Eine LÃ¶sung des 8-Damen-Problems.}
  \label{fig:queens-solution}
\end{figure}


Das 8-Damen-Problem ist natÃ¼rlich nur eine spielerische Anwendung der Aussagen-Logik.
Trotzdem zeigt es die LeistungsfÃ¤higkeit des Algorithmus von Davis
und Putnam sehr gut, denn die Menge der Klauseln, die von der Prozedur \texttt{allClauses}
berechnet wird, fÃ¼llt unformatiert fÃ¼nf Bildschirm-Seiten, falls diese  eine Breite von 80
Zeichen haben.  In dieser Klausel-Menge kommen 64 verschiedene Variablen vor.
Der Algorithmus von Davis und Putnam benÃ¶tigt zur Berechnung einer Belegung, die diese
Klauseln erfÃ¼llt, auf meinem iMac weniger als fÃ¼nf Sekunden.

In der Praxis gibt es viele Probleme, die sich in ganz Ã¤hnlicher Weise auf die LÃ¶sung einer
Menge von Klauseln zurÃ¼ckfÃ¼hren lassen.  Dazu gehÃ¶rt zum Beispiel das Problem, einen
Stundenplan zu erstellen, der gewissen Nebenbedingungen genÃ¼gt.  Verallgemeinerungen des
Stundenplan-Problems werden in der Literatur als \emph{Scheduling-Problemen} bezeichnet.
Die effiziente LÃ¶sung solcher Probleme ist Gegenstand der aktuellen Forschung.
