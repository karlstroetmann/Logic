...
\item Bei der Implementierung des Prädikats \texttt{edge/2} verwenden wir in den Zeilen 12
    ...
    \item Die Implementierung des Prädikats \texttt{edge/2} besteht aus zwei Klauseln.  In
      ...
      Die zweite Klausel befaßt sich mit dem Fall, dass das Boot am rechten Ufer liegt.
...
In diesem Abschnitt zeigen wir, wie sich das 8-Damen-Problem in \textsl{Prolog} lösen läßt.  Wir beginnen mit
...
\item Das Prädikat \texttt{attack(C, X, L)} erhält drei Argumente.  Das erste Argument spezifiziert eine
      ...
      \begin{enumerate}
      \item Falls die durch $C$ spezifizierte Dame in der selben Spalte steht wie die erste
      ...
      \item Die nächste Klausel behandelt den Fall, dass die erste Dame der Liste $L$ in der selben 
      ...
      \item Analog behandelt die dritte Klausel den Fall, dass die erste Dame der Liste $L$ in der selben 
...
Das Ergebnis ist zwar immer das selbe, nämlich \texttt{L = []}, es wird aber zweimal ausgegeben:
\begin{Verbatim}[ frame         = lines, 
                  framesep      = 0.3cm, 
                  labelposition = bottomline,
                  numbers       = left,
...
eine Liste zu zerlegen, wird beim Backtracking immer wieder die selbe Lösung generiert. 
Beispielsweise liefert die Anfrage \\[0.1cm]
\hspace*{1.3cm} \texttt{bubble\_sort( [ 4, 3, 2, 1 ], L ), write(L), nl, fail.} \\[0.1cm]
16 mal die selbe Lösung.  Abbildung \ref{fig:bubble_sort_cut} zeigt eine Implementierung,
...
\texttt{L} läßt sich ja auf jeden Fall dadurch sortieren, dass rekursiv die Liste \\[0.1cm]
\hspace*{1.3cm} $\mathtt{append(L_1, [Y,X|L_2])}$ \\[0.1cm]
...
\texttt{[X, Y | L2]} gibt, bei der \texttt{Y} kleiner als \texttt{X} ist.  Dann muß aber
