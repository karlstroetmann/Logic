...
\begin{enumerate}
\item Der Software-Entwickler erstellt eine Datenbank.
...
\item Ein \emph{Fakt} ist eine atomare Formel.  Die Syntax ist\\[0.2cm]
      ...
      1 -- 5 Fakten.  Umgangssprachlich können wir diese wie folgt lesen:
      \begin{enumerate}
...
\item Eine \emph{Regel} ist eine bedingte Aussage.  Die Syntax ist \\[0.2cm]
      ...
      \\[0.2cm]
      Dabei sinder $A$ und $B_1, \cdots, B_n$ atomare Formeln, haben also die Gestalt \\[0.2cm]
      ...
      7--12 Regeln.  Schreiben wir diese Regeln als prädikatenlogische Formeln,
      so erhalten wir:
      \begin{enumerate}
...
Prolog hat sehr einfache Regeln um Variablen von Prädikats- und Funktions-Zeichen
...
\item Strings, die mit einem kleinen Buchstaben beginnen und nur aus Buchstaben, Ziffern
      ...
      Die folgenden Strings sind können also als Funktions- oder Prädikats-Zeichen
      verwendet werden:
      \\[0.2cm]
      \hspace*{1.3cm}      
...
\item Die Strings \\[0.2cm]
      ...
      ``\texttt{$\backslash$==}''.
      \\[0.2cm]
      bezeichnen Prädikats-Zeichen. Für diese Prädikats-Zeichen ist eine
      Infix-Schreibweise zulässig.
      
      Gegenüber den Sprachen \texttt{C} und \textsl{Java}
      gibt es hier die folgenden Unterschiede, die besonders Anfängern oft
      Probleme bereiten.
      \begin{enumerate}
      \item Bei dem Operator ``\texttt{=<}'' treten die Zeichen ``\texttt{=}''
            und ``\texttt{<}'' nicht in der selben Reihenfolge auf, wie das in den
            Sprachen \texttt{C} oder \textsl{Java} der Fall ist, denn dort wird dieser
      ...
      \item Der Operator ``\texttt{==}'' testet, ob die beiden Argumente gleich sind,
            ...
            ungleich sind.  In \texttt{C} und \textsl{Java} hat der entsprechende Operator
            die Form ``\texttt{!=}''.
      \item Der Operator ``\texttt{=}'' ist der Unifikations-Operator.
            ...
            lösen.  (Leider wird dabei der \emph{Occur-Check} nicht durchgeführt.)
...
Sie finden dieses Prolog-System im Netz unter \texttt{www.swi-prolog.org}.}
in einer Shell mit dem Kommando: \\[0.2cm]
\hspace*{1.3cm} \texttt{pl}\\[0.2cm]
...
\begin{verbatim}
    X = caesar 
\end{verbatim}
Wenn wir mit dieser Antwort zufrieden sind, drücken wir \textsl{Return} und erhalten einen
neuen Prompt.  Wenn wir statt dessen nach weiteren Personen suchen wollen, die einerseits
mächtig sind und andererseits spinnen, dann geben wir das Zeichen ``\texttt{;}'' ein,
bevor wir \textsl{Return} drücken.  In diesem Fall erhalten wir die Antwort
\begin{verbatim}
    No
    ?- 
\end{verbatim}
Bei den oben gegebenen Regeln und Fakten gibt es außer Caesar also niemanden, der mächtig
ist und spinnt.  Wenn wir das 
\textsl{Prolog}-System wieder verlassen wollen, dann geben wir den Befehl
``\texttt{halt}.'' ein.
...
\item \textbf{Gesucht}: Eine Substitution $\sigma$, so dass die Instanz $G\sigma$ aus den
      ...
      Hier bezeichnet $\forall(P)$ den Allabschluß der Konjunktion aller Klauseln aus $P$
      und $\forall(G\sigma)$ bezeichnet entsprechend den Allabschluß von $G\sigma$.
\end{enumerate}
...
\item Gibt es mehrere solche Regeln, so
      ...
      \item Außerdem setzen wir an dieser Stelle einen Auswahl-Punkt (\emph{Choice-Point}),
            um später hier eine anderer Regel wählen zu können, falls dies notwendig werden
            sollte.
      \end{enumerate}
...
\begin{enumerate}
\item Die Schaltfläche \raisebox{-0.2cm}{\epsfig{file=Figures/down,scale=1}}
      ...
      Alternativ hat die Taste ``\texttt{i}'' die selbe Funktion.
\item Die Schaltfläche \raisebox{-0.2cm}{\epsfig{file=Figures/step,scale=1}}
      dient dazu, einen einzelnen Schritt bei der Abarbeitung einer Anfrage
      durchzuführen.

      Alternativ hat die Leertaste die selbe Funktion.
\item Die Schaltfläche \raisebox{-0.2cm}{\epsfig{file=Figures/skip,scale=1}}
      ...
      Alternativ hat die Taste ``\texttt{s}'' (\emph{skip}) die selbe Funktion.
\item Die Schaltfläche \raisebox{-0.2cm}{\epsfig{file=Figures/finish,scale=1}} dient dazu,
      ...
      Alternative hat die Taste ``\texttt{f}'' (\emph{finish}) die selbe Funktion.
\item Die Schaltfläche \raisebox{-0.2cm}{\epsfig{file=Figures/redo,scale=1}} 
      dient dazu, eine bereits beantwortete Anfrage noch einmal zu beantworten.
      Dies kann sinnvoll sein, wenn man beim Tracen einer Anfrage nicht aufgepaßt hat
      und noch einmal sehen möchte, wie das \textsl{Prolog}-System zu seiner Antwort
      gekommen ist.
      
      Alternative hat die Taste ``\texttt{r}'' (\emph{retry}) die selbe Funktion.
\item Die Schaltfläche \raisebox{-0.2cm}{\epsfig{file=Figures/nodebug,scale=1}}
      beantwortet die gestellte Anfrage ohne weitere Unterbrechung.

      Alternative hat die Taste ``\texttt{n}'' (\emph{nodebug}) die selbe Funktion.

      Von den weiteren Schaltflächen sind fürs erste nur noch zwei interessant.
\item Die Schaltfläche \raisebox{-0.2cm}{\epsfig{file=Figures/continue,scale=1}}
       läßt das Programm bis zum nächsten Haltepunkt laufen.

      Alternative hat die Taste ``\texttt{l}'' (\emph{continue}) die selbe Funktion.
\item Die Schaltfläche \raisebox{-0.2cm}{\epsfig{file=Figures/break,scale=1}} 
      ...
      Alternative hat die Taste ``\texttt{!}''  die selbe Funktion.
\end{enumerate}

\begin{figure}[!h]
  \centering
  \framebox{\epsfig{file=Figures/debug,scale=14}} 
...
\item Die einzige Regel, die das Prädikat \texttt{roemer/1} definiert, ist \\[0.2cm]
      ...
      Diese Regel läßt sich nicht mit der Anfrage ``\texttt{roemer(asterix)}'' unifizieren.
...
beschriftet ist, bezeichnen wir als wie \emph{Wurzel} des Suchbaums.  Die \emph{Lösungen} zu der
...
Anhand des in Abbildung \ref{fig:depth-first.eps} gezeigten Suchbaums läßt sich nun die Tiefensuche
erklären:  Wenn der Prolog-Interpreter nach einer Lösung sucht, so wählt es immer den linkesten Ast
...
Das obige Beispiel war bewußt einfach gehalten um die Sprache \textsl{Prolog} einzuführen.
...
\hspace*{1.3cm} 
$\textsl{Language} \in \{ \texttt{java}, \texttt{prolog}, \texttt{setl} \}$. \\[0.2cm]
...
\hspace*{1.3cm} \texttt{car( person(hans, seat, setl), X ).}\\[0.2cm]
...
\hspace*{0.3cm} 
$\texttt{car( person(hans, seat, setl), X )} \doteq \texttt{car( person(Name, Car, Language), Car )}$
\\[0.2cm]
zu lösen.  Bei der Unifikation findet sich die Lösung \\[0.2cm]
\hspace*{1.3cm} 
$\mu = [ \texttt{Name} \mapsto \texttt{hans},\; \texttt{Car} \mapsto \texttt{seat},\;\texttt{Language} \mapsto \texttt{setl},\; \texttt{X} \mapsto \texttt{seat}]$.
\\[0.2cm]
...
\item[5.] der Name der durch \texttt{Michael} bezeichneten Person den Wert \texttt{michael}
         hat und
\item[6.] \texttt{Michael} in \textsc{Setl} programmiert und
...
Das Programm implementiert das dreistellige Prädikat \texttt{concat}\footnote{In dem
  \textsl{SWI-Prolog}-System gibt es das vordefinierte Prädikat \texttt{append/3},
  das genau das selbe leistet wie unsere Implementierung von \texttt{concat/3}.}.  
Die Intention ist,
dass $\texttt{concat}(l_1,l_2,l_3)$ für drei Listen $l_1$, $l_2$ und $l_3$ genau dann
wahr sein soll,
...
\item Um an eine Liste \texttt{[ X | L1 ]}, die aus dem Element \texttt{X} und dem Rest \texttt{L1} besteht,
      ...
      Das Ergebnis erhalten wir, wenn wir vor \texttt{L3} noch das Element \texttt{X}
...
``\texttt{concat.pl}'' abgespeichert sind und dass wir diese Datei mit dem Befehl ``\texttt{consult(concat).}'' geladen haben.
Dann stellen wir die Anfrage \\[0.2cm]
\hspace*{1.3cm} \texttt{?- concat( [ 1, 2, 3 ], [ a, b, c ], L ).} \\[0.2cm]
Wir erhalten die Antwort:
\begin{verbatim}
    L = [1, 2, 3, a, b, c] 
\end{verbatim}
Die obige Interpretation des gegebenen Prolog-Programms ist \emph{funktional}, dass heißt 
wir fassen die ersten beiden Argumente des Prädikats \texttt{concat} als \emph{Eingaben} auf und 
interpretieren das letzte \emph{Argument} als Ausgabe.  Diese Interpretation ist ist aber keineswegs die 
einzig mögliche Interpretation.  Um das zu sehen, geben wir als Ziel \\[0.2cm]
\hspace*{1.3cm} \texttt{concat(L1, L2, [1,2,3]).} \\[0.2cm]
...
es gibt, um die Liste ``\texttt{[1, 2, 3]}'' in zwei Teile zu zerlegen.
...
die letzten $n-1$ Zahlen und fügen dann das erste Element in die sortierte Liste an der
richtigen Stelle ein.
Mit dieser Idee besteht das Programm aus zwei Prädikaten:
\begin{enumerate}
\item Das Prädikat \texttt{insert/3} erwartet
      ...
      wiederum in aufsteigender Reihenfolge sortiert ist. Das so berechnete Ergebnis
      wird als letztes Argument des Prädikats zurück gegeben.  
...
\item Das Prädikat \texttt{insertion\_sort/2} hat die Typ-Spezifikation \\[0.2cm]
      ...
      \\[0.2cm]
      Der Aufruf \texttt{insertion\_sort}(\textsl{List}, \textsl{Sorted}) sortiert
      \textsl{List} in aufsteigender Reihenfolge.
\end{enumerate}
Abbildung \ref{fig:insertion-sort} zeigt das \textsl{Prolog}-Programm.
...
\item Die dritte Klausel greift, wenn die Liste, in die die Zahl  \texttt{X} eingefügt
      ...
      Rest0 der Liste, in die wir \texttt{X} einfügen wollen.
...
Rechenzeit nur mit dem Faktor $n * \textsl{log}_2(n)$.  Den Nachweis dieser Behauptung
erbringen wir im zweiten Semester.
Wenn es sich bei der zu sortierenden Liste beispielsweise um ein Telefonbuch mit einer
Millionen Einträgen handelt, dann ist der relative Unterschied des Rechenzeit-Verbrauchs
durch einen Faktor der Größe $50\,000$ gegeben. 
...
\item In der ersten Phase wird die zu sortierende Liste in zwei etwa gleich große
      Teile aufgeteilt.
\item In der zweiten Phase werden diese Teile rekursiv sortiert.
...
\item $x \leq y \rightarrow \texttt{mix}([x|s], [y|t]) = [x|\texttt{mix}(s, [y|t])]$.
      ...
      der Liste die entsteht, wenn wir die Listen $[x|s]$ und $[y|t]$ mischen.
...
\texttt{merge/3} und \texttt{sort/2} gibt, haben wir statt dessen die Namen
...
Im Rahmen einer Übung haben wir ein \textsc{Setl}-Programm entwickelt, das arithmetische Ausdrücke
symbolisch differenziert.  Damals war es notwendig gewesen, die zu differenzierenden Terme
durch geeignete \textsc{Setl}-Objekte zu repräsentieren.  An dieser Stelle ist die
\textsl{Prolog}-Implementierung einfacher, denn arithmetische Ausdrücke können
unmittelbar durch Terme dargestellt werden.  
...
\item Zahlen sind arithmetische Ausdrücke.
      ...
      Sowohl die ganzen Zahlen als auch die reellen Zahlen sind Bestandteil der Sprache
