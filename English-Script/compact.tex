...
\item[I.A.:] $i \mapsto 0$.
    ...
    \[ B_0 = \bigl\{ n \el \mathbb{N} \mid \I_n(p_0) = \mathtt{true} \bigr\} = D \]
     ...
     Ist die obige Menge $D$ endlich, so muß die Menge 
...
\item[I.S.:] $i \mapsto i + 1$.
     ...
     \[G:=\textsl{card}\bigl(\{ n \el B_i \mid \I_n(p_{i+1}) = \mathtt{false}\}\bigr)\]
      ...
      so dass auch $B_{i+1}$ unendlich ist.  Falls $F$ endlich ist, muß $G$ unendlich
...
Da die Menge $B_i$ unendlich viele natürliche Zahlen enthält, muß sie auch beliebig große
Zahlen enthalten.  
...
herleiten läßt, dann  ganz sicher auch aus den in $M$ enthaltenen Klauseln, denn $M$ ist
...
trotzdem nie leer wird?  Wir zeigen an Hand eines konkreten Beispiels, wie dies möglich
