...
function divisors(k: number): number[] {
    return Array.from({ length: k }, (_, t) => t + 1).filter(t => k % t == 0);
}
...
\cite{turing:1937}.
Of course, Turing did not solve the problem for \textsl{Python} but rather
for the so called 
...
However, if you check the proof given for \textsl{Python} you will easily
