...
A string $t$ is a \blue{test function with name $f$} iff $t$ has the form \\[0.2cm]
...
\noindent
The \blue{halting problem} for \textsl{Python} functions is the question whether there is a
...
A \blue{Turing maschine} can be interpreted as a formal description of an algorithm.  
...
whether there are infinitely many \blue{twin primes}.  A \blue{twin prime} is pair of natural numbers
...
\exerciseEng
A set $X$ is called \blue{countable}  iff there is a function 
\\[0.2cm]
\hspace*{1.3cm}
...
\blue{partially equivalent}. \hspace*{\fill} $\Box$
\end{Definition}
