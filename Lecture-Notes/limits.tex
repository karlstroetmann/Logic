...
Second, we prove that it is impossible to atomatically check whether two functions are equivalent.
...
had some procedure that could check whether the function $\texttt{find\_counter\_example}(1)$ does terminate,
...
A string $t$ is a \emph{test function with name $f$} iff $t$ has the form \\[0.2cm]
...
\noindent
The \emph{halting problem} for \textsl{Python} functions is the question whether there is a
...
then the halting problem for \textsl{Python} would be decidable.
...
\noindent
We are now ready to state the \emph{equivalence problem}.  A \textsl{Python} function \texttt{equal} solves the
