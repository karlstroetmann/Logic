...
In order to do this, we first need the notion of a \blue{substitution}.
...
\begin{Definition}[Substitution]\index{Substitution}
    ...
    We also define the \blue{domain} of a substitution as \\[0.2cm]
    \hspace*{1.3cm} $\textsl{dom}(\sigma) := \{ x_1, \cdots, x_n\}$.
    \\[0.2cm]
    We denote the set of all substitutions by \blue{$\textsl{Subst}_\Sigma$}.\index{Subst}
    \eox
\end{Definition}

\noindent
Substitutions become interesting for us because we can \blue{apply} them to terms.  If $t$ is a
...
is a substitution.  We define the \blue{application}\index{application of a substitution} of $\sigma$ to $t$
(notation \blue{$t\sigma$}) \index{$t\sigma$} by induction on the term $t$: 
\begin{enumerate}
...
Just as we can apply substitutions to a term, we can also apply a substitution
...
\begin{Definition}[Composition of Substitutions] \index{composition of substitutions}
    ...
    we define the \blue{composition} $\sigma\tau$ \index{$\sigma\tau$} of  $\sigma$ and $\tau$ as \\[0.2cm]
...
\begin{Proposition} \label{set:composition}
...
\begin{Definition}[Syntactic Equation] \index{syntactic equation}
In this section we define a \blue{syntactic equation} to be an expressions of the form
$s \doteq t$, \index{$s \doteq t$} such that one of the following holds:
\begin{enumerate}
\item $s$ and $t$ are terms or
\item $s$ and $t$ are atomic formulas.
\end{enumerate}
Further, we define a \blue{system of syntactic equation} as a set
of syntactic equations. \index{system of syntactic equations}
\eox
\end{Definition}
