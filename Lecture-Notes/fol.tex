...
\end{enumerate}
The predicate \textsl{kind}$(x,y)$ is true if and only if $x$ is a child of $y$.
...
\item We apply the resolution rule to the clause $\bigl\{\textsl{rot}(s(d))\bigl\}$ and the clause $k_3$. First, we have
      \\[0.2cm]
      \hspace*{1.3cm}
      $\mathtt{mgu}\bigl(\textsl{rot}(s(d)), \textsl{rot}(x)\bigr) = [x \mapsto s(d)]$
      \\[0.2cm]
...
\item To resolve the clause $\bigl\{\textsl{fliegt}(s(d))\bigl\}$ with the clause $k_2$, we compute
      \\[0.2cm]
      \hspace*{1.3cm}
...
\item We now apply the resolution rule to the result $\bigl\{\textsl{glücklich}(d)\bigl\}$ and the clause $k_6$:
      \\[0.2cm]
      \hspace*{1.3cm}
