...
In order to apply the equivalences of the last group, it may be necessary to rename bound variables. If $f$ is
a first-order logic formula and $x$ and $y$ are two variables, where $y$ does not appear in $f$, then
\blue{$f[x \mapsto y]$} denotes the formula that results from $f$ by replacing every occurrence of the variable
$x$ in $f$ with $y$. For example, \\[0.2cm] 
\hspace*{1.3cm} $\bigl(\forall u : \exists v : p(u,v)\bigr)[u \mapsto z] = \forall z : \exists v : p(z,v)$
\\[0.2cm]
With this we can specify one last equivalence:
\begin{enumerate}
\item[9.] If $f$ is a first-order logic formula, $x \in BV(f)$ and $y \not\in FV(f) \cup BV(f)$, then \\[0.2cm] 
          \hspace*{1.3cm} $\models f \leftrightarrow f[x \mapsto y]$.
\end{enumerate}
...
we introduce another concept of equivalence.  We want to motivate this concept via an example.
...
return one such $y$ for every $x$. 

\begin{Definition}[{\color{blue}Skolemisation}]\index{Skolemisation} \hspace*{\fill} \linebreak
  Assume that $\Sigma = \langle \mathcal{V}, \mathcal{F}, \mathcal{P}, \textsl{arity} \rangle$
  is a signature and that $f$ is a closed $\Sigma$ formula of the form \\[0.2cm]
  \hspace*{1.3cm} 
  $f = \forall x_1, \cdots, x_n \colon \exists y \colon g$. \\[0.2cm]
  Then we choose an \underline{\blue{new}} $n$-ary function character $s$, i.e.~we take a character $s$ that
  is not used in the  signature $\Sigma$ and extend the signature $\Sigma$ to the signature \\[0.2cm]
  \hspace*{1.3cm} 
  $\Sigma' := \Bigl\langle \mathcal{V}, \mathcal{F} \cup \{s\}, \mathcal{P}, \textsl{arity} \cup \bigl\{ s \mapsto n \bigr\} \bigr\rangle$, \\[0.2cm]
  where $s$ is declared as a new $n$-ary function character.  Furthermore, we define the $\Sigma'$ formula
  $f'$ as follows: \\[0.2cm]
  \hspace*{1.3cm} 
  $f' := \mathtt{Skolem}(f) := 
  ...
  $s(x_1,\cdots,x_n)$.  We say that the formula $f'$ results from the formula $f$ by a \blue{Skolemisation step}. 
  \eox
\end{Definition}

\exampleEng
Let $f$ be the following formula from group theory:
\\[0.2cm]
\hspace*{1.3cm}
...
function characters is called \blue{Skolemisation}.  If we have put a formula $F$
into prenex normal form and then successively remove all existential quantifiers by the process of
Skolemisation, the resulting formula has the form\\[0.2cm] 
...
\blue{matrix}\index{matrix} of the above formula.  If the original formula $F$ had no free variables, then all
remaining variables are universally quantified and we lose no information when we drop the universal quantifiers.
We can then transform $g$ with the help of
of the propositional equivalences known from the last chapter into conjunctive normal form.  The resulting 
formula will have the form \\[0.2cm]
\hspace*{1.3cm} $k_1 \wedge \cdots \wedge k_m$
\\[0.2cm]
where the formulas $k_i$ are disjunctions of atomic formulas.
If a formula has this form, we say that is in \blue{first-order clause normal form}
\index{first-order clause normal form} and a formula 
 $k$ where $k$ is a disjunction of first-order literals,
...
satisfiability-equivalent set of first-order clauses.
\vspace*{0.2cm}
