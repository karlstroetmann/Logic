...
\begin{Definition}[Symbolic Transition System]
  ...
  \item \texttt{Transition} is a \textsc{Fol} formula.  The free variables of this formula are elements of
        ...
        or they are primed variables from the set $\mathtt{Vars}'$.  The primed variables refer to the values
        of the variables after the transition.
  \end{enumerate}
\end{Definition}
Every \blue{state} of a transition system is a mapping of the variable to values.
The idea is that that formula \texttt{Start} describes the start state of our search problem, \texttt{Goal}
describes the state that we want to reach, while \texttt{Invariant} is a formula that must be true initially
and that remains after every transition of our system.  The formula \texttt{Transition} describes how the
variables in the transition system change during a transition.

In order to clarify this definition we show the \emph{missionaries and cannibals} problem can be formulated as
...
\item $\texttt{Goal}  := (\mathtt{M} = 0, \mathtt{C} = 0 \}$.
\item $\texttt{Invariant} := (\mathtt{M} = 3 \vee \mathtt{M} = 0 \vee \mathtt{M} = C \}$,

      since there is no problem when all missionaries are on the same shore or when the number of missionaries
      is the same as the number of infidels on one shore, because then these numbers have to agree on the
      other shore as well.
\item $\texttt{Transition} :=
      \begin{array}[t]{l}
        0 \leq \mathtt{B} \leq 1 \;\wedge\; \mathtt{B}' = 1 - \mathtt{B}  \;\wedge\; \\[0.2cm]
        \bigl(\mathtt{B} = 1 \;\rightarrow\; 1 \leq \mathtt{M} - \mathtt{M}'  + \mathtt{C} - \mathtt{C}' \leq 2 \;\wedge\;
        \mathtt{M}' \leq \mathtt{M} \;\wedge\; \mathtt{C}' \leq \mathtt{C}\bigr) \\[0.2cm]
        \bigl(\mathtt{B} = 0 \;\rightarrow\; 1 \leq \mathtt{M}' - \mathtt{M}  + \mathtt{C}' - \mathtt{C} \leq 2 \;\wedge\;
      ...
      \begin{itemize}
      \item $0 \leq \mathtt{B} \leq 1$

            expresses the fact that there is just one boat.  This boat is either on the western shore or on the
            eastern shore.
      \item $\mathtt{B}' = 1 - \mathtt{B}$
            ...
            shore afterwards, i.e. we will then have $\texttt{B}' = 1$.
      \item $\mathtt{B} = 1 \;\rightarrow\; 1 \leq \mathtt{M} - \mathtt{M}'  + \mathtt{C} - \mathtt{C}' \leq 2 \;\wedge\;
            ...
            infidels will decrease, as they leave for the eastern shore.  Then $\texttt{M} - \texttt{M}'$
      ...
      \item $\mathtt{B} = 0 \;\rightarrow\; 1 \leq \mathtt{M}' - \mathtt{M}  + \mathtt{C}' - \mathtt{C} \leq 2 \;\wedge\;
            ...
            This formula describes the transition from the eastern shore to the western shore.
      \end{itemize}
\end{enumerate}
...
\begin{Verbatim}[frame         = lines, 
    ...
    def transition(M$\alpha$, C$\alpha$, B$\alpha$, M$\beta$, C$\beta$, B$\beta$):
        if not (0 <= B$\alpha$ <= 1 and B$\beta$ == 1 - B$\alpha$):
            return False
        if B$\alpha$ == 1:
...
represented as a symbolic transition system in \textsl{Python}.  Since varibale cannot be primed in
\textsl{Python} we use the convention that we append the character $\alpha$ to the names of the original
variables from the set \texttt{Vars}, while we append $\beta$ to these names to get the primed versions of the
corresponding variable.

Figure \ref{fig:Missionaries-STS.ipynb-2} shows how we can turn the symbolic transition system into a \textsc{Csp}.
