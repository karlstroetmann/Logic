...
in 63 moves.  The tour starts in one of the corners of the board.  
...
\begin{minted}[ frame         = lines, 
    ...
    def all_variables():
        Variables = set()
        for i in range(63+1):
            Variables.add(row(i)); Variables.add(col(i))
        return Variables
    ...
    def all_constraints():
        ...
        for i in range(63+1):
            Constraints.add(f'{row(i)} >= 0'); Constraints.add(f'{col(i)} >= 0')
        return Constraints
\end{minted}
\vspace*{-0.3cm}
...
\item It is not convenient to declare all of the 128 variables $\mathrm{R}_i$ and $\mathtt{C}_i$ for $i=0,1,\cdots,63$
...
\item The function \texttt{is\_knight\_move} checks whether the move from position 
...
\item The function \texttt{all\_constraints} computes the set of all constraints.
      ...
      its tour at the leftmost topmost corner of the board.
...
that has been found.
\begin{enumerate}
\item In line 2 we define the dictionary \texttt{Environment}  which will serve as the local environment for
      the functions \texttt{exec} and \texttt{eval} below.
\item We import the package \texttt{z3} into this environment in line 3.
\item Then we declare in line 5 that the strings from the set \texttt{Variable} represent \texttt{Z3} bit-vector
...
\item We create a solver object in line 6 and add the constraints to this solver in the following two lines.
      ...
      The constraints \texttt{c} in the set \texttt{Constraints} are just strings.  
...
\\[0.2cm]
\hspace*{1.3cm}
\href{https://github.com/karlstroetmann/Logic/blob/master/Python/Chapter-5/Sudoku-Z3-Frame.ipynb}{https://github.com/karlstroetmann/Logic/blob/master/\\
      \hspace*{2.6cm}  
          Python/Chapter-5/Sudoku-Z3-Frame.ipynb}
    \eox
