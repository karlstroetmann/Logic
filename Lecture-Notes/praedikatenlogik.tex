...
Die Prädikatenlogik untersucht zusätzlich auch die Struktur dieser elementaren Aussagen.  Dazu werden in der
...
\begin{Definition}[Semantik der Terme]
    ...
    so definieren wir für jeden Term $t$ den \blue{Wert} $\struct(\mathcal{I}, t)$ durch Induktion über den Aufbau
    von $t$:
    \begin{enumerate}
...
\begin{Definition}[äquivalent]
  ...
  \\[0.2cm] 
  \hspace*{1.3cm} $\models \forall x_1: \cdots\, \forall x_n: (F \leftrightarrow G)$
  \\[0.2cm] 
  gilt.
  \eox
\end{Definition}
...
$\{ a, b \}$ besteht.  In \textsc{SetlX} können wir diese Struktur durch den in Abbildung
\ref{fig:gruppen.stlx} gezeigten Code implementieren.
...
\item In Zeile 6 fassen wir die einzelnen Interpretationen zu der Relation $\texttt{J}$
      ...
      Da wir später den in \textsc{SetlX} eingebauten Parser verwenden wollen, müssen wir die
...
Die Implementierung dieser Prozedur ist in \myfig{modify.stlx} gezeigt.
 Bei der Auswertung eines All-Quantors können wir ausnutzen, dass die Sprache \textsc{SetlX}
...
Zeile 7, ob diese Liste tatsächlich ein Element der Menge $\mathtt{pJ}$ ist.
...
in Abbildung \ref{fig:gruppen.stlx} gezeigte Struktur die Axiome einer
...
\item Zeigen Sie, dass die Formel
      ...
      \\[0.2cm]
      \underline{nicht} allgemeingültig ist, indem Sie in \textsc{SetlX} eine geeignete
...
Formeln, sogenannte \blue{Klauseln}, beschränken.  Wir zeigen in diesem Abschnitt, dass jede Formel-Menge $M$
...
eine Substitution. Wir definieren die \blue{Anwendung} von $\sigma$ auf $t$ (Schreibweise $t\sigma$) durch Induktion über
den Aufbau von $t$: 
\begin{enumerate}
...
aufwendiger als der Kalkül, den wir gleich einführen werden,  denn der Kalkül für die Prädikatenlogik ohne
...
\\[0.2cm]
Dies zeigt, dass automatische Theorem-Beweiser durchaus nützliche Werkzeuge sein
können.  Nichtsdestoweniger ist die Prädikatenlogik unentscheidbar und bisher sind 
