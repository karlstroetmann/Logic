...
The idea is that that the formula \texttt{Start} describes the start state of our search problem, \texttt{Goal}
describes the state that we want to reach, while \texttt{Invariant} is a formula that must be true initially
and that has to remain true after every transition of our system.  The formula \texttt{Transition} describes how the
variables in the transition system can change during a transition.
...
\item $\texttt{Transition} :=
      ...
      \item $0 \leq \mathtt{B} \leq 1$
            ...
            eastern shore.  If it it on the western shore, $\mathtt{B} = 1$, otherwise $\mathtt{B} = 0$. 
