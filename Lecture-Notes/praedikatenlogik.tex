...
denote equality. 
We will use a parser for predicate logic formulas that does not support binary infix operators like
...
\subsection{Representation of Predicate Logic Structures in \textsl{Python}}
...
\item The interpretation $f^\mathcal{J}$ of this function symbol is looked up in line 6 in the dictionary
      $\mathcal{J}$.
\item The tuple consisting of the arguments of $f$ is recursively evaluated in line 7.
...
\\[0.2cm]
holds. This test is conducted in line 6. The rest of the implementation of the function
$\mytt{evalAtomic}$ is analogous to the implementation of the function $\mytt{evalTerm}$.
...
handling of quantifiers. In lines 10, 11, and 12, we deal with the evaluation of universally quantified formulas.
If $F$ is a formula of the form $\forall x: G$, then the formula $F$ is represented by the tuple
\\[0.2cm]
\hspace*{1.3cm}
$F = (\mytt{'∀'}, x, G)$.
\\[0.2cm]
The evaluation of $\forall x\colon G$ is implements the formula
\\[0.2cm]
\hspace*{1.3cm}
...
$
\\[0.2cm]
To implement the this, we use the procedure $\mytt{modify}()$, which modifies the
variable assignment $\mathcal{I}$ at position $x$ to $c$, hence
\\[0.2cm]
\hspace*{1.3cm}
$\mytt{modify}(\mathcal{I},x,c) = \mathcal{I}[x/c]$. 
\\[0.2cm]
The implementation of this procedure is shown in \myfig{modify.ipynb}.
...
The evaluation of an existential quantifier is analogous to the evaluation of a universal quantifier.
The only difference is that instead of the function \mytt{all}, we use the function \mytt{any}.
The expression
\\[0.2cm]
\hspace*{1.3cm}
$\mytt{any}(S)$
\\[0.2cm]
is true for a set of truth values $S$ exactly when there is at least one element in the set $S$
that is \mytt{True}.
...
\myfig{isGroup.ipynb} is a group. The output shown in Figure \ref{fig:isGroup.out} allows us to conclude that
this structure is indeed a commutative group. 
...
\item Show that the formula
      ...
      In order to do this, implement a suitable predicate logic structure $\mathcal{S}$ in \textsl{Python}
...
\item Provide a \underline{satisfiable} first-order formula $F$ that is always false in a predicate logic
      structure $\mathcal{S} = \pair(\mathcal{U},\mathcal{J})$ if  the universe $\mathcal{U}$ is finite.
