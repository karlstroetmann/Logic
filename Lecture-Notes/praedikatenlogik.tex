...
A \blue{group} is a triple $\mathcal{G} = \langle G, \mathrm{e}, \circ \rightarrow$
such that
\begin{enumerate}
\item $G$ is a set.
...
\end{enumerate}
We will show both with the help of \textsl{Vampire}.  Figure \ref{fig:group-right-identity.tptp} on page
...
\begin{Verbatim}[ frame         = lines, 
    ...
    fof(left_e, conjecture, ! [X] : mult(X, e) = X).
\end{Verbatim}
\vspace*{-0.3cm}
...
\item Line 1 states the axiom $\forall x: \mathrm{e} \circ x = x$.  Every formula is written in the form
      ...
      \item \textsl{type} is either the string ``\texttt{axiom}'' or the string ``\texttt{conjecture}''.
            Every file must hold exactly one conjecture.  The conjecture is the formula that is to be proven from the
            axioms.
...
\\[0.2cm]
\hspace*{1.3cm}
\texttt{fof(left\_inverse, conjecture, ![X]: ?[Y]: mult(X, Y) = e).}
...
\exerciseEng
Using \textsl{Vampire} show that in every group the left inverse is unique. \eox
...
\noindent
The question then is: Who killed Agatha?  As there are only three possibilities, we try to prove the following
...
successful.  Hence we must conclude that Agatha committed suicide. 
