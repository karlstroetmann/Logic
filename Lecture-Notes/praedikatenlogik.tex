...
\item In Zeile 2 definieren wir die Interpretation des nullstelligen Funktions-Zeichens $\texttt{E}$
      als das \textsl{Python}-Dictionary, das dem leeren Tupel die Zahl $0$ zuordnet.
\item In Zeile 3 definieren wir eine Funktion $\texttt{Product}$ als \textsl{Python}-Dictionary.  Für
...
\item In Zeile 4 haben wir  die Interpretation $\texttt{Equals}^\mathcal{J}$ des
      Prädikats-Zeichens ``$\texttt{Equals}$'' als die Menge $\{ \pair(0,0), \pair(1,1)\}$ definiert.
...
\item Die Interpretation $\texttt{J}$ wird dann in Zeile 6 mit dem
...
\item Das Funktions-Zeichen $f$ des Terms $t$ ist die erste Komponente des Tupels $t$,
      die Argumente werden in der Liste \texttt{args} zusammen gefasst.
...
\texttt{$p$, *args = $a$}
\\[0.2cm]
in seine Komponenten zerlegen.  \texttt{args} ist dann die Liste $[t_1,\cdots,t_n]$.
...
\end{minted}
\vspace*{-0.3cm}
\caption{Die Funktion \texttt{evalFormula}.}
...
\\[0.2cm]
A variable assignment $\mathcal{I}$ is a \blue{solution} of the \textsc{Csp} $\mathcal{P}$ \index{solution of \textsc{Csp}}
...
\begin{Definition}[{\color{blue}Skolemisierung}] \hspace*{\fill} \linebreak
