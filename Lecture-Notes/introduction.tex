...
we have discussed three important properties that an algorithm should have:
...
\begin{enumerate}
\item A stark example of \gls{it} failure consequences is the
...
\item Between 1999 and 2015 the British Post Office used a faulty accounting software provided by Fujitsu.
      ...
      convicted of embezzlement.  As a consequence, some of these employees were imprisoned.  Four of those
      that were falsely convicted committed suicide.  These tragic incidents are known as the
...
\item Logic is used to build interactive theorem provers that are able to establish the correctness of
      software.  For example, \textsl{Microsoft}\textsuperscript{TM} has built the
...
\item The correctness of digital circuits can be verified using \blue{automatic theorem provers} that are based on
      ...
      Personally, I have worked on hardware verification at the Siemens Research Center up to 2002.  Their current product
      is called \href{https://www.siemens.com/en-us/products/ic/questa-one/formal-verification/}{Questa One}.
\end{enumerate}
...
good computer scientist.  However, computer science is much more than just programming.
...
In computer science, a good programmer need not be a scientist at all, while a
{\color{blue}computer \underline{scientist}}, by its very name, is a {\color{blue}scientist}.  
...
\item The third chapter discusses two different methods that can be used to prove the correctness of a program:
      ...
      \item \blue{Computational induction} is the method of choice for proving the correctness of recursive
            algorithms.
      \item \blue{Symbolic execution} is used to verify iterative algorithms.
      \end{itemize}
\item The fourth chapter discusses \href{https://en.wikipedia.org/wiki/Propositional_calculus}{propositional logic}.
      ...
      We will use it to solve various logical puzzles.  In particular, we solve the
      \begin{enumerate}
      \item the \href{https://en.wikipedia.org/wiki/Eight_queens_puzzle}{8 queens puzzle}, and
      \item a \href{https://en.wikipedia.org/wiki/Sudoku}{Sudoku puzzle}.
      \end{enumerate}
\item Next, we discuss \href{https://en.wikipedia.org/wiki/First-order_logic}{first-order logic}.

      We begin by defining \gls{fol} formulas and their meaning.  Next, we introduce \glspl{csp}
      and show to use these problems via \blue{backtracking}.  Then we show how to formalise certain problems
      as \glspl{csp} problems and then solve them.  In particular, we will discuss the following problems:
      \begin{enumerate}
      \item the eight queens problem,
      \item the \href{https://en.wikipedia.org/wiki/Zebra_Puzzle}{Zebra Puzzle},
      \item the \href{https://en.wikipedia.org/wiki/Missionaries_and_cannibals_problem}{missionaries and
            cannibals problem} and similar search problems.
      \end{enumerate}
      To this end we will introduce \texttt{Z3}, which is a high performance constraint solver developed by
      Microsoft.
      
      
\item In the last chapter,      
      the most important concept will be the notion of a \blue{formal proof} in
...
\begin{itemize}
\item Is it possible that a coding mistake in an accounting software can kill people?
\item Provide three examples of faulty software systems that have caused great harm.
      Do not cite the examples given in this Chapter but rather search the web to find more examples.
  
\item Provide two examples of automatic theorem provers that are used in industry.
\item Is first order logic more expressive than propositional logic or is it the other way around?
\end{itemize}
