...
\item $\texttt{sin}(x)$ computes the \href{https://en.wikipedia.org/wiki/Sine}{sine} of $x$.
      Furthermore, the \blue{trigonometrical} functions $\texttt{cos}(x)$ and $\texttt{tan}(x)$ are supported.  The
      \blue{inverse} trigonometrical functions are written as 
...
\end{enumerate}
\textsc{SetlX} supports \blue{unlimited precision} via rational numbers.  For example, the expression
\\[0.2cm]
\hspace*{1.3cm}
\texttt{1/2 + 1/3;}
\\[0.2cm]
