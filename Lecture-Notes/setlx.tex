...
\begin{enumerate}
\item ``\texttt{!}''  represents the logical \blue{negation}.
...
\item ``\texttt{\&\&}'' represents the logical \blue{conjunction}.
...
\item ``\texttt{||}''  represents the logical \blue{disjunction}.
...
\end{enumerate}
Syntactically, the operator ``\texttt{||}'' has the lowest \blue{precedence}, while the \blue{precedence} of
the operator ``\texttt{!}'' is higher than the \blue{precedence} of ``\texttt{\&\&}'' and ``\texttt{||}''.
...
\section{Strings}
\setlx\ support \blue{strings}.  In  \textsc{SetlX} these have to be enclosed either in double quotes or in
...
as the \blue{preflop} or the \blue{hole}.  Next, there is a bidding phase where players can bet on their
...
vertex.  This path is then called a \colorbox{amethyst}{\blue{cycle}}.
