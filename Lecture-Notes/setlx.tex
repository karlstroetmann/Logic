...
In the following section, I wil present an application that is practically relevant.  In order to prepare for this,
this Section discusses the problem of finding a path in a
...
\\[0.2cm]
Whne first encountered, this formula might look intimidating as it suggests an infinite computation.
Fortunately, it turns out that we do not have to compute all powers of the form $R^i$.  Let me
...
For example, in this section we will1 develop a program that reads a string like 
\\[0.2cm]
\hspace*{1.3cm}
``\texttt{x * exp(x)}'',
\\[0.2cm]
...
In order to build terms, we first need \emph{functors}.  It is important not to confuse functors with
...
\\[0.2cm]
\hspace*{1.3cm}
\texttt{\symbol{64}f}, \quad \texttt{\symbol{64}FabcXYZ}, \quad \texttt{\symbol{64}sum}, \quad \texttt{\symbol{64}Hugo\_}.
\\[0.2cm]
...
not a function symbol but rather a functor.  Next, we define terms.  If $F$ is a functor and 
...
\examplesEng
\begin{enumerate}
\item \texttt{\symbol{64}Address(\symbol{34}Coblitzallee 1-9 \symbol{34}, 68163, \symbol{34}Mannheim\symbol{34})}

      is a term that represents an address.
\item \texttt{\symbol{64}product(\symbol{64}variable(\symbol{34}x\symbol{34}), \symbol{64}exp(\symbol{64}Variable(\symbol{34}x\symbol{34})))}
...
\\[0.2cm]
\hspace*{1.3cm}
$\texttt{\symbol{64}nil}() \;\widehat{=}\; \texttt{[]}$,
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$\texttt{\symbol{64}cons}(x, r) \;\widehat{=}\; \texttt{[}x\texttt{]} + r$. 
\\[0.2cm]
Concretely, the list \texttt{[1,2,3]} is represented as the term
\\[0.2cm]
\hspace*{1.3cm}
\texttt{\symbol{64}cons(1, \symbol{64}cons(2, \symbol{64}cons(3, \symbol{64}nil())))}.
\\[0.2cm]
...
\item The function \texttt{fct} returns the functor of a given term.
      ...
      \\[0.2cm]
      \hspace*{1.3cm}
      \texttt{fct(\symbol{64}cons(1, \symbol{64}cons(2, \symbol{64}cons(3, \symbol{64}nil()))))}
      \\[0.2cm]
...
\end{enumerate}
The function \texttt{diff} interprets its argument \texttt{t} as a function of the variable \texttt{x}.  This
function is then differentiated with respect to \texttt{x}.  For example, in order to compute the derivative of
the function
\\[0.2cm]
\hspace*{1.3cm}
$x \mapsto x^x$,
