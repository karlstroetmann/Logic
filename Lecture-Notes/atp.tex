...
\blue{\textsc{Fol} conjunctive normal form} (abbreviated as \textsc{Fol-Cnf}) provides a standardized
...
\item A \blue{clause} is a disjunction of one or more literals. For example,
      ...
      As we have done in propositional logic, we will use set notation to represent clauses.
\item A formula in \blue{\textsc{Cnf}} is a conjunction of clauses.

      Again, we will use set notation and represent the formula as a set of its clauses
...
\blue{matrix}.  Every \textsc{Fol} formula in classical logic is logically equivalent to a formula in
\blue{prenex normal form} \index{prenex normal form} (abbreviated as \textsc{Pnf}). 
...
\blue{equisatisfiability} \index{equisatisfiability}. The original formula and the Skolemized formula are
...
$2^n$ clauses. However, for many practical applications, the worst case does not occur. 
...
\noindent
The Jupyter notebook linked below contains a \textsl{TypeScript} programm that we can use to transform predicate
logic formulae into a satisfiability-equivalent set of first-order clauses. 
\\[0.2cm]
\hspace*{1.3cm}
\href{https://github.com/karlstroetmann/Logic/blob/master/TypeScript/Chapter-6/01-FOL-CNF.ipynb}{Chapter-6/01-FOL-CNF.ipynb}.
\vspace*{0.2cm}

\subsection{Refutational Theorem Proving}
Next, our goal is to develop a method that can be used to verify that a first-order formula
$f$ that contains no free variables is universally valid, i.e.~we want to be able to prove that \\[0.2cm]
\hspace*{1.3cm}
$\models f$
\\[0.2cm]
...
because the closed formula $f$ is universally valid iff there is no $\Sigma$-structure $\mathcal{S}$ such that
\\[0.2cm]
\hspace*{1.3cm}
$\mathcal{S} \models \neg f$,
\\[0.2cm]
i.e.~$f$ is universally valid iff $\neg f$ is unsatisfiable. 
Therefore, in order to prove that $f$ is universally valid, we transform the formula $\neg f$  into 
...
\item If $y\in\mathcal{V}$ and  $t \not\in \mathcal{V}$, then we have:
      ...
      \Big\langle E \cup \big\{ y \doteq t \big\},\; \sigma \Big\rangle$ \qquad
      \mbox{if $y \in \mathcal{V}$ and $t \not\in \mathcal{V}$}.   
      \\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
\href{https://github.com/karlstroetmann/Logic/blob/master/TypeScript/Chapter-6/02-Unification.ipynb}{Chapter-6/03-Unification.ipynb}.
\\[0.2cm]
...
The resolution rule by itself is not sufficient to derive the empty clause from a clause set $M$ that is
...
\begin{enumerate}
\item[4.] all red dragons are happy.
\end{enumerate}
