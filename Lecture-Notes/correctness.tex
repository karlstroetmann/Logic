...
\item The \blue{induction step}.
      ...
      assume that the results computed by the recursive invocations are correct.
      This assumption is called the \blue{induction hypothesis}.
...
\item \textbf{Induction step}:
      ...
      $\mathtt{power}(m, \lfloor n / 2 \rfloor)$.  By the induction hypothesis we may assume that
      \\[0.2cm]
      \hspace*{1.3cm}
...
$m$ is written with $k$ bits $b_0$, $b_1$, $\cdots$, $b_{k-1}$ where $b_0$ is the least significant bit, then we have
\\[0.2cm]
\hspace*{1.3cm}
...
\item[I.S.:] $\lfloor m / 2 \rfloor \mapsto m$
  ...
  By induction hypothesis we know that our claim is true for the recursive invocation of \texttt{divMod} in
...
\blue{computational induction}.  If a function is instead implemented via loops,
then the method of computational induction is not applicable.   Instead, the method of 
\blue{symbolic execution} \index{symbolic execution} is used.
...
\begin{Verbatim}[ frame         = lines, 
    ...
    \colorbox{sepia}{    \green{let} r$_0$ = 1n;}
...
\end{Verbatim}
\vspace*{-0.3cm}
\caption{An annotated program to compute powers.}
...
$\texttt{r}_N$ in line 10, where $N$ denotes the total number of times that the test \texttt{y > 0} has been executed.
...
\item[I.S.:] $n \mapsto n + 1$.
            ...
            Our proof proceeds by a case distinction with respect to the expression $y_n \mmod 2$:
            \begin{enumerate}
            \item $y_n \mmod 2 = 1$.

                  Then we have $y_{n} = 2 \cdot \lfloor y_n /  2\rfloor + 1$, 
                  $r_{n+1} = r_n \cdot x_n$ and $y_{n+1} = \lfloor y_n / 2\rfloor$.  Hence
                  
                  \begin{eqnarray*}
                      &   & r_{n+1} \cdot x_{n+1}^{y_{n+1}} \\[0.2cm] 
                      & = & (r_{n} \cdot x_n) \cdot (x_{n} \cdot x_{n})^{\lfloor y_{n}/2 \rfloor} \\[0.2cm] 
                      & = & r_{n} \cdot x_n^{2 \cdot \lfloor y_{n}/ 2\rfloor + 1} \\[0.2cm]
                      & = & r_{n} \cdot x_n^{2 \cdot \lfloor y_{n}/ 2\rfloor + y_n \mmod 2} \\[0.2cm] 
                      & = & r_{n} \cdot x_n^{y_n} \\
            ...
            \item $y_n \mmod 2 = 0$.
                  ...
                  \begin{eqnarray*}
                      ...
                      & = & r_{n} \cdot x_n^{2 \cdot \lfloor y_{n} / 2\rfloor + 0} \\[0.2cm]
                      & = & r_{n} \cdot x_n^{2 \cdot \lfloor y_{n} / 2\rfloor + y_n \mmod 2} \\[0.2cm] 
                      & = & r_{n} \cdot x_n^{y_n} \\
                      & \stackrel{i.h.}{=} & a^{b} \quad \text{\mycheck}
                  \end{eqnarray*}
...
$$
\begin{array}[t]{cll}
                 & r_N \cdot x_N^{y_N} = a^b \\[0.2cm]
\Leftrightarrow  & r_N \cdot x_N^{0}  = a^b \\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$y_{n+1} = \lfloor y_n / 2 \rfloor < y_n$ \quad as long as \quad $y_n > 0$
\\[0.2cm]
...
\blue{integer square root} of its argument $a$ as long as $a$ is less than $2^{128\,000}$, i.e.~if $a$ is represented as an unsigned
number, then it can be represented with $128\,000$ bits.  In the implementation of the function \texttt{root} we have
...
program, we first 
have to establish invariants for the variables $p_k$ and $r_k$.  In order to be able to formulate the invariant
...
\\[0.2cm]
\hspace*{1.3cm}
$\ds \mathtt{isqrt}(a) = \sum\limits_{i=0}^{n} b_i \cdot 2^i$, \quad where $b_i \in \{0,1\}$
\\[0.2cm]
and $n+1$ is the number of bits that are needed to represent $\mathtt{isqrt}(a)$.  Note that then the binary
representation of the number $a$ needs at most twice as much bits.  In the program below we have $n+1 = 64\,000$.
Therefore, we assume that the argument $a$ of $\mathtt{isqrt}$ is less or equal than $2^{2\cdot (n+1)}$,
i.e. we assume that
\\[0.2cm]
\hspace*{1.3cm}
$a \leq 2^{128\,000}$,
\\[0.2cm]
since then we are sure that we can represent $\mathtt{isqrt}(a)$ with $64,000$ bits.
In order to compute $\mathtt{isqrt}(a)$ we start with the last bit $b_{n}$.  If the square of $2^{n}$ is
less than or equal to $a$, then we must have $b_{n} = 1$.  Otherwise we have $b_{n} = 0$.  Assume now that
we have already computed the bits $b_{n}$, $b_{n-1}$, $\cdots$, $b_{n-k}$.  In order to compute $b_{n-(k+1)}$
we have to check whether 
\\[0.2cm]
\hspace*{1.3cm}
$\ds\left(\ds 2^{n-(k+1)} + \sum\limits_{i=n-k}^{n} b_i \cdot 2^i \right)^2 \leq a$.
\\[0.2cm]
If it is, then  $b_{n-(k+1)} = 1$, else $b_{n-(k+1)} = 0$.
With this in mind, we can state the invariants that hold when the \texttt{while} loop in line 4 is entered:
\begin{enumerate}
\item $\ds p_k = 2^{n - k}$ \quad for $k=0,\cdots,n$.
\item $\ds r_k = \sum\limits_{i=0}^{k-1} b_{n-i} \cdot 2^{n-i}$ \quad where the $b_i$ are the bits of the
       binary representation of $\mathtt{isqrt}(a)$.
\end{enumerate}
...
\begin{Verbatim}[ frame         = lines, 
    ...
    \colorbox{sepia}{\green{function} \blue{root}(a: bigint): bigint \{ }
    \colorbox{sepia}{    let r$_0$ = 0n;                   }
    \colorbox{sepia}{    let p$_0$ = 2n ** 64\_000n;        }
    \colorbox{sepia}{    \green{while} (p$_k$ > 0) \{               }
    \colorbox{sepia}{        \green{if} (a >= (r$_k$ + p$_k$) ** 2n) \{}
    \colorbox{sepia}{            r$_{k+1}$ = r$_k$ + p$_k$;         }
    \colorbox{sepia}{        \}                          }
    \colorbox{sepia}{        p$_{k+1}$ = p$_k$ / 2n;             }
    \colorbox{sepia}{    \}                              }
    \colorbox{sepia}{    \green{return} r$_K$;                     }
    \colorbox{sepia}{\}                                  }
\end{Verbatim}
\vspace*{-0.3cm}
\caption{An annotated program to compute the integer square root.}
...
\item[I.S.:] $k \mapsto k + 1$
  ...
  $p_{k+1} = \lfloor p_k / 2 \rfloor \;\stackrel{IV}{=}\;
     \bigl\lfloor 2^{n-k} / 2 \bigr\rfloor \;=\; 2^{n-(k+1)}$.

   This holds as long as $k+1 \leq n$.  Since we have $p_{n} = 2^{n-n} = 2^0 = 1$ it follows that
   \\[0.2cm]
   \hspace*{1.3cm}
   $p_{n+1} \;=\; \lfloor p_{n} / 2 \rfloor = \lfloor 1 / 2 \rfloor = 0$.
   \\[0.2cm]
   This shows that the body of the \texttt{while} loop is executed $n+1$ times.  This was to be expected, as
   each execution of this loop computes one bit of the result.
\end{enumerate}
We proceed to prove the second invariant $\ds r_k = \sum\limits_{i=0}^{k-1} b_{n-i} \cdot 2^{n-i}$ by induction.
\begin{enumerate}
\item[B.C.:] $k = 0$
  ...
  \\[0.2cm]
  \hspace*{1.3cm}
  $\sum\limits_{i=0}^{0-1} b_{n-i} \cdot 2^{n-i} = 0$,
  \\[0.2cm]
  since the sum $\sum\limits_{i=0}^{-1} \cdots$ is empty and hence has the value $0$.
\item[I.S.:] $k \mapsto k+1$

  Now we need to perform a case distinction with respect to the test $a \geq (r_k + p_k)^2$.
  \begin{enumerate}
  \item $a \geq (r_k + p_k)^2$.

    In this case we have that $r_k + p_k \leq \mathtt{isqrt}(a)$ and since $p_k = 2^{n-k}$ this shows that
    \\[0.2cm]
    \hspace*{1.3cm}
    $r_k + 2^{n-k} \leq \mathtt{isqrt}(a)$.
    \\[0.2cm]
    ...
    $
    \begin{array}[t]{lcl}
      r_{k+1} & = & r_k + p_k \\[0.2cm] 
              & \stackrel{IV}{=} & \ds\sum\limits_{i=0}^{k-1} b_{n-i} \cdot 2^{n-i} + 2^{n-k} \\[0.5cm]
              & = & \ds\sum\limits_{i=0}^{k-1} b_{n-i} \cdot 2^{n-i} + b_{n-k} \cdot 2^{n-k} \\[0.5cm]
              & = & \ds\sum\limits_{i=0}^{k} b_i \cdot 2^i 
    \end{array}
    $
  \item $a < (r_k + p_k)^2$. 

    In this case we have that $r_k + p_k > \mathtt{isqrt}(a)$ and since $p_k = 2^{n-k}$ this shows that
    \\[0.2cm]
    \hspace*{1.3cm}
    $r_k + 2^{n-1} > \mathtt{isqrt}(a)$.
    \\[0.2cm]
    ...
    \begin{array}[t]{lcl}
      r_{n+1} & = & r_k  \\[0.2cm] 
              & \stackrel{IV}{=} & \ds\sum\limits_{i=0}^{k-1} b_{n-i} \cdot 2^{n-i} \\[0.5cm]
              & = & \ds\sum\limits_{i=0}^{k-1} b_{n-i} \cdot 2^{n-i} + 0 \cdot 2^{n-k} \\[0.5cm]
              & = & \ds\sum\limits_{i=0}^{k-1} b_{n-1} \cdot 2^{n-i} + b_{k} \cdot 2^{n-k} \\[0.5cm]
              & = & \ds\sum\limits_{i=0}^{k} b_i \cdot 2^i 
    \end{array}
    $
  \end{enumerate}
...
\\[0.2cm]
\hspace*{1.3cm}
$\ds \mathtt{root}(a) = r_K = r_{n+1} = \sum\limits_{i=0}^{n} b_{n-i} \cdot 2^{n-i} = \sum\limits_{i=0}^{n} b_i \cdot 2^i = \mathtt{isqrt}(a)$. \qed
...
that computes the greatest common divisor of $x$ and $y$ satisfies the equation
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
\hspace*{1.3cm}
$\mathtt{gcd}(x_n, y_n) = \mathtt{gcd}(a, b)$ \quad where $a := x_0$ and $b := y_0$.
\\[0.2cm]
...
\colorbox{sepia}{    \green{function} \blue{ggt}(x$_0$: number, y$_0$: number): number \{              }
\colorbox{sepia}{        \green{while} (y$_n$ != 0) \{                    }
...
\colorbox{sepia}{    function \blue{square}(s$_0$: number): number \{}
\colorbox{sepia}{        let r$_0$ = 0;          }
\colorbox{sepia}{        let u$_0$ = 1;          }
\colorbox{sepia}{        while (s$_n$ > 0) \{    }
\colorbox{sepia}{            r$_{n+1}$ += u$_n$;     }
\colorbox{sepia}{            u$_{n+1}$ += 2;      }
\colorbox{sepia}{            s$_{n+1}$ -= 1;      }
\colorbox{sepia}{        \}                   }
\colorbox{sepia}{        return r$_N$;          }
\colorbox{sepia}{    \}                       }
\end{Verbatim}
\vspace*{-0.3cm}
