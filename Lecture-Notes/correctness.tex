...
In order to compute $\mathtt{isqrt}(a)$ we start with the last bit $b_{n-1}$.  If the square of $2^{n-1}$ is
less than or equal to $a$, then we must have $b_{n-1} = 1$.  Otherwise we have $b_{n-1} = 0$.  Assume now that
we have already computed the bits $b_{n-1}$, $b_{n-2}$, $\cdots$, $b_{n-k}$.  In order to compute $b_{n-(k+1)}$
we have to check whether 
\\[0.2cm]
\hspace*{1.3cm}
$\ds\left(\ds 2^{n-(k+1)} + \sum\limits_{i=n-k}^{n-1} b_i \cdot 2^i \right)^2 \leq a$.
\\[0.2cm]
...
\end{enumerate}
We proceed to prove the second invariant $\ds r_k = \sum\limits_{i=n-(k+1)}^{n-1} b_i \cdot 2^i$ by induction.
\begin{enumerate}
\item[B.C.:] $k = 0$
