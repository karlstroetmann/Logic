...
\hline
\rule{0pt}{15pt} \blue{Modus Ponens} & \blue{Modus Ponendo Tollens} & \blue{Modus Tollendo Tollens} \\[0.3cm]
\hline
$
...
die oben als ``\blue{Modus Ponens}'' und ``\blue{Modus Ponendo Tollens}'' bezeichneten
Schluss-Regeln korrekt sind, während die als  ``\blue{Modus Tollendo Tollens}'' bezeichnete
...
Problemen die Gestalt von Klauseln.  Daher stellen wir jetzt eine Schluss-Regel vor, in der
...
Formel $p \rightarrow q$, dann ist das nichts anderes als der \blue{Modus Ponens}.  
...
\begin{Definition}[Erfüllbarkeit]
  ...
  Falls es eine aussagenlogische Interpretation $\I$ gibt, die alle Formeln aus $M$ erfüllt, für die also
  \\[0.2cm]
  \hspace*{1.3cm}
...
\item Die \texttt{while}-Schleife, die in Zeile 23 beginnt, hat die Aufgabe, die Schnitt-Regel
      ...
      Schleife nur durch die Ausführung einer der beiden \texttt{return}-Befehle (Zeile 26 bzw.~Zeile 30)
      abgebrochen werden. 
\item In Zeile 24 wird die Menge \texttt{Derived} dadurch ermittelt, dass alle Klauseln berechnet
...
Zeile 23 spätestens nach $2^{2 \cdot n}$ Iterationen abgebrochen wird. 
...
\begin{Definition}[Unit-Klausel]
  Eine Klausel $k$ heißt \blue{Unit-Klausel}, wenn $k$ nur aus einem Literal besteht.
...
\begin{Definition}[Triviale Klausel-Mengen]
  ...
  \item $K$ enthält nur Unit-Klauseln mit \colorbox{red}{verschiedenen} Aussage-Variablen.
        ...
        \\[0.3cm]
        \hspace*{1.3cm}
        $\forall k \el K: \texttt{card}(k) = 1$ \quad und \quad
...
\\[0.2cm]
\hspace*{1.3cm}
$\texttt{unitCut}(K,l) = \Bigl\{ k \backslash \bigl\{ \komplement{\,l\,} \bigr\} \;\Big|\; k \in K \Bigr\}$.
\\[0.2cm]
