...
gibt, dass die Formel wahr wird.  Im Rahmen der Vorlesung werden wir verschiedene Verfahren
...
\blue{Beweisen} verknüpfen lassen.  Die \blue{Semantik} befasst sich mit der \blue{Bedeutung} der Formeln.
...
In diesem Abschnitt definieren wir die \blue{Semantik}, also die Bedeutung, aussagenlogischer Formeln.  Wir
...
\begin{enumerate}
\item $\verum$ wird repräsentiert durch den String \texttt{'$\top$'}.
      ...
      Unicode-Zeichens ``$\top$'' und ein Unicode Zeichen, das den Namen $n$ hat, kann in \textsl{Python}
      als \texttt{'\symbol{92}N\{$n$\}'} geschrieben werden.  Also haben wir
      \\[0.2cm]
      \hspace*{1.3cm}
      $\textsl{rep}(\verum) := \texttt{'$\top$'}$.
\item $\falsum$  wird repräsentiert durch den String \texttt{'$\falsum$'}.
...
\item Da aussagenlogische Variablen nichts anderes als Strings sind, können wir eine aussagenlogische Variable
...
\item Sind $f_1$ und $f_2$ aussagenlogische Formeln, so repräsentieren wir $f_1 \wedge f_2$ mit
      ...
      \\[0.2cm]
      \hspace*{1.3cm} 
      $\textsl{rep}(f \vee g) := \bigl(\texttt{'$\wedge$'}, \textsl{rep}(f), \textsl{rep}(g)\bigr)$.
...
\item Sind $f_1$ und $f_2$ aussagenlogische Formeln, so repräsentieren wir $f_1 \leftrightarrow f_2$ mit
      Hilfe des Unicode-Zeichens  \texttt{'$\leftrightarrow$'}, das den Namen ``\texttt{left rigfht arrow}'' hat:
      \\[0.2cm]
      \hspace*{1.3cm} 
