...
\emph{\color{blue}Junktoren}.  Dabei sind Junktoren Worte wie ``\blue{und}'', ``\blue{oder}'',
...
\end{enumerate}
Einfache Aussagen dieser Art bezeichnen wir auch als {\color{blue}atomare} Aussagen, weil sie sich nicht weiter in Teilaussagen
zerlegen lassen.  Atomare Aussagen lassen sich mit Hilfe der eben angegebenen Junktoren zu 
{\color{blue}zusammengesetzten Aussagen} verknüpfen.  
...
{\color{blue}Aussage-Variablen} ein.  Diese stehen für atomare Aussagen.
...
\end{enumerate}
{\color{blue}folgt logisch} die Aussage \\[0.2cm]
...
$$
Die Aussagen über dem Bruchstrich bezeichnen wir als {\color{blue}Prämissen}, die Aussage unter dem Bruchstrich ist die {\color{blue}Konklusion}.
Statt Beweis-Prinzip sagen wir oft auch {\color{blue}Schluss-Regel}.
...
gültig ist: Wenn alle Prämissen gültig sind, dann folgt aus rein logischen Gründen auch die Gültigkeit der Konklusion. 
 ...
 {\color{blue}Meta-Variablen} $p$, $q$ und $r$, die für beliebige aussagenlogische Formeln stehen.
...
mehrere {\color{blue}logische Schlüsse} aneinander fügen, erhalten wir einen {\color{blue}Beweis}. 
Die als wahr erkannten Tatsachen und Beobachtungen bezeichnet wir dabei als {\color{blue}Axiome}.
...
\item $\vdash$ bezeichnet ein System von Schluss-Regeln.  Eine solches System bezeichnen wir auch als {\color{blue}Kalkül}.
\item $r$ ist eine Aussage.  
\end{itemize}
Die Schreibweise $M \vdash r$ wäre dann als \\[0.2cm]
\hspace*{1.3cm} ``{\color{blue}Aus den Axiomen der Menge $M$ kann die Aussage $r$ hergeleitet werden}'' \\[0.2cm]
zu interpretieren.  Wir lesen  $M \vdash r$ als ``{\color{blue}$M$ leitet $r$ her}''.
...
Das Zeichen $\vdash$  symbolisiert dabei den {\color{blue}Herleitungs-Begriff}, den wir auch
als {\color{blue}Kalkül} bezeichnen.   
...
{\color{blue}Spielregeln} ist, mit dem wir aus gegebenen Formeln rein mechanisch neue Formeln herleiten
können, ohne dass wir dabei die Formeln im Detail verstehen müssen.  Durch den Umstand, dass
...
Parallel zu dem Herleitungs-Begriff gibt es auch den sogenannten {\color{blue}Folgerungs-Begriff}.
Wir schreiben 
\\[0.2cm]
\hspace*{1.3cm}
 $M \models r$, 
...
inhaltlicher Begriff, wir sprechen auch von einem {\color{blue}semantischen} Begriff,
denn beim Folgerungs-Begriff geht es um die {\color{blue}Bedeutung} der Formeln.
Die Notation $M \models r$ wird gelesen als ``{\color{blue}$r$ folgt aus $M$}''.
Das können wir anders auch so
formulieren:  Immer wenn alle Aussagen aus $M$ wahr sind, dann ist auch die Aussage $r$ wahr.
Wir können den Begriff der {\color{blue}logischen Folgerung} aber
...
Formel, die immer wahr ist, bezeichnen wir als eine {\color{blue}Tautologie}.  Andere
aussagenlogische Formeln sind nie wahr, beispielsweise ist die Formel
\\[0.2cm]
\hspace*{1.3cm}
$p \wedge \neg p$
\\[0.2cm]
immer falsch.  Eine Formel heißt {\color{blue}erfüllbar}, wenn es wenigstens eine Möglichkeit
...
\item Analyse und Design digitaler Schaltungen.
      ...
      Schaltungen eingesetzt werden, kosten heutzutage über $100\,000\,\symbol{36}$\footnote{
        Die Firma Magma bietet beispielsweise den {\color{blue}Equivalence-Checker}
...
\item Erstellung von Einsatzplänen ({\color{blue}crew scheduling}).
...
\item Erstellung von Verschlussplänen für die Weichen und Signale von Bahnhöfen.
      ...
      neu eingestellt werden müssen, um sogenannte {\color{blue}Fahrstraßen} für die Züge zu
      realisieren.  Verschiedene Fahrstraßen dürfen sich aus Sicherheitsgründen nicht kreuzen.  
      Die einzelnen Fahrstraßen werden durch sogenannte {\color{blue}Verschlusspläne} beschrieben.
...
Wir behandeln zunächst die {\color{blue}Syntax} der Aussagenlogik und besprechen anschließend die
{\color{blue}Semantik}.  Die \textsl{Syntax} gibt an, wie Formeln geschrieben werden.
Die {\color{blue}Semantik} befasst sich mit der Bedeutung der Formeln.
...
Wir betrachten eine Menge $\mathcal{P}$ von  {\color{blue}Aussage-Variablen} als gegeben.
...
\item $\verum \in \mathcal{F}$ und $\falsum \in \mathcal{F}$.
      ...
      Formel steht, die immer falsch ist.  Die Formel $\verum$ trägt auch den Namen
      \textsl{\color{blue}Verum},
      für $\falsum$ sagen wir auch \textsl{\color{blue}Falsum}.
...
\item Ist $f \in \mathcal{F}$, so gilt auch $\neg f \in \mathcal{F}$.
      ...
      Die Formel $\neg f$ bezeichnen wir auch als die {\color{blue}Negation} von $f$.
\item Sind $f_1, f_2 \in \mathcal{F}$, so gilt auch
      \begin{tabbing}
        $(f_1 \vee f_2) \in \mathcal{F}$ \hspace*{0.5cm} \= (\textsl{gelesen}: \quad \= $f_1$ oder $f_2$ \hspace*{2.5cm} \=
         auch: {\color{blue}Disjunktion} von $f_1$ und $f_2$),            \\
        $(f_1 \wedge f_2) \in \mathcal{F}$                 \> (\textsl{gelesen}:       \> $f_1$ und $f_2$ \>
         auch: {\color{blue}Konjunktion} von $f_1$ und $f_2$),            \\
        $(f_1 \rightarrow f_2) \in \mathcal{F}$                \> (\textsl{gelesen}:       \> wenn $f_1$, dann $f_2$ \>
         auch: {\color{blue}Implikation} von $f_1$ und $f_2$),            \\
        $(f_1 \leftrightarrow f_2) \in \mathcal{F}$                \> (\textsl{gelesen}:       \> $f_1$ genau dann, wenn $f_2$ \>
         auch: {\color{blue}Bikonditional} von $f_1$ und $f_2$).            
      \end{tabbing}
\end{enumerate}
Die Menge ${\color{blue}\mathcal{F}}$ ist nun die kleinste Teilmenge der aus dem Alphabet $\mathcal{A}$
...
\item Die Junktoren  $\vee$ und $\wedge$ werden implizit links geklammert, d.h.~wir
      ...
      \\[0.2cm]
      Operatoren, die implizit nach links geklammert werden, nennen wir {\color{blue}links-assoziativ}.

      \underline{\color{red}Beachten} Sie, dass die Junktoren $\wedge$ und $\vee$ dieselbe Bindungsstärke
      haben.  Das ist anders als in der Sprache \textsc{SetlX}, denn dort bindet der Operator
      ``\texttt{\&\&}'' stärker als der Operator ``\texttt{||}''.
\item Der Junktor $\rightarrow$ wird implizit rechts geklammert, d.h.~wir
      ...
      \\[0.2cm]
      Operatoren, die implizit nach rechts geklammert werden, nennen wir {\color{blue}rechts-assoziativ}.
...
ebenfalls aussagenlogische Junktoren wie ``{\color{blue}genau dann, wenn}'' oder 
``{\color{blue}wenn $\cdots$,  dann}'' verwenden.  Dabei entsteht dann die Gefahr, dass wir die Junktoren,
...
\item Innerhalb einer aussagenlogischen Formel wird der Junktor  
      ``{\color{blue}wenn $\cdots$,  dann}'' als ``$\rightarrow$''  geschrieben.
...
\end{enumerate}
Analog wird der Junktor ``{\color{blue}genau dann, wenn}'' innerhalb einer aussagenlogischen Formel als
...
den Begriff einer {\color{blue}aussagenlogischen Interpretation} festlegen.

\begin{Definition}[Aussagenlogische Interpretation]
  Eine {\color{blue}aussagenlogische Interpretation} ist eine Funktion \\[0.2cm]
  ...
  \eox
\end{Definition}
Eine aussagenlogische Interpretation wird oft auch als {\color{blue}Belegung} der
...
Die Interpretation des aussagenlogischen Junktoren ist rein {\color{blue}extensional}:
...
``\textsl{wenn $\cdots$, dann}'' auch eine {\color{blue}kausale} Bedeutung.  Mit der extensionalen
...
\item Eine aussagenlogische Variable $p \in \mathcal{P}$ repräsentieren wir 
      ...
      können wir lateinische Buchstaben als Variablen verwenden.
...
es auch wichtig, dass die Repräsentation für die zu entwickelnden Algorithmen {\color{red}adäquat}
...
\\[0.2cm]
\hspace*{1.3cm}
\texttt{@@@implication(@@@variable(\symbol{34}p\symbol{34}), \\
\hspace*{4.4cm}
        @@@disjunction(@@@variable(\symbol{34}p\symbol{34}), @@@not(@@@variable(\symbol{34}q\symbol{34}))))}
\\[0.2cm]
...
der Operator ``\texttt{\&\&}'' wird durch ``\texttt{@@@conjunction}'' repräsentiert und
...
Wert \texttt{true}.  Formeln mit dieser Eigenschaft  bezeichnen wir als {\color{blue}Tautologie}.
\begin{Definition}[Tautologie]
  ...
  dann ist $f$ eine {\color{blue}Tautologie}.  In diesem Fall schreiben wir \\[0.2cm]
...
{\color{blue}allgemeingültig} ist.
...
\begin{Definition}[Äquivalent]
  Zwei Formeln $f$ und $g$ heißen {\color{blue}äquivalent} g.d.w.  \\[0.2cm]
...
durchprobieren.  Eine solche Tabelle heißt auch {\color{blue}Wahrheits-Tafel}.
...
die Formel mit Hilfe der oben aufgeführten Äquivalenzen {\color{blue}vereinfachen}.  Wenn es gelingt, eine Formel $F$ unter Verwendung
...
\begin{Definition}[Literal]
  Eine aussagenlogische Formel $f$ heißt {\color{blue}Literal} g.d.w. einer der folgenden Fälle vorliegt:
  \begin{enumerate}
  \item $f = \verum$ oder $f = \falsum$.
  \item $f = p$, wobei $p$ eine aussagenlogische Variable ist.

        In diesem Fall sprechen wir von einem {\color{blue}positiven} Literal.
  \item $f = \neg p$, wobei $p$ eine aussagenlogische Variable ist. 

        In diesem Fall sprechen wir von einem {\color{blue}negativen} Literal.
  \end{enumerate}
  Die Menge aller Literale bezeichnen wir mit ${\color{blue}\mathcal{L}}$.
  \eox
\end{Definition}

Später werden wird noch den Begriff des {\color{blue}Komplements} eines Literals benötigen.
...
\begin{Definition}[Klausel]
  Eine aussagenlogische Formel $k$ ist eine {\color{blue}Klausel} wenn $k$ die Form \\[0.2cm]
...
Oft werden Klauseln auch einfach als {\color{blue}Mengen} von Literalen betrachtet.  
...
{\color{blue}Mengen-Schreibweise}.  
...
\begin{Definition}
  Eine Klausel $k$ ist {\color{blue}trivial}, wenn einer der beiden folgenden Fälle vorliegt:
  \begin{enumerate}
  \item $\verum \in k$.
  \item Es existiert $p \in \mathcal{P}$ mit $p \in k$ und $\neg p \in k$.

        In diesem Fall bezeichnen wir $p$ und $\neg p$ als {\color{blue}komplementäre Literale}.
        \eox
\end{enumerate}
\end{Definition}
...
\begin{Definition}[Konjunktive Normalform]  
  Eine Formel $f$ ist in {\color{blue}konjunktiver Normalform} (kurz KNF)
...
{\color{blue}Mengen-Schreibweise} einzuführen:  Ist die Formel
\\[0.2cm]
...
\item Schiebe die Negationszeichen soweit es geht nach innen.  Verwende dazu die folgenden Äquivalenzen:
      ...
      Eigenschaft bezeichnen wir auch als Formeln in {\color{blue}Negations-Normalform}.
\item Stehen in der Formel jetzt ``$\vee$''-Junktoren über ``$\wedge$''-Junktoren, so können wir durch
      {\color{blue}Ausmultiplizieren}, sprich Verwendung der äquivalenz \\[0.2cm]
      \hspace*{1.3cm} 
      $
      \begin{array}{cl}
...
Mengen-Schreibweise überführt wird.  Schlie{\ss}lich entfernt die Funktion \texttt{simplify}
...
können wir uns fragen, ob  die  Formel $g$ aus $f_1$, $\cdots$, $f_n$ {\color{blue}folgt}, ob
also 
...
ist es, die Formel $f$ mit Hilfe von {\color{blue}Schluss-Regeln} aus den gegebenen Formeln 
...
\begin{Definition}[Schluss-Regel]
    Eine {\color{blue}Schluss-Regel} ist eine Paar  $\langle \{f_1, \cdots, f_n\}, k \rangle$.
    Dabei ist 
    $\{f_1, \cdots, f_n\}$ eine Menge von Formeln und $k$ ist eine einzelne Formel.  
    Die Formeln $f_1$, $\cdots$, $f_n$ bezeichnen wir als
    {\color{blue}Prämissen}, die Formel $k$ heißt die {\color{blue}Konklusion} der Schluss-Regel.
    Ist das Paar 
...
\hline
\rule{0pt}{15pt} {\color{blue}Modus Ponens} & {\color{blue}Modus Ponendo Tollens} & {\color{blue}Modus Tollendo Tollens} \\[0.3cm]
\hline
$
...
\begin{Definition}[Korrekte Schluss-Regel]
  ...
  ist genau dann {\color{blue}korrekt}, wenn 
  $\models f_1 \wedge \cdots \wedge f_n \rightarrow k$ gilt. \eox
\end{Definition}
Mit dieser Definition sehen wir, dass 
die oben als ``{\color{blue}Modus Ponens}'' und ``{\color{blue}Modus Ponendo Tollens}'' bezeichneten
Schluss-Regeln korrekt sind, während die als  ``{\color{blue}Modus Tollendo Tollens}'' bezeichnete
...
\begin{Definition}[Schnitt-Regel]
    ...
    als die {\color{blue}Schnitt-Regel}: 
    \\[0.2cm]
    \hspace*{1.3cm}
...
Formel $p \rightarrow q$, dann ist das nichts anderes als der {\color{blue}Modus Ponens}.  
Die Regel {\color{blue}Modus Tollens} ist ebenfalls ein Spezialfall der Schnitt-Regel.  Wir
...
\begin{Definition}[Herleitungs-Begriff, $\vdash$]
    ...
    Die Formeln aus $M$ bezeichnen wir als unsere {\color{blue}Annahmen}, die Formel
    $f$ hei{\ss}t {\color{blue}Konklusion}.  Unser Ziel ist es, mit den Annahmen aus $M$
...
\begin{Satz}[{\color{blue}Korrektheit}]
...
\begin{Satz}[{\color{blue}Widerlegungs-Vollständigkeit}] \label{widerlegungs-vollstaendig}
...
{\color{blue}Erfüllbarkeit}, den wir jetzt formal einführen. 

\begin{Definition}[Erfüllbarkeit]
  Es sei $M$ eine Menge von aussagenlogischen Formeln.
  Falls es eine aussagenlogische Interpretation $\I$ gibt, die alle Formeln aus $M$ erfüllt, nennen
  wir $M$ {\color{blue}erfüllbar}. 

  Wir sagen, dass $M$ {\color{blue}unerfüllbar} ist und schreiben 
  \\[0.2cm]
  \hspace*{1.3cm}
  ${\color{blue}M \models \falsum}$
  \\[0.2cm]
  ...
  \textsc{\color{blue}Ali}, so schreibt sich das formal als
  \\[0.2cm]
  \hspace*{1.3cm}
  ${\color{blue}M \models \falsum}$ \quad g.d.w. \quad 
...
$\I$, die alle Klauseln aus $M$ erfüllt.
...
\item Die Funktion \texttt{complement} erhält als Argument ein Literal $l$ und berechnet das
      {\color{blue}Komplement} $\komplement{\,l\,}$ dieses Literals.  Falls $l$ die Form $\neg p$ mit einer
...
\item Die Funktion \texttt{extractVar} extrahiert die aussagenlogische Variable aus einem Literal $l$.
      Die Implementierung verläuft analog zur Implementierung der Funktion \texttt{complement} über eine
      Fallunterscheidung, bei der wir berücksichtigen, dass $l$ entweder die Form $\neg p$ oder die
...
zurück gegeben wird, unter Anwendung der Schnitt-Regel {\color{blue}saturiert} ist, was formal wie folgt
definiert ist:
\begin{enumerate}
...
\item Die \texttt{while}-Schleife, die in Zeile 23 beginnt, hat die Aufgabe, die Schnitt-Regel
      solange wie möglich anzuwenden um mit Hilfe der Schnitt-Regel neue Klauseln aus den gegebenen
      Klauseln herzuleiten.  Da die Bedingung dieser Schleife den Wert \texttt{true} hat, kann diese
      Schleife nur durch die Ausführung des \texttt{return}-Befehls in Zeile 31 abgebrochen werden.
...
\item Andernfalls ziehen wir in Zeile 28 von der Menge \texttt{Derived} zunächst die Klauseln
      ab, die wir schon in der Menge \texttt{Clauses} vorhanden waren, denn es geht uns darum
      festzustellen, ob wir im letzten Schritt tatsächlich 
      \underline{neue} Klauseln gefunden haben, oder ob alle Klauseln, die wir im letzten Schritt in Zeile 24
...
\item Falls wir nun in Zeile 29 feststellen, dass wir keine neuen Klauseln hergeleitet haben,
      dann ist die Menge \texttt{Clauses} saturiert und wir geben diese Menge in Zeile 30 zurück.
\item Andernfalls fügen wir in Zeile 32 die Klauseln, die wir neu gefunden haben, zu der Menge
...
Zeile 23 nach spätestens $2^{2 \cdot n}$ Iterationen abgebrochen wird. 
...
\begin{enumerate}
\item Zunächst berechnen wir in Zeile 36 die Menge aller aussagenlogischen Variablen, die in
...
\item In Zeile 37 saturieren wir die Menge \texttt{Clauses} und berechnen alle Klauseln, die aus der
      ...
      \begin{enumerate}
      \item Falls die leere Klausel hergeleitet werden kann, dann ist die ursprünglich gegebene Menge
            von Klauseln widersprüchlich und wir geben als Ergebnis an Stelle einer Belegung den
            Wert \texttt{false} zurück, denn eine widersprüchliche Menge von Klauseln ist sicher
            nicht erfüllbar.
      \item Andernfalls berechnen wir nun eine aussagenlogische Belegung, unter der alle Klauseln aus
            ...
            Literalen \texttt{Literals}.  Die Idee ist dabei, dass wir die Variable \texttt{p} genau
            ...
            $\mathcal{I}$ kann dann gemä{\ss} der Formel  
            \\[0.2cm]
            \hspace*{1.3cm}
            $\mathcal{I}(\mathtt{p}) = \left\{
             \begin{array}{ll}
               \mathtt{true}  & \mbox{falls $\mathtt{ p} \in \mathtt{Literals}$} \\
               \mathtt{false} & \mbox{falls $\mathtt{!p} \in \mathtt{Literals}$}
             \end{array}\right.
            $
            \\[0.2cm]
            berechnet werden.
...
\item Die Berechnung der Menge \texttt{Literals} erfolgt nun über eine \texttt{for}-Schleife.
      Dabei ist die Idee, dass wir für eine aussagenlogische Variable \texttt{p} genau dann das Literal
     ...
     \texttt{!p} zu dieser Menge hinzu.
      ...
      \\[0.2cm]
      \hspace*{1.3cm}
      $C \subseteq \{ \komplement{l_1}, \cdots, \komplement{l_n}, \texttt{p} \}$ \quad und \quad $\texttt{p} \in C$
      \\[0.2cm]
      gilt.  Wenn $\I(C) = \texttt{true}$ gelten soll, dann muss $\I(\texttt{p}) = \texttt{true}$ gelten, denn
      nach Konstruktion von $\I$ gilt 
      \\[0.2cm]
      \hspace*{1.3cm}
      $\I(\komplement{l_i}) = \texttt{false}$ \quad für alle $i \in \{1,\cdots,n\}$
      \\[0.2cm]
...
dass in Zeile 39 nicht der Wert \texttt{false} zurück gegeben wird, eine aussagenlogische Belegung $\I$ berechnet, bei der
...
\item[I.S.:] $n \mapsto n+1$.
             ...
             \item Für alle $i \in \{1,\cdots,n\}$ mit $l_i \in D$ gilt $\I(l_i) = \texttt{false}$ und es gilt $l_{n+1} = p_{n+1}$.
                   ...
                   \\[0.2cm]
                   \hspace*{1.3cm}
                   $C \subseteq \{ \komplement{l_1}, \cdots, \komplement{l_n} \} \cup \{ p_{n+1} \}$
                   \\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm} $\texttt{eval}(k,\I) = \texttt{true}$ \quad für alle $k\in K$ \\[0.2cm]
gilt.  In diesem Fall sagen wir auch, dass die Belegung $\I$ eine {\color{blue}Lösung} der
...
$p \wedge \neg q \wedge \neg p $
\\[0.2cm]
Offenbar ist $K_3$ ebenfalls unlösbar, denn eine Lösung $\I$ müsste \texttt{p} gleichzeitig
...
\begin{Definition}[Unit-Klausel]
  Eine Klausel $k$ heißt {\color{blue}Unit-Klausel}, wenn $k$ nur aus einem Literal besteht.
  Es gilt dann entweder
  \\[0.2cm]
  \hspace*{1.3cm}
  $k = \{p\}$ \quad oder \quad $k = \{\neg p\}$ 
  \\[0.2cm]
  für eine geeignete Aussage-Variable \texttt{p}. \eox
\end{Definition}

\begin{Definition}[Triviale Klausel-Mengen]
  Eine Klausel-Menge $K$ heißt {\color{blue}trivial} wenn einer der beiden folgenden Fälle
  vorliegt.
  \begin{enumerate}
  \item $K$ enthält die leere Klausel: \qquad $\{\} \el K$.

        In diesem Fall ist $K$ offensichtlich unlösbar.
  \item $K$ enthält nur Unit-Klauseln mit \colorbox{red}{verschiedenen} Aussage-Variablen.
...
und folglich eine {\color{blue}Unit-Klausel} ist.  Da es unser Ziel ist, die Klausel-Mengen
...
{\color{blue}Unit-Schnitte}.  Um alle mit einer gegebenen Unit-Klausel $\{l\}$ möglichen Schnitte
...
\\[0.2cm]
Beachten Sie, dass die Menge $\textsl{unitCut}(K,l)$ genauso viele Klauseln enthält wie die Menge
$K$.  Allerdings sind die Klauseln aus der Menge $K$, die das Literal $\komplement{\,l\,}$
enthalten, verkürzt worden. 
...
gilt.  Allgemein sagen wir, dass eine Klausel $k$
 von einer Unit-Klausel $u$ {\color{blue}subsumiert} wird, wenn
\\[0.2cm]
...
vereinfachen, wir können also die Klausel $k$ aus $K$ löschen.  Allgemein definieren wir eine Funktion
\\[0.2cm]
\hspace*{1.3cm}
...
{\color{blue}Fallunterscheidung}.  Dieses Prinzip basiert auf dem folgenden
Satz.
...
zunächst Unit-Subsumptionen und anschließend Unit-Schnitte durchführen können.
...
\hspace*{1.3cm} $\I(k) = \texttt{true}$ \quad für alle $k \in K$.\\[0.2cm]
...
logisch äquivalent sind, wenn wir diese Mengen als Formeln in konjunktiver Normalform
interpretieren.  
...
durchführen. Bei komplexeren Beispielen ist es häufig so, dass wir mehrere Fallunterscheidungen
durchführen müssen.
...
\item Die Prozedur \texttt{selectLiteral} wählt ein beliebiges Literal aus 
      ...
      $\texttt{arb}()$ benutzen können.   Allerdings habe ich experimentell herausgefunden, dass
      der Davis-Putnam Algorithmus im Allgemeinen deutlich effizienter wird,  wenn wir das Literal
      mit Hilfe der Funktion $\texttt{rnd}()$ auswählen.
...
\hspace*{1.3cm} von {\color{blue}M. Moskewicz, C. Madigan, Y. Zhao, L. Zhang, S. Malik} 
...
gegeben, so brauchen wir eine Formel die aussagt, dass {\color{blue}höchstens} eine der Variablen aus
...
Als nächstes berechnen wir eine Formel die aussagt, dass {\color{blue}mindestens} eine Dame in einer gegebenen
...
{\color{blue}Hauptdiagonale}, besteht im Fall eines $8 \times 8$-Bretts aus den
Variablen \\[0.2cm]
\hspace*{1.3cm} 
$\texttt{Var}(8,1),\; \texttt{Var}(7,2),\; \texttt{Var}(6,3),\; \texttt{Var}(5,4),\; \texttt{Var}(4,5),\; \texttt{Var}(3,6),\; 
 \texttt{Var}(2,7),\; \texttt{Var}(1,8)$. 
\\[0.2cm]
Die Indizes $i$ und $j$ der Variablen $\texttt{Var}(i,j)$ erfüllen offenbar
die Gleichung \\[0.2cm]
...
berechnete Menge \texttt{solution} enthält für jede der Variablen $\texttt{Var}(i,j)$
...
Stundenplan-Problems werden in der Literatur als {\color{blue}Scheduling-Problemen} bezeichnet.
