...
\item In Zeile 5 testen wir, ob die Formel $f$ für alle möglichen Interpretationen $I$
      ...
      Auswertung von $f$ den Wert \texttt{false} liefert, so bilden wir in Zeile 8 die
      Menge aller solcher Belegungen und wählen mit Hilfe der Funktion
      $\texttt{arb}$ eine beliebige Belegungen aus dieser Menge aus, die wir dann
...
die Formel mit Hilfe der oben aufgeführten Äquivalenzen \blue{vereinfachen}.  Wenn es gelingt, eine Formel $F$ unter Verwendung
...
\begin{Definition}[Herleitungs-Begriff, $\vdash$]
    ...
    Die Formeln aus $M$ bezeichnen wir als unsere \blue{Annahmen}, die Formel
    $f$ hei{\ss}t \blue{Konklusion}.  Unser Ziel ist es, mit den Annahmen aus $M$
...
\begin{Definition}[Erfüllbarkeit]
  ...
  wir $M$ \blue{erfüllbar}. 

  Wir sagen, dass $M$ \blue{unerfüllbar} ist und schreiben 
  \\[0.2cm]
  \hspace*{1.3cm}
