...
\emph{\color{blue}Junktoren}.  Dabei sind Junktoren Worte wie ``\textsl{und}'', ``\textsl{oder}'',
...
\end{enumerate}
Einfache Aussagen dieser Art bezeichnen wir auch als \emph{\color{blue}atomare} Aussagen, weil sie sich nicht weiter in Teilaussagen
zerlegen lassen.  Atomare Aussagen lassen sich mit Hilfe der eben angegebenen Junktoren zu 
\emph{\color{blue}zusammengesetzten Aussagen} verkn\"{u}pfen.  
...
\begin{enumerate}
\item Der Herleitungsbegriff sollte \underline{\color{red}korrekt} sein, es sollte nicht m\"{o}glich sein,
      Unsinn zu beweisen.  Damit muss gelten: \\[0.2cm]
      \hspace*{1.3cm} Aus $M \vdash r$ folgt $M \models r$. 
      \\[0.2cm]
      Wenn wir die Aussage $r$ aus den Axiomen der Menge $M$ herleiten k\"{o}nnen, dann folgt
      $r$ auch aus $M$.
...
durchprobieren.  Eine solche Tabelle hei\ss{}t auch \emph{\color{blue}Wahrheits-Tafel}.
...
jedes Mal der Wert \texttt{true} ergibt.  Dazu m\"{u}ssen wir zun\"{a}chst einen Weg finden, alle
...
die Formel mit Hilfe der oben aufgef\"{u}hrten \"{A}quivalenzen \emph{\color{blue}vereinfachen}.  Wenn es gelingt, eine Formel $F$ unter Verwendung
...
\begin{Definition}[Literal]
  Eine aussagenlogische Formel $f$ hei\ss{}t \emph{\color{blue}Literal} g.d.w. einer der folgenden F\"{a}lle vorliegt:
  \begin{enumerate}
  \item $f = \verum$ oder $f = \falsum$.
  \item $f = p$, wobei $p$ eine aussagenlogische Variable ist.

        In diesem Fall sprechen wir von einem \emph{\color{blue}positiven} Literal.
  \item $f = \neg p$, wobei $p$ eine aussagenlogische Variable ist. 

        In diesem Fall sprechen wir von einem \emph{\color{blue}negativen} Literal.
  \end{enumerate}
  Die Menge aller Literale bezeichnen wir mit ${\color{blue}\mathcal{L}}$.
  \eox
\end{Definition}

Sp\"{a}ter werden wird noch den Begriff des \emph{\color{blue}Komplements} eines Literals ben\"{o}tigen.
...
Oft werden Klauseln auch einfach als \emph{\color{blue}Mengen} von Literalen betrachtet.  
...
Da f\"{u}r die Konjunktion analog zur Disjunktion das Assoziativ-, Kommutativ- und Idempotenz-Gesetz
...
\begin{Satz}[{\color{blue}Widerlegungs-Vollst\"{a}ndigkeit}] \label{widerlegungs-vollstaendig}
...
\item Die \texttt{while}-Schleife, die in Zeile 23 beginnt, hat die Aufgabe, die Schnitt-Regel
      ...
      Schleife nur durch die Ausf\"{u}hrung des \texttt{return}-Befehls in Zeile 31 abgebrochen werden.
...
\item Andernfalls ziehen wir in Zeile 28 von der Menge \texttt{Derived} zun\"{a}chst die Klauseln
      ab, die wir schon in der Menge \texttt{Clauses} vorhanden waren, denn es geht uns darum
      festzustellen, ob wir im letzten Schritt tats\"{a}chlich 
...
\item In Zeile 37 saturieren wir die Menge \texttt{Clauses} und berechnen alle Klauseln, die aus der
      ...
      \item Andernfalls berechnen wir nun eine aussagenlogische Belegung, unter der alle Klauseln aus
            der Menge \texttt{Clauses} wahr werden.  Zu diesem Zweck berechnen wir zun\"{a}chst eine Menge von
            ...
            Zeile 50 die Menge \texttt{Literals} zur\"{u}ck.  Die gesuchte aussagenlogische Belegung
            $\mathcal{I}$ kann dann gem\"{a}{\ss} der Formel  
            \\[0.2cm]
            \hspace*{1.3cm}
...
\item Die Berechnung der Menge \texttt{Literals} erfolgt nun \"{u}ber eine \texttt{for}-Schleife.
      Dabei ist die Idee, dass wir f\"{u}r eine aussagenlogische Variable \texttt{p} genau dann das Literal
      $\texttt{p}$ zu der Menge \texttt{Literals} hinzuf\"{u}gen, wenn die Belegung $\I$ die Variable \texttt{p}
      auf \texttt{true} abbilden muss um die Klauseln zu erf\"{u}llen.  Andernfalls f\"ugen wir stattdessen das Literal 
     \texttt{!p} zu dieser Menge hinzu.
      ...
      \\[0.2cm]
      \hspace*{1.3cm}
      $\I(\komplement{l_i}) = \texttt{false}$ \quad f\"{u}r alle $i \in \{1,\cdots,n\}$
      \\[0.2cm]
...
\\[0.2cm]
und zeigen durch Induktion nach $n$, dass nach $n$ Durchl\"{a}ufen der Schleife f\"{u}r jede Klausel $D \in \texttt{Clauses}$, 
...
\item[I.S.:] $n \mapsto n+1$.
             ...
             \item F\"{u}r alle $i \in \{1,\cdots,n\}$ mit $l_i \in D$ gilt $\I(l_i) = \texttt{false}$ und es gilt $l_{n+1} = p_{n+1}$.
                   ...
                   $\I(p_{n+1}) = \texttt{true}$
                   \\[0.2cm]
                   ist und dann gilt nat\"{u}rlich auch $\I(D) = \texttt{true}$.
             \item F\"{u}r alle $i \in \{1,\cdots,n\}$ mit $l_i \in d$ gilt $\I(l_i) = \texttt{false}$ und es gilt $l_{n+1} = \neg p_{n+1}$.
                   ...
                   \item Es gibt eine weitere Klausel $C$ in der Menge \texttt{Clauses}, so dass
                         ...
                         in diesem Fall w\"{u}rde um die Klausel $C$ wahr zu machen das Literal $p_{n+1}$ zur Menge
                   ...
                   \item Es gibt \underline{keine} Klausel $C$ in der Menge \texttt{Clauses}, so dass
                         ...
                         hinzugef\"{u}gt und damit gilt zun\"{a}chst $\I(p_{n+1}) = \texttt{false}$ und folglich
...
\cite{davis:1960, davis:1962} zur\"{u}ck.  Verfeinerungen dieses Verfahrens werden beispielsweise
...
$p \wedge \neg q \wedge \neg p $
\\[0.2cm]
Offenbar ist $K_3$ ebenfalls unl\"{o}sbar, denn eine L\"{o}sung $\I$ m\"{u}sste \texttt{p} gleichzeitig
...
\begin{Definition}[Unit-Klausel]
  ...
  \\[0.2cm]
  f\"{u}r eine geeignete Aussage-Variable \texttt{p}. \eox
\end{Definition}
...
Falls wir nun eine L\"{o}sung finden, ist dies auch eine L\"{o}sung von $K$ und wenn wir weder
f\"{u}r $K_1$ noch f\"ur $K_2$ eine L\"{o}sung finden, dann kann auch $K$ keine L\"{o}sung haben,
denn jede L\"osung $\mathcal{I}$ von $K$ muss die Variable $P$ entweder wahr oder falsch machen.
...
\texttt{davisPutnam(Clauses, Literals)} 
\\[0.2cm]
als Ergebnis die Menge $\bigl\{ \{\} \bigr\}$ zur\"{u}ck, denn die leere Klausel repr\"{a}sentiert die unerf\"{u}llbare Formel $\falsum$.
...
\item In Zeile 7 extrahieren mit der Funktion \texttt{arb} das Literal \texttt{l} der Klausel
      ...
      das dieser Funktion als Argument \"{u}bergeben wird.  Enth\"{a}lt diese Menge nur ein Element, so
...
\item Die Prozedur \texttt{selectLiteral} w\"{a}hlt ein beliebiges Literal aus 
      ...
      resultierenden Menge wird mit Hilfe der Funktion $\texttt{rnd}()$ zuf\"allig ein Literal
      ausgew\"{a}hlt.  An Stelle der Funktion $\texttt{rnd}()$ h\"atten wir hier auch die Funktion 
      $\texttt{arb}()$ benutzen k\"onnen.   Allerdings habe ich experimentell herausgefunden, dass
      der Davis-Putnam Algorithmus im Allgemeinen deutlich effizienter wird,  wenn wir das Literal
      mit Hilfe der Funktion $\texttt{rnd}()$ ausw\"ahlen.
...
gegeben, so brauchen wir eine Formel die aussagt, dass {\color{blue}h\"{o}chstens} eine der Variablen aus
...
Klauseln.  Diese Klauseln sind genau dann wahr, wenn h\"{o}chstens eine der Variablen aus $S$
...
Als n\"{a}chstes berechnen wir eine Formel die aussagt, dass {\color{blue}mindestens} eine Dame in einer gegebenen
...
automatisch klar, dass h\"ochstens 8 Damen auf dem Brett stehen.  Damit stehen also insgesamt genau
8 Damen auf dem Brett.  Dann kann aber in jeder Spalte nur h\"ochstens eine Dame stehen und genauso muss
in jeder Zeile mindestens eine Dame stehen, denn sonst w\"urden wir nicht auf 8 Damen kommen.
...
berechnete Menge \texttt{solution} enth\"{a}lt f\"{u}r jede der Variablen $\texttt{Var}(i,j)$
...
Stundenplan-Problems werden in der Literatur als \emph{\color{blue}Scheduling-Problemen} bezeichnet.
