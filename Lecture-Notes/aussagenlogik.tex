...
\begin{Verbatim}[ frame         = lines, 
    ...
    findValuation := procedure(Clauses) {
        ...
        return { complement(l) : l in Literals };
    };
\end{Verbatim}
\vspace*{-0.3cm}
...
\item In Zeile 37 saturieren wir die Menge \texttt{Clauses} und berechnen alle Klauseln, die aus der
      ...
      \item Andernfalls berechnen wir nun eine aussagenlogische Belegung, unter der alle Klauseln aus
            ...
            Variable \texttt{p} zu \texttt{false} auswertet.  Andernfalls nehmen wir an Stelle von \texttt{p} das
            Literal $\neg \texttt{p}$ in der Menge \texttt{Literals} auf.  Als Ergebnis geben wir daher in
            Zeile 50 die Menge $M$ zur\"{u}ck, bei der wir jedes Literal $l$ aus der 
            Menge \texttt{Literals} durch sein Komplement $\komplement{\,l\,}$ ersetzen.  

            Im engeren Sinne ist diese Menge $M$ keine aussagenlogische Belegung, aber wir
...
\item Die Berechnung der Menge \texttt{Literals} erfolgt nun \"{u}ber eine \texttt{for}-Schleife.
      ...
      $\neg \texttt{p}$ zu der Menge \texttt{Literals} hinzuf\"{u}gen, wenn die Belegung $\I$ die Variable \texttt{p}
      ...
      \\[0.2cm]
      \hspace*{1.3cm}
      $C \subseteq \{ l_1, \cdots, l_n, \texttt{p} \}$ \quad und \quad $\texttt{p} \in C$
      \\[0.2cm]
      ...
      $\neg \texttt{p}$ in die Menge \texttt{Literals} ein. 
\end{enumerate}
...
\item[I.S.:] $n \mapsto n+1$.
             ...
             \item F\"{u}r alle $i \in \{1,\cdots,n\}$ mit $l_i \in D$ gilt $\I(l_i) = \texttt{false}$ und es gilt $l_{n+1} = p_{n+1}$.
                   ...
                   \\[0.2cm]
                   \hspace*{1.3cm}
                   $D \subseteq \texttt{Literals} \cup \{ p_{n+1} \}$
                   \\[0.2cm]
             ...
             \item F\"{u}r alle $i \in \{1,\cdots,n\}$ mit $l_i \in d$ gilt $\I(l_i) = \texttt{false}$ und es gilt $l_{n+1} = \neg p_{n+1}$.
                   ...
                   \item Es gibt eine weitere Klausel $C$ in der Menge \texttt{Clauses}, so dass
                         ...
                         \\[0.2cm]
                         \hspace*{1.3cm}
                         $C \subseteq \texttt{Literals} \cup \{p_{n+1}\}$ \quad und \quad $p_{n+1} \in C$
                         \\[0.2cm]
