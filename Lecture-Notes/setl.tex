...
erfahrungsgem\"{a}\ss{} dem Anf\"{a}nger aufgrund ihrer Abstraktheit gewisse Schwierigkeiten.  Um
...
\texttt{setlx}
\\[0.2cm]
in einer Kommando-Zeile eingeben.  Anschlie\ss{}end meldet sich der Interpreter dann wie wie
...
\texttt{1 + 2;}
\\[0.2cm]
ein und dr\"{u}cken anschlie\ss{}end auf die Eingabe-Taste, so erhalten Sie
...
Funktion, mit deren Hilfe Sie Werte ausdrucken k\"{o}nnen.  Diese Funktion hei\ss{}t \texttt{print}.  Wenn
Sie im Interpreter
\\[0.2cm]
\hspace*{1.3cm}
\texttt{print(\symbol{34}Hallo\symbol{34});}
\\[0.2cm]
eingeben und anschlie\ss{}end auf die Eingabe-Taste dr\"{u}cken, so erhalten Sie
die folgende Ausgabe:
...
Ausdruck auch unmittelbar hinter dem Prompt eingeben und mit einem Semikolon ``\texttt{;}'' abschlie\ss{}en.
...
\item Die zweite Zeile enth\"{a}lt eine Zuweisung.  Die Funktion $\textsl{read}()$ gibt
      ...
      \item Die Namen von Variablen oder Funktion m\"{u}ssen in der Sprache \textsc{SetlX} mit einem
            ...
            In Gegensatz dazu d\"{u}rfen diese Namen in \texttt{C} auch mit einem gro\ss{}en
...
\item Zeile 1 bis Zeile 7 enthalten die Definition der Prozedur \texttt{sum}.  Die 
      ...
      und danach  eine schlie\ss{}enden Klammer ``\texttt{)}''.
      ...
      \item Falls sich ``\texttt{true}'' ergibt, werden anschlie\ss{}end die Kommandos
...
\item Anschlie\ss{}end wird das berechnete Ergebnis durch einen \texttt{print}-Befehl ausgegeben.
\end{enumerate}
...
ob \texttt{a} eine Teilmenge von \texttt{b} ist.  Schlie\ss{}lich testen wir in Zeile 18 mit Hilfe des
...
bei gro\ss{}en Mengen viel zu m\"{u}hsam.  Eine Alternative ist daher die Definition einer Menge
...
Faktoren $\leq$ 10 sind.  (Ein Produkt der Form $a \cdot b$ hei\ss{}t dabei \emph{trivial} genau
...
vorgegebenen Gr\"{o}\ss{}e $n$ und ist so kurz wie eindrucksvoll.  Die zugrunde liegende
...
Buchstaben beginnen!  Normalerweise h\"{a}tte ich die Relation \texttt{q} mit einem gro\ss{}en
\texttt{Q} bezeichnen wollen, aber das geht nicht, denn alle Namen, die mit einem gro\ss{}en
