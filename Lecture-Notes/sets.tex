...
\href{http://en.wikipedia.org/wiki/Natural_number}{set of natural numbers}, which is denoted by the symbol $\mathbb{N}$.
...
\begin{enumerate}
\item $\mathbb{N}^*$ is the set of \blue{positive natural numbers},  we have
      \\[0.2cm]
      \hspace*{1.3cm}
...
\item $\mathbb{R}$ is the set of \blue{real numbers}.
      ...
      A clean  mathematically definition of the notion of a \href{https://en.wikipedia.org/wiki/Real_number}{real number}
...
The \blue{axiom of specification} (Im Deutschen verwenden wir hier den Begriff \blue{Aussonderungs-Axiom} oder auch
\blue{Auswahl-Axiom}), also known as the 
...
The  \blue{power set} of a set $M$ is defined as the set of all subsets
of $M$.  This set is written as $2^M$.  Therefore we have
\\[0.2cm] 
\hspace*{1.3cm}
$2^M := \{ x \;|\; x \subseteq M \}$.
...
If two sets $M$ and $N$ are given, the \blue{union}
...
\\[0.2cm]
(For two natural numbers $x$ and $k$, the notation $x \modulo k$ is ued in computer science to denote the
\blue{remainder} that is left when $x$ is divided by $k$.)
How can we write the set $A \cap B$ using the axiom of specification?
\eox
