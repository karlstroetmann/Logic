...
\item The function \texttt{evaluate} takes two arguments.
      ...
      \item The second argument $I$ is a propositional evaluation.  This evaluation is represented as a set of
        ...
        \\[0.2cm]
        Given a propositional variable $p$, the value of $\mathcal{I}(p)$ is then computed
...
\begin{enumerate}
\item In line 6, we check whether $f$ is a string.  In this case
...
\item In line 8 and 9 we deal with the cases where $f$ is equal to verum or falsum.
...
\item In line 10, we consider the case where $f$ is a negation.  Then $f$ has the form
      \\[0.2cm]
      \hspace*{1.3cm}
      \texttt{['¬', $g$]}  
      \\[0.2cm]
        ...
        switch (f[0]) {
            ...
            default: 
                ...
                switch (op) { 
                    ...
                    case '∨': return flatMap(gs, c => hs.map(d => c.union(d)));
    }}} 
\end{minted}
\vspace*{-0.3cm}
...
character \texttt{"*"}.  This character represents those digits that are initially unknown and whose value has
to be computed.
...
Emscripten. \texttt{Logic-Solver} abstracts away the manual conversion to Conjunctive Normal Form (CNF) by
...
algorithm spent 21 minutes laboring through the SAT derivation, the Jupyter notebook that uses \texttt{Logic-Solver}
