...
\\[0.2cm]
\hspace*{1.3cm}
$\bigl\{ \texttt{var}(\texttt{row}, \mathtt{col}) \mid \mathtt{col} \in \{1, \cdots, n \} \bigr\}$
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
\href{https://github.com/karlstroetmann/Logic/blob/master/TypeScript/Chapter-4/Zweistein-Frame.ipynb}{\texttt{github.com/karlstroetmann/Logic/.../TypeScript/Chapter-4/Zweistein-Frame.ipynb}}
\\[0.2cm]
...
\end{minted}
\vspace*{-0.3cm}
\caption{The function $\texttt{variable}(f, i)$.}
\label{fig:var_f_i}
\end{figure}
...
\begin{minted}[ frame         = lines, 
    ...
    function somewhere(x: string): Set<Literal> {
        return range(1, 5).map(i => variable(x, i))
    }
\end{minted}
\vspace*{-0.3cm}
...
\begin{minted}[ frame         = lines, 
    ...
    function atMostOneAt(S: Set<string>, i: number): Set<Clause> {
        return atMostOne(S.map(x => variable(x, i)));
    }
\end{minted}
\vspace*{-0.3cm}
...
\begin{minted}[ frame         = lines, 
    ...
    function sameHouse(a: Variable, b: Variable): Set<Clause> {
        ...
        return Result
            .flatMap(range(1, 5),
                     i => parseKNF(`${variable(a, i)} ↔ ${variable(b, i)}`));
    }
\end{minted}
\vspace*{-0.3cm}
...
function nextTo(a: Variable, b: Variable): Set<Clause> {
    const Formulas = [
        `${variable(a, 1)} → ${variable(b, 2)}`, // House 1
        ...[2, 3, 4].map(
          i => `${variable(a, i)} → ${variable(b, i-1)} ∨ ${variable(b, i+1)}`), 
        `${variable(a, 5)} → ${variable(b, 4)}`  // House 5
    ];
...
\begin{minted}[ frame         = lines, 
    ...
    function leftTo(a: Variable, b: Variable): Set<Clause> {
        const Formulas = [
            ...[1, 2, 3, 4].map(i => `${variable(a, i)} ↔ ${variable(b, i+1)}`),
            `¬${variable(a, 5)}`
        ];
        const Result = set<Clause>();
        return Result.flatMap(Formulas, formula => parseKNF(formula));
...
function allClauses(): Set<Clause> {
    ...
    Result.flatMap(
        [Nations, Drinks, Pets, Brands, Colours],
        S => onePerHouse(S)
    );
    ...
    const puzzleClues = [
        ...
        leftTo("Ivory", "Green"), // The green house is immediately to the right of
                                  // the ivory house.
        sameHouse("OldGold", "Snails"), // The Old Gold smoker owns snails.
        sameHouse("Kools", "Yellow"), // Kools are smoked in the yellow house.
        parseKNF(variable("Milk", 3)), // Milk is drunk in the middle house.
        parseKNF(variable("Norwegian", 1)), // The Norwegian lives in the first house.
        nextTo("Chesterfields", "Fox"), // Chesterfields smoker lives next to the
                                        // fox owner.
        nextTo("Kools", "Horse"), // Kools are smoked next to the horse.
        sameHouse("Luckies", "OrangeJuice"), // The Lucky Strike smoker drinks
                                             // orange juice.
...
\begin{enumerate}
\item In the rooms where there is a tiger, the statement on the sign is \texttt{false}.
\item In the room where the princess is, the statement is \texttt{true}.
\item With regards to the the empty rooms, the situation is a bit more
      complicated, because there are two possibilities:
      \begin{itemize}
      \item Either all the inscriptions on the empty rooms are \texttt{true},
      \item or all the inscriptions on the empty rooms are \texttt{false}.
      \end{itemize}
