...
Furthermore, we introduce mathematical operators for the logical connectives
...
\item Next, we formally define the notions \blue{tautology} and  \blue{satisfiability} for propositional formulas.   
\item We discuss algebraic manipulations of propositional formulas and introduce the notion of
      a  \blue{conjunctive normal form}.

      Some algorithms discussed later require that the propositional formulas have conjunctive normal form.
\item Next we discuss the concept of a  \blue{logical derivation}.  The purpose of a logical derivation is to
...
\item Finally we discuss the algorithm of \blue{Davis and Putnam} for checking the satisfiability of a set of
      propositional formulas.  As an application, we solve the eight queens puzzle with this algorithm.
\end{enumerate}
...
\item Controlling the signals and switches of railroad stations.
      ...
      At a larger railway station, there are several hundred switches and signals that have to be 
      re\-adjusted constantly to provide routes for the trains.
...
\item $\verum \in \mathcal{F}$ and $\falsum \in \mathcal{F}$.
      ...
      \blue{verum}\footnote{Verum is the Latin word for ``true''.},  while $\falsum$ is called
      \blue{Falsum}\footnote{Falsum is the Latin word for ``false''.}.
...
\item The operators  $\vee$ and $\wedge$ associate to the left.  Therefore, we write 
      ...
      \underline{\color{red}Note} that in this lecture the logical operators 
      ...
      In the programming languages  \texttt{C} and \textsl{Java} the  operator ``\texttt{\&\&}'' has a higher
...
\item You should note that the operator $\leftrightarrow$ neither associates to the left nor to the right.
      ...
      \\[0.2cm]
      is \underline{\red{ill-defined}} and has to be parenthesised.  If you encounter this type of expression
...
Therefore, we will implement this evaluation in  \textsl{Python}.
