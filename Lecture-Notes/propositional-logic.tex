...
\begin{Definition}[Satisfiability] \index{satisfiable}
  Let $M$ be a set of propositional formulas.
  If there exists a propositional interpretation $\mathcal{I}$ that satisfies all formulas from $M$, for which
  \\[0.2cm]
  \hspace*{1.3cm}
  $\mathcal{I}(f) = \texttt{True}$ \quad for all $f \in M$
  \\[0.2cm]
  holds, then we call $M$ \textcolor{blue}{satisfiable}. Furthermore, we say that $M$ is \textcolor{blue}{unsatisfiable}\index{unsatisfiable}
  and write 
  \\[0.2cm]
  \hspace*{1.3cm}
  $\textcolor{blue}{M \models \falsum}$, \index{$M \models \falsum$}
...
\item[I.S.:] $k \mapsto k+1$.
  ...
  \item There is no clause
    ...
    \item $D = E \cup \{ p_{k+1} \}$.
          ...
          This case cannot occur due to the condition of the case distiction $(b)$.
    \item $D = E \cup \{ \neg p_{k+1} \}$.
...
Otherwise, a propositional assignment $\I$ should be computed,
under which all clauses from the set \texttt{Clauses} are satisfied.
