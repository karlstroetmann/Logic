...
\begin{minted}[ frame         = lines, 
    ...
    function evaluate(F: Formula, I: string[]): boolean {
...
\item First we define the type that we are going to use.
      ...
      \item A \texttt{Formula} is either a \texttt{Variable} or a nested tuple as discussed above.
      \end{enumerate}
...
\begin{minted}[ frame         = lines, 
    ...
    function allTrue(Gs: Formula[], I: RS<Variable>): boolean {
...
\item In line 29 these formulas are transformed into nested tuples.
...
\begin{minted}[ frame         = lines, 
    ...
    function collectVars(f: Formula): RS<Variable> {
        if (typeof f == 'string') {
            return new RS(f);
        }
        switch (f[0]) {
...
\begin{minted}[ frame         = lines, 
    ...
    function tautology(f: Formula): true | RS<Variable> {
...
\begin{minted}[ frame         = lines, 
    ...
    function collectVariables(M: RS<Clause>): RS<Variable> {
        const r = empty<Variable>();
        return M.reduce((a, c) => a.union(c.map(extractVariable)), r);
    }
    function cutRule(C1: Clause, C2: Clause): RS<Clause> {
...
\begin{minted}[ frame         = lines, 
    ...
    function saturate(Clauses: RS<Clause>): RS<Clause> {
...
\begin{minted}[ frame         = lines, 
    ...
    function findValuation(Clauses: RS<Clause>): false | RS<Literal> {
