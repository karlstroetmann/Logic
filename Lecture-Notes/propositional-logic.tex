...
\item The second argument $I$ is a propositional evaluation.  This evaluation is represented as a set of
  ...
  this type as \texttt{RS}, i.e. we use the following type definition:
  \\[0.2cm]
  \hspace*{1.3cm}
  \texttt{type Set<T extends Value> = RecursiveSet<T>;}
  \\[0.2cm]
  Given a propositional variables $p$, the value of $\mathcal{I}(p)$ is then computed
  by the expression  $I\mathtt{.has}(p)$.
\end{enumerate}
    ...
    function evaluate(F: Formula, I: Set<string>): boolean {
        ...
        switch (F[0]) {
            ...
            case '⊥': 
                return false;
            case '¬': 
                return !evaluate(F[1], I);
            case '∧': 
                return evaluate(F[1], I) && evaluate(F[2], I);
            case '∨': 
                return evaluate(F[1], I) || evaluate(F[2], I);
            case '→': 
                return evaluate(F[1], I) <= evaluate(F[2], I);
...
\item Line 8 deals with the case that the argument $F$ is a propositional variable.  We can recognize this by
      ...
      In this case we have to check whether the variable $p$ is an element of the list $I$, because $p$ is
      interpreted as \texttt{true} if and only if $p \in I$.  The method $L\mathtt{.has}(x)$
      takes a set $L$ and checks, whether $x$ is an element of $L$.
...
\item If $F$ has the form $\neg G$, we recursively evaluate $G$ given the evaluation $I$ and negate the result.
      Since $F$ is a list of the form [\tsq{$\neg$}, $G$], the subformula $G$ satisfies $G = F[1]$.
\item If $F$ has the form $G \wedge H$, we recursively evaluate
      $G$ and $H$ using $I$.  The results are then combined with the \textsl{TypeScript} operator
      ``\texttt{\&\&}''.
\item If $F$ has the form $G \vee H$, we recursively evaluate
      $G$ and $H$ using $I$.  The results are then combined with the \textsl{TypeScript} operator
      ``\texttt{||}''.
\item If $F$ has the form $G \rightarrow H$, we recursively evaluate
      $G$ and $H$ using $I$.  Then we exploit the fact that in \textsl{TypeScript} the values \texttt{true} and
...
\begin{minted}[ frame         = lines, 
    ...
    type Set<T extends Value> = RecursiveSet<T>;
...
\item Next, we define the function \texttt{transform}.  This function takes a propositional formula that is
      represented as a string and transforms it into a nested list.
\item Line 6 defines the set $P$ of propositional variables.  We use the propositional variable \texttt{a} to
...
\item In line 29 these formulas are transformed into nested lists.
...
\item Line 35 computes the set of all propositional variables that render all formulas from \texttt{Fs} true.
...
\\[0.2cm]
\hspace*{1.3cm}
\href{https://github.com/karlstroetmann/Logic/blob/master/TypeScript/Chapter-4/02-Usual-Suspects.ipynb}{Chapter-4/02-Usual-Suspects.ipynb}.
\\[0.2cm]
...
\begin{minted}[ frame         = lines, 
    ...
    function collectVars(f: Formula): Set<Variable> {
        ...
        switch (f[0]) {
            ...
            default: // case '↔', '→', '∧', '∨':
                return collectVars(f[1]).union(collectVars(f[2]));
        }
    }
\end{minted}
\vspace*{-0.3cm}
...
\begin{minted}[ frame         = lines, 
    ...
    function tautology(f: Formula): true | Set<Variable> {
        ...
        for (const I of collectVars(f).powerset()) {
            if (!evaluate(f, I)) {
                return I;
            }
        }
...
\item In line 5 we deal with the cases where $f$ is equal to Verum or Falsum.
      ...
      \textsl{TypeScript} by the list \texttt{('⊥',)}.  In this case, $f$
...
\end{minted}
\vspace*{-0.3cm}
  \caption{Computing the negation normal form, the function \texttt{neg}.}
  \label{fig:nnf}
\end{figure}
...
\begin{minted}[ frame         = lines, 
    ...
    function cnf(f: NNF): CNF {
        if (typeof f == 'string') { 
            return new Set<Clause>(new Set<Literal>(f));
        }
        switch (f[0]) {
            case '⊤':
                return new Set<Clause>();     
            case '⊥':
                return new Set<Clause>(new Set<Literal>());
            case '¬': {
                const clause = new Set<Literal>(new Tuple('¬', f[1]));
                return new Set<Clause>(clause);
            }            
            case '∧': {
...
\begin{minted}[ frame         = lines, 
    ...
    function empty<T extends Value>(): Set<T> {
        return new RecursiveSet<T>()
    }
    function single<T extends Value>(x: T): Set<T> {
        return new RecursiveSet<T>(x)
    }
    function collectVariables(M: Set<Clause>): Set<Variable> {
        const r = empty<Variable>();
        return M.reduce((a, c) => a.union(c.map(extractVariable)), r);
    }
    function cutRule(C1: Clause, C2: Clause): Set<Clause> {
        return C1.reduce((r, l) => {
            const lBar = complement(l);
            if (C2.has(lBar)) {
                // (C1 \ {l}) U (C2 \ {lBar})
                r.add(C1.difference(single(l)).union(C2.difference(single(lBar))));
            }
            return r;
        }, empty<Clause>()); 
    }  
...
\begin{minted}[ frame         = lines, 
    ...
    function saturate(Clauses: Set<Clause>): Set<Clause> {
        ...
        while (true) {
            ...
            let newClauses = empty<Clause>();
            for (const C1 of currentClauses) {
                for (const C2 of currentClauses) {
                    const derived = cutRule(C1, C2)
                    if (derived.some(D => D.isEmpty())) {
                        return single(empty<Literal>());
                    }
                    newClauses = newClauses.union(derived);
                }
            }
            currentClauses = currentClauses.union(newClauses);
...
\begin{minted}[ frame         = lines, 
    ...
    function findValuation(Clauses: Set<Clause>): false | Set<Literal> {
        ...
        return Variables.reduce((Literals, p) => {
            const Complements = Literals.map(complement);         
            if (saturated.some(C => C.has(p) && restFalse(C, p, Complements))) {
                Literals.add(p);
            } else {
                Literals.add(complement(p));
            }        
            return Literals;
        }, empty<Literal>()); 
    }
...
\\[0.2cm]
\hspace*{1.3cm}
\href{https://github.com/karlstroetmann/Logic/blob/master/TypeScript/Chapter-4/Prince-Tiger.ipynb}{\texttt{github.com/karlstroetmann/Logic/blob/master/TypeScript/Chapter-4/Prince-Tiger.ipynb}}
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
\href{https://github.com/karlstroetmann/Logic/blob/master/TypeScript/Chapter-4/Zweistein.ipynb}{\texttt{github.com/karlstroetmann/Logic/blob/master/TypeScript/Chapter-4/Zweistein.ipynb}}
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
\href{https://github.com/karlstroetmann/Logic/blob/master/TypeScript/Chapter-4/10-Sudoku.ipynb}{\texttt{github.com/karlstroetmann/Logic/blob/master/TypeScript/Chapter-4/10-Sudoku.ipynb}}
...
A \href{https://github.com/conda/pycosat}{TypeScript API} to this solver is available as
