...
We begin by defining the basic types and a utility function to generate sequences of numbers. This is shown in Figure \ref{fig:logicsolver-range}.
...
The TypeScript types \texttt{Variable}, \texttt{NegatedVariable}, \texttt{Literal}, and \texttt{Clause} establish how boolean logic primitives are formatted as strings. The \texttt{range} function acts as a helper to easily generate arrays of numbers, such as $[1, \dots, 9]$, which we need for iterating over the Sudoku grid.
...
The \texttt{varName} function creates strings like \texttt{"Q<1,2,3>"} to represent the boolean variable stating that row 1, column 2 holds the digit 3. The \texttt{complement} function returns the logical negation of a literal by either adding or removing a leading minus sign (\texttt{-}), satisfying the format expected by \texttt{Logic-Solver}.
...
In Figure \ref{fig:logicsolver-exactlyOneForDigit}, we define \texttt{Position} as a tuple of row and column. The \texttt{exactlyOneForDigit} function leverages \texttt{Logic.exactlyOne}, a powerful cardinality constraint built directly into \texttt{Logic-Solver}. It takes a list of positions \texttt{L} and ensures that the specified \texttt{digit} is placed in exactly one of those positions.
...
\begin{minted}[ frame         = lines, 
    ...
    function exactlyOneForEveryDigit(L: Position[]): Logic.Formula[] {
        return range(1, 9).map(d => exactlyOneForDigit(L, d));
    }
\end{minted}
\vspace*{-0.3cm}
  \caption{The function \texttt{exactlyOneForEveryDigit}.}
...
The \texttt{exactlyOneForEveryDigit} function (Figure \ref{fig:logicsolver-exactlyOneForEveryDigit}) maps over the numbers 1 through 9, returning an array of formulas. This effectively states that within the given set of coordinates \texttt{L} (which will later represent a full row, column, or block), every digit from 1 to 9 must occur exactly once.

Similarly, we must ensure that every single cell contains exactly one digit.
...
The \texttt{exactlyOneDigit} function in Figure \ref{fig:logicsolver-exactlyOneDigit} enforces that for a fixed \texttt{row} and \texttt{col}, exactly one of the variables corresponding to digits 1 through 9 evaluates to \texttt{true}.
...
The \texttt{constraintsFromPuzzle} function (Figure \ref{fig:logicsolver-constraintsFromPuzzle}) iterates over the $9 \times 9$ array. For every non-asterisk entry, it generates the corresponding variable string and adds it to the \texttt{Facts} array. It then combines all these facts using \texttt{Logic.and}, creating a single formula that anchors the starting state of the board.
...
The \texttt{getBlockCells} function in Figure \ref{fig:logicsolver-getBlockCells} takes the 0-based index of a macro-block (\texttt{r} and \texttt{c}, both spanning 0 to 2) and returns the nine 1-based coordinates \texttt{[row, col]} on the board that belong to that block.
...
The \texttt{allConstraints} function (Figure \ref{fig:logicsolver-allConstraints}) takes a \texttt{Logic.Solver} instance and sequentially feeds it all our rules via \texttt{solver.require(...)}. It ensures:
\begin{enumerate}
...
Executing this entire program yields an astonishing performance improvement.  While the Davis-Putnam
alternating spent 21 minutes laboring through the SAT derivation, \texttt{11-Logic-Solver-Sudoku.ipynb}
