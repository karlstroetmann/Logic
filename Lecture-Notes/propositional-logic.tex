...
\item If $f$ has the form $g \odot h$, where $\odot$ is some binary operator, i.e.~
      ...
      $g$ and $h$ using $I$.  The resulting Boolean values are stored in $gs$ and $hs$ and are
      ...
      \item If $\odot = \rightarrow$ then we exploit the fact that in \textsl{TypeScript} the values \texttt{true} and
            ...
            \\[0.2cm]
            Hence we combine $gs$ and $hs$ using the operator ``\texttt{<=}''.
      \item If $\odot = \leftrightarrow$ we exploit the fact that the formula $g \leftrightarrow h$ is true if and
...
\item Line 38 computes the set of all propositional variables that render all formulas from \texttt{Fs} true.
...
\begin{minipage}{0.95\linewidth}
  ...
  \textbf{Xenia}. Xenia is the youngest child. The following facts are given: 
  \begin{enumerate}
  ...
  \item Either Helen or Xenia will visit the Waltons.
...
\begin{minted}[ frame         = lines, 
    ...
    function eliminateBiconditional(f: Formula): Formula1 {
        ...
        switch (f[0]) {
            case '↔': { const [_, g, h] = f;
                        const [gs, hs] = [g, h].map(eliminateBiconditional);
                        return ['∧', ['→', gs, hs], ['→', hs, gs]]; }
            case '⊤':
            case '⊥': return f;        
            case '¬': { const [_, g] = f;
                        const gs = eliminateBiconditional(g);
                        return ['¬', gs];                     }
            default:  { const [op, g, h] = f;
                        const [gs, hs] = [g, h].map(eliminateBiconditional);
                        return [op, gs, hs];                  }
        }
    }                  
...
\begin{minted}[ frame         = lines, 
    ...
    function eliminateConditional(f: Formula1): Formula2 {
        ...
        switch (f[0]) {
            ...
            case '⊥': return f;
            case '→': { const [_, g, h] = f;
                        const [gs, hs] = [g, h].map(eliminateConditional);
                        return ['∨', ['¬', gs], hs];                       }
            case '¬': { const [_, g] = f;
                        const gs = eliminateConditional(g);
                        return ['¬', gs];                                  }
            default:  { const [op, g, h] = f;
                        const [gs, hs] = [g, h].map(eliminateConditional);
                        return [op, gs, hs];                               }
...
\begin{minted}[ frame         = lines, 
    ...
    function nnf(f: Formula2): NNF {
        ...
        switch (f[0]) {
            ...
            default:  { const [op, g, h] = f; return [op, nnf(g), nnf(h)]; }
        }
    }
\end{minted}
\vspace*{-0.3cm}
...
\begin{minted}[ frame         = lines, 
    ...
    function cnf(f: NNF): CNF {
        ...
        switch (f[0]) {
            ...
            default: {
                const [op, g, h] = f;
                const [gs, hs] = [g, h].map(cnf); 
                switch (op) { 
                    case '∧': return gs.union(hs);
                    case '∨': { const pairs = gs.cartesianProduct(hs);
