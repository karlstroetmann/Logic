...
solve :-
    find_path( side( 1, 1, 1, 1), side( 0, 0, 0, 0), [ side( 1, 1, 1, 1) ], Path ),
    nl,
    write('Lösung:' ),
    nl, nl,
    print_path(Path).
...
% edge( +Point, -Point ).
edge( side( W, G, C, 1), side( WN, GN, CN, 0) ) :-
        !,
        between( 0, W, WF),
        between( 0, G, GF),
        between( 0, C, CF),
...
edge( Side, Next ) :-
        switch( Side, Switched ),
        edge( Switched, SwitchedNext ),
        switch( SwitchedNext, Next ).

% Aufgabe 3: Implementieren Sie ein Prädikat switch/2.
% switch( +Point, -Point ).
switch( side( W1, G1, C1, F1 ), side( W2, G2, C2, F2) ) :-
        W2 is 1 - W1,
        G2 is 1 - G1,
        C2 is 1 - C1,
        F2 is 1 - F1.

% Aufgabe 4: Implementieren Sie ein Prädikat problem/4.
% problem( +Number, +Number, +Number ).
problem( Wolf, 1, Cabbage) :-
        Wolf + Cabbage >= 1.
