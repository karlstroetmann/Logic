...
\exercise
Bei den folgenden beiden Teilaufgaben sollen Sie den Operator ``\texttt{**}'', mit dem Sie in \textsc{SetlX}
die Potenz-Menge einer Menge berechnen k\"onnen, \underline{\red{nicht}} benutzen.
\begin{enumerate}
...
\item Implementieren Sie eine Funktion \texttt{power}, so dass $\texttt{power}(M)$ f\"ur eine
      gegebene Menge $M$ die Potenz-Menge von $M$ berechnet.  Sie sollen diese Funktion rekursiv implementieren
      und sich dabei an dem Beweis im Skript zur Vorlesung 
      ``\href{https://github.com/karlstroetmann/Lineare-Algebra/blob/master/Skript/lineare-algebra.pdf}{Lineare Algebra}'' 
      orientieren, mit dem wir gezeigt haben, dass f¸r eine endliche Menge $M$ die Gleichung
      \\[0.2cm]
      \hspace*{1.3cm}
      $\displaystyle\texttt{card}\bigl(2^M\bigr) = 2^{\texttt{card}(M)}$
      \\[0.2cm]
      gilt.
\item Geben Sie nun eine alternative Implementierung der Funktion \texttt{power} an, bei der Sie die Funktion
      \texttt{subsets} aus Teil (a) dieser Aufgabe verwenden.
\end{enumerate}
\pagebreak
