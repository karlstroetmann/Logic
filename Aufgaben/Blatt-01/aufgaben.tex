...
Eine Zahl $n \in \mathbb{N}$ heiﬂt \emph{perfekt}, wenn $n$ gleich der Summe aller echten Teiler von
...
\item Implementieren Sie nun eine Prozedur \texttt{ggt}, so dass der Aufruf
      $\mathtt{ggt}(m,n)$ den grˆﬂten gemeinsamen Teiler der beiden Zahlen $m$ und $n$
      berechnet. 
\end{enumerate}
\vspace{0.3cm}
...
die Potenz-Menge einer Menge berechnen k\"onnen, nicht benutzen.
\begin{enumerate}
...
\item Implementieren Sie eine Funktion \texttt{power} so dass $\texttt{power}(M)$ f\"ur eine
      gegebene Menge $M$ die Potenz-Menge von $M$ berechnet.

      \textbf{Hinweis}:  Am einfachsten ist es, die Funktion  $\texttt{power}(M)$ rekursiv zu
      implementieren. 
\end{enumerate}
\pagebreak
...
Karten $\pair(8,\spade)$ und $\pair(9,\spade)$ erhalten.  Schreiben Sie ein
