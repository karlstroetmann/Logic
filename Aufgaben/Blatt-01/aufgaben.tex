...
\begin{enumerate}
\item Implementieren Sie eine Funktion \texttt{echteTeiler}, so dass der Aufruf
      $\texttt{echteTeiler}(n)$ f¸r eine nat¸rliche Zahl $n$ die Menge aller echten Teiler von
      $n$ berechnet.
\item Implementieren Sie eine Funktion \texttt{isPerfect}, so dass der Aufruf
      $\texttt{isPerfect}(n)$ f¸r eine nat¸rliche Zahl $n$ genau dann das Ergebnis \texttt{True}
      zur¸ck gibt, wenn $n$ eine perfekte Zahl ist.
\item Berechnen Sie die Menge aller perfekten Zahlen, die kleiner als $10\,000$ sind.
\end{enumerate}
\textbf{Hinweis:} Versuchen Sie, bei der Lˆsung dieser Aufgabe ohne \texttt{for}-Schleifen
oder \texttt{while}-Schleifen auszukommen.  Sie sollen statt dessen mit Mengen arbeiten.
Dieser Hinweis gilt auch f¸r die folgenden Aufgaben.
\vspace{0.3cm}

\exercise
\begin{enumerate}
\item Implementieren Sie eine Funktion \texttt{gt}, so dass der Aufruf $\texttt{gt}(m,n)$ f¸r zwei nat¸rliche
...
\item Implementieren Sie nun eine Funktion \texttt{ggt}, so dass der Aufruf $\texttt{ggt}(m,n)$ 
...
\exercise
Implementieren Sie  eine Funktion \texttt{kgv}, so dass der Aufruf
...
\begin{enumerate}
\item Implementieren Sie eine Funktion \texttt{pythagoras}, so dass der Aufruf
      \\[0.2cm]
      \hspace*{1.3cm}
      $\texttt{pythagoras}(n)$
      \\[0.2cm]
      die Menge aller geordneten  pythagoreischen Tripel $\langle a,b,c \rangle$ berechnet, f¸r die $c \leq n$ ist.
\item Ein pythagoreisches Tripel $\langle a,b,c \rangle$ ist ein \blue{reduziertes} Tripel, wenn
...
\item Implementieren Sie eine Funktion \texttt{reducedPythagoras}, so dass der Aufruf
      \\[0.2cm]
      \hspace*{1.3cm}
      ...
      Berechnen Sie mit dieser Funktion alle reduzierten geordneten pythagoreischen Tripel
      $\langle a,b,c \rangle$, f¸r die $c \leq 50$ ist. 
\end{enumerate}
...
\textsl{Python}-Programm, dass die folgenden Fragen beantworten.
\begin{enumerate}
...
feste Zahl $s$ vorgegeben.  Entwickeln Sie eine \textsl{Python} Funktion \texttt{numberDiceRolls}, so dass der Aufruf
\\[0.2cm]
\hspace*{1.3cm}
...
\noindent
\textbf{Hinweis}:  Implementieren Sie die Funktion durch Rekursion nach $n$.
\end{document}
