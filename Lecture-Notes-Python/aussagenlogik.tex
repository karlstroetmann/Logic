...
\blue{einfachen Aussagen} verstehen Sätze, die 
\begin{itemize}
...
Formeln \blue{Wahrheitswerte} zu.  Damit diese möglich ist, definieren wir
...
\begin{enumerate}
\item $\verum$ wird repräsentiert durch den String \texttt{'N\symbol{92}up tack'}, der das Unicode-Zeichen
      ``$\top$'' darstellt: 
      \\[0.2cm]
      \hspace*{1.3cm}
      $\textsl{rep}(\verum) := \texttt{'N\symbol{92}up tack'}$.
\item $\falsum$  wird repräsentiert durch den String \texttt{'N\symbol{92}down tack'}, der das Unicode-Zeichen
     ``$\bot$'' darstellt:
      \\[0.2cm]
      \hspace*{1.3cm}
      $\textsl{rep}(\falsum) := \texttt{'N\symbol{92}down tack'}$.
...
\item Ist $f$ eine aussagenlogische Formel, so repräsentieren wir $\neg f$ als verschachteltes Tupel,
      bei dem wir das Unicode-Zeichen \texttt{'N\symbol{92}not sign'} verwenden, denn dieses Zeichen wird
      als ``$\neg$'' dargestellt:
      \\[0.2cm]
      \hspace*{1.3cm} 
      $\textsl{rep}(\neg f) := \bigl(\texttt{'N\symbol{92}not sign'}, \textsl{rep}(f)\bigr)$.
\item Sind $f_1$ und $f_2$ aussagenlogische Formel, so repräsentieren wir $f_1 \wedge f_2$ mit
      Hilfe des Unicode-Zeichens  \texttt{'N\symbol{92}logical and'}:
      \\[0.2cm]
      \hspace*{1.3cm} 
      $\textsl{rep}(f \vee g) := \bigl(\texttt{'N\symbol{92}logical and'}, \textsl{rep}(f), \textsl{rep}(g)\bigr)$.
\item Sind $f_1$ und $f_2$ aussagenlogische Formel, so repräsentieren wir $f_1 \vee f_2$ mit
      Hilfe des Unicode-Zeichens  \texttt{'N\symbol{92}logical or'}:
      \\[0.2cm]
      \hspace*{1.3cm} 
      $\textsl{rep}(f \vee g) := \bigl(\texttt{'N\symbol{92}logical or'}, \textsl{rep}(f), \textsl{rep}(g)\bigr)$.
\item Sind $f_1$ und $f_2$ aussagenlogische Formel, so repräsentieren wir $f_1 \rightarrow f_2$ mit
      Hilfe des Unicode-Zeichens  \texttt{'N\symbol{92}rightward arrow'}:
      \\[0.2cm]
      \hspace*{1.3cm} 
      $\textsl{rep}(f \rightarrow g) := \bigl(\texttt{'N\symbol{92}rightward arrow'}, \textsl{rep}(f), \textsl{rep}(g)\bigr)$.
\item Sind $f_1$ und $f_2$ aussagenlogische Formel, so repräsentieren wir $f_1 \leftrightarrow f_2$ mit
      Hilfe des Unicode-Zeichens  \texttt{'N\symbol{92}left right arrow'}:
      \\[0.2cm]
      \hspace*{1.3cm} 
      $\textsl{rep}(f \leftrightarrow g) := \bigl(\texttt{'N\symbol{92}left right arrow'}, \textsl{rep}(f), \textsl{rep}(g)\bigr)$.
\end{enumerate}
...
\item In den Zeilen 7 -- 17 definieren wir die Formeln $f_1$, $\cdots$, $f_6$      .
      Wir müssen hier die Formeln in die \textsl{Python}-Repräsentation bringen.
...
$
\\[0.2cm]
Um die aussagenlogische Belegung $\mathcal{I}$ in \textsl{Python} darstellen zu können,
...
\ref{fig:evaluate.py} auf Seite \pageref{fig:evaluate.py} gezeigten Programm.
