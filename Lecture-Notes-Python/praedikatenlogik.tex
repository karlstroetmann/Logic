...
\item Diese Terme werden aus \blue{Variablen} und \blue{Funktions-Zeichen}
      zusammengesetzt: 
      \\[0.2cm]
      \hspace*{1.3cm}
      $\textsl{vater}(x),\quad \textsl{mutter}(\textsl{isaac}), \quad x+7, \quad \cdots$.
\item Verschiedene Objekte werden durch \blue{Prädikats-Zeichen} in Relation gesetzt:
      \\[0.2cm]
      \hspace*{1.3cm}
...
\subsection{Implementierung prädikatenlogischer Strukturen in \textsl{Python}}
...
$
\\[0.2cm]
gegeben worden.  Hierbei ist also ``$\mathrm{e}$'' ein 0-stelliges Funktions-Zeichen, ``$*$'' ist
...
$\{ a, b \}$ besteht.  In \textsl{Python} können wir diese Struktur durch den in Abbildung
\ref{fig:Group.ipynb} gezeigten Code implementieren.
...
\begin{Verbatim}[ frame         = lines, 
    ...
    Equals    = { (x, x) for x in U }
    ConstantA = { (): a }
    J = { "E": ConstantA, "Multiply": Product, "Equals": Equals }
    S = (U, J)
    I = { "x": a, "y": b, "z": a }
\end{Verbatim}
\vspace*{-0.3cm}
...
\item Zur Abkürzung haben wir in den Zeile 1 und 2 die Variablen $\texttt{a}$ und $\texttt{b}$
      ...
      Funktions-Zeichens ``$*$'' einfacher angeben.
...
\item In Zeile 5 haben wir  die Interpretation $=^\mathcal{J}$ des
      Prädikats-Zeichens ``$=$'' als Menge aller Paare der Form $[x, x]$ dargestellt.
\item In Zeile 6 fassen wir die einzelnen Interpretationen zu der Relation $\texttt{J}$
      zusammen, so dass für ein Funktions-Zeichen $f$ die Interpretation $f^\mathcal{J}$ durch
      den Wert $\texttt{J}(f)$ gegeben ist. 

      Da wir später den in \textsl{Python} eingebauten Parser verwenden wollen, müssen wir die
      prädikatenlogischen Formeln als
      Terme darstellen.  Dazu stellen wir den Operator ``\texttt{*}'' durch den Funktor ``\texttt{@@@product}''
      dar und für das Prädikats-Zeichen ``\texttt{=}'' verwenden wir den Funktor ``\texttt{@@@equals}'',
      denn das sind die Funktoren, die von dem in \textsl{Python} integrierten Parser intern benutzt werden.
      Das neutrale Element ``$1$'' stellen wir durch den Funktor ``\texttt{@E}'' dar, so dass später der
      Ausdruck ``$1$'' durch den Term ``\texttt{@E()}'' repräsentiert wird.
\item Die Interpretation $\texttt{J}$ wird dann in Zeile 7 mit dem
      Universum $\texttt{U}$ zu der Struktur $\texttt{S}$ zusammengefasst.  
...
$\texttt{evalFormula}(f, S, I)$, der als Argumente eine prädikatenlogische Formel $f$, eine Struktur
$\mathcal{S}$ und eine Variablen-Belegung $\mathcal{I}$ übergeben werden.  Die Formel wird
dabei als Term dargestellt.  

Die Auswertung einer prädikatenlogischen Formel ist nun analog zu der in Abbildung \ref{fig:pl-evaluate.stlx}
auf Seite \pageref{fig:pl-evaluate.stlx} gezeigten Auswertung aussagenlogischer Formeln.  
...
$\struct\bigl(\mathcal{I}, \forall x\colon h\bigr) \;:=\; \left\{
      \begin{array}{ll}
         \mathtt{true}  & \mbox{falls}\; \struct(\mathcal{I}[x/c], h) = \mathtt{true}\quad \mbox{für alle}\; c\in \mathcal{U}\;\mbox{gilt}; \\
         \mathtt{false} & \mbox{sonst}.
      \end{array}
      \right.
$
...
\\[0.2cm]
Die Implementierung dieser Prozedur ist in \myfig{modify.stlx} gezeigt.
...
unterstützt.  Wir können also direkt testen, ob die 
...
\myfig{evalAtomic.stlx} zeigt die Auswertung atomarer Formeln.  Um eine atomare Formel der Form
\\[0.2cm]
\hspace*{1.3cm}
$a = p(t_1, \cdots, t_n)$ 
\\[0.2cm]
auszuwerten, verschaffen wir uns in Zeile 4 zunächst die dem Prädikats-Zeichen $p$ in der
Struktur $\mathcal{S}$ zugeordnete Menge $\mathtt{pJ}$.  Anschließend werten wir die Argumente
 $t_1, \cdots, t_n$ aus.  Dabei erhalten wir die Liste $[t_1, \cdots, t_n]$ der Argumente des
 Prädikats-Zeichens $p$ durch Anwendung der Funktion $\mathtt{args}$ in Zeile 5.
...
\\[0.2cm]
Dies ist eine Liste von Werten aus dem Universum $\mathcal{U}$ der Struktur $\mathcal{S}$.  Wir prüfen nun in
Zeile 7, ob diese Liste tatsächlich ein Element der Menge $\mathtt{pJ}$ ist.  Wenn dies so ist, gibt die
Funktion $\texttt{evalAtomic}$ den Wert $\texttt{true}$ zurück, sonst wird $\texttt{false}$ zurück gegeben. 
...
Die in \myfig{evalTerm.stlx} gezeigte Prozedur $\texttt{evalTerm}()$ arbeitet wie folgt:
...
\begin{enumerate}
\item Falls $t$ eine Variable ist, so geben wir in Zeile 4 einfach den Wert zurück, der in
      der Variablen-Belegung $\mathcal{I}$ für diese Variable eingetragen ist.  
      Die Variablen-Belegung $\mathcal{I}$ wird dabei durch eine
      zweistellige Relation dargestellt, die wir als Funktion benutzen.

      Variablen werden von dem in \textsl{Python} vorgegebenen Parser als Terme der Form
      \\[0.2cm]
      \hspace*{1.3cm}
      \texttt{@@@variable($v$)}
      \\[0.2cm]
      ...
      \\[0.2cm]
      \hspace*{1.3cm}
      \texttt{parse(\symbol{34}x\symbol{34})}
      \\[0.2cm]
      als Ergebnis den Term
      \\[0.2cm]
      \hspace*{1.3cm}
      \texttt{@@@variable(\symbol{34}x\symbol{34})}.
\item Falls der auszuwertende Term $t$ die Form 
      \\[0.2cm]
      \hspace*{1.3cm}
      $t = f(t_1,\cdots,t_n)$
      \\[0.2cm]
      hat,  werden in Zeile 10 zunächst rekursiv die 
      ...
      $f$ für die gegebenen
      Argumente auszuwerten, wobei in Zeile 12 der Fall betrachtet wird, dass tatsächlich Argumente
      vorhanden sind, während  in Zeile 14 der Fall behandelt wird, dass es sich bei dem
...
in Abbildung \ref{fig:Group.ipynb} gezeigte Struktur die Axiome einer
\href{https://en.wikipedia.org/wiki/Abelian_group}{kommutativen Gruppe}
erfüllt.  Diese Axiome sind wie folgt:
\begin{enumerate}
...
\item Entwickeln Sie ein \textsl{Python}-Programm, dass die obige Formel in allen Strukturen
...
Im nächsten Abschnitt gehen wir daran, den früher  erwähnten Kalkül $\vdash$ für die
Prädikatenlogik zu definieren.  
Es zeigt sich, dass die Arbeit wesentlich einfacher wird, wenn wir uns auf bestimmte
Formeln, sogenannte \blue{prädikatenlogische Klauseln}, beschränken.  Wir zeigen in diesem Abschnitt, dass jede Formel-Menge $M$
...
\begin{Satz}
  ...
  \item Falls $x$ eine Variable ist, für die  $x \not\in \FV(g) \cup \BV(g)$ gilt, so haben wir die folgenden Äquivalenzen:
    \begin{enumerate}
    \item $\models \big(\forall x\colon f) \vee g \leftrightarrow \forall x\colon (f \vee g)$ \quad und \quad $\models g \vee \big(\forall x\colon f) \leftrightarrow \forall x\colon (g \vee f)$,
    \item $\models \big(\exists x\colon f) \wedge g \leftrightarrow \exists x\colon (f \wedge g)$ \quad und \quad $\models g \wedge \big(\exists x\colon f) \leftrightarrow \exists x\colon (g \wedge f)$.
    \end{enumerate}
  \end{enumerate}
\end{Satz}
...
\begin{Definition}[Erfüllbarkeits-Äquivalenz] \hspace*{\fill} \\
...
Dabei sind die $k_i$ Disjunktionen von \blue{Literalen}.  In der Prädikatenlogik ist ein
\blue{Literal} entweder eine atomare Formel oder die Negation einer atomaren Formel.  Wenden wir
hier  die Äquivalenz 
\\[0.2cm]
\hspace*{1.3cm}
...
\begin{Satz}[{\color{blue}Widerlegungs-Vollständigkeit}] \hspace*{\fill} \\
