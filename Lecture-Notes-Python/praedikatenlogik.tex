...
\item Dieses Tupel dient dann in Zeile 9 als Argument für das Dictionary $f^\mathcal{J}$.  Der in diesem
      Dictionary für die Argumente abgelegte Wert ist dann das Ergebnis der Auswertung des Terms $t$.
\end{enumerate}
...
Abbildung \ref{fig:evalAtomic.ipynb} zeigt die Auswertung einer atomaren Formel.  Eine atomare Formel $a$ hat
...
It is time to see an application of first order logic. One of these applications is \blue{constraint programming}.  
\emph{Constraint programming} is an example of the \blue{declarative programming} paradigm.  The idea is that
in order to solve a given problem, this problem is only \blue{specified} and then this \blue{specification} is
...
the programmer only has to specify the problem precisely.  Usually, the specification of a problem is much
shorter than the specification of an algorithm to solve the problem.  This approach works well for those problems
that can be specified using first order logic.  In the remainder of this section is structured as follows:
\begin{enumerate}
...
\\[0.2cm]
\hspace*{1.3cm}
$\mathcal{I}(V_1) := 4,\; \mathcal{I}(V_2) := 8,\; \mathcal{I}(V_3) := 1,\; \mathcal{I}(V_4) := 2,\; \mathcal{I}(V_5) := 6,\; \mathcal{I}(V_6) := 2,\; \mathcal{I}(V_7) := 7,\; \mathcal{I}(V_8) := 5$,
\\[0.2cm]
...
variable assignments as dictionaries.  For example, $\mathcal{I}$ would then be represented as the dictionary
\\[0.2cm]
\hspace*{1.3cm}
$\mathcal{I} = \bigl\{ \mathrm{V}_1:4, \mathrm{V}_2:8, \mathrm{V}_3:1, \mathrm{V}_4:2, 
...
\\[0.2cm]
\hspace*{1.3cm}
$\mathcal{B} := \bigl\{ \mathrm{1}_V:4, \mathrm{V}_2:8, \mathrm{V}_3:1) \bigr\}$,
\\[0.2cm]
then $\mathcal{B}$ is a partial assignment and $\texttt{dom}(\mathcal{B}) = \{ \mathrm{V}_1, \mathrm{V}_2, \mathrm{V}_3 \}$.  This
...
Next, we evaluate all constraints that mention only the variable $x_1$ and check whether these constraints
...
\item The procedure $\texttt{solve}$ takes a constraint satisfaction problem $\texttt{CSP}$ as input and tries
      to find a solution.    
      \begin{enumerate}
      \item  First, the $\texttt{CSP}$ is split into its three components.
      ...
      \item Next, we call the procedure $\texttt{backtrack\_search}$ to compute a solution of $\texttt{CSP}$.
            This procedure is enclosed in a \texttt{try}-\texttt{except}-block.
            The procedure $\texttt{backtrack\_search}$ either returns a solution or, if it is not able to
...
\item Otherwise, we have to extend the partial $\texttt{Assignment}$.  In order to do so, we first have to
      ...
      We pick the next variable in the list \texttt{Variables} that is yet unassigned.
...
nun um sogenannte \blue{prädikatenlogische Klauseln}.  Wir zeigen in diesem Abschnitt, dass jede Formel-Menge $M$
